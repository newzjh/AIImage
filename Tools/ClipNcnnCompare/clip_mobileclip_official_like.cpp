...
{
    long long load_ms = 0;
    long long text_prep_ms = 0;
};

struct ImageRunTimings
{
    long long image_ms = 0;
    long long score_ms = 0;
    long long total_ms = 0;
};

class MobileClipOfficialLike
{
public:
    MobileClipOfficialLike(const std::string& model_root, Timings& timings)
    {
        ...
        timings.load_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
    }

    std::vector<float> encode_image(const std::string& image_path, ImageRunTimings& timings)
    {
        auto t0 = std::chrono::steady_clock::now();
        int w = 0;
        int h = 0;
        ...
        auto t1 = std::chrono::steady_clock::now();
        timings.image_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
        return result;
    }

    std::vector<float> encode_text(const std::vector<int>& tokens)
    {
...
int main(int argc, char** argv)
{
    if (argc < 4)
    {
        std::cerr << "Usage: clip_mobileclip_official_like <clip_root> <model_name> <image_path1> [image_path2 ...]\n";
        return 1;
    }
    ...
    try
    {
        ...
        MobileClipOfficialLike model(model_root, timings);
        auto text_t0 = std::chrono::steady_clock::now();
        std::vector<std::vector<float>> text_features(labels.size());
        for (size_t i = 0; i < labels.size(); i++)
        {
            auto tokens = tokenizer.tokenize(labels[i].second);
            text_features[i] = model.encode_text(tokens);
        }
        auto text_t1 = std::chrono::steady_clock::now();
        timings.text_prep_ms = std::chrono::duration_cast<std::chrono::milliseconds>(text_t1 - text_t0).count();

        std::cout << "STARTUP\tload_ms\t" << timings.load_ms
                  << "\ttext_prep_ms\t" << timings.text_prep_ms
                  << "\tstartup_total_ms\t" << (timings.load_ms + timings.text_prep_ms) << "\n";

        for (size_t image_index = 0; image_index < image_paths.size(); image_index++)
        {
            ImageRunTimings image_timings;
            auto total_t0 = std::chrono::steady_clock::now();
            auto image_features = model.encode_image(image_paths[image_index], image_timings);

            auto score_t0 = std::chrono::steady_clock::now();
            std::vector<double> logits(labels.size());
            for (size_t i = 0; i < labels.size(); i++)
                logits[i] = dot_product(image_features, text_features[i]) * 100.0;
            ...
            auto score_t1 = std::chrono::steady_clock::now();
            image_timings.score_ms = std::chrono::duration_cast<std::chrono::milliseconds>(score_t1 - score_t0).count();

            auto total_t1 = std::chrono::steady_clock::now();
            image_timings.total_ms = std::chrono::duration_cast<std::chrono::milliseconds>(total_t1 - total_t0).count();

            const size_t best = order.empty() ? 0 : order[0];
            std::cout << "IMAGE\t" << image_paths[image_index]
                      << "\timage_ms\t" << image_timings.image_ms
                      << "\tscore_ms\t" << image_timings.score_ms
                      << "\ttotal_ms\t" << image_timings.total_ms
                      << "\tbest_label\t" << labels[best].first
                      << "\tbest_prob\t" << probs[best] << "\n";

            for (size_t rank = 0; rank < order.size(); rank++)
            {
                const size_t i = order[rank];
                std::cout << "RESULT\t" << image_paths[image_index] << "\t" << rank + 1 << "\t" << labels[i].first << "\t" << logits[i] << "\t" << probs[i] << "\t" << labels[i].second << "\n";
            }
        }
    }
