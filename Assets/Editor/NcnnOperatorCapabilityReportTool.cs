...
public static class NcnnOperatorCapabilityReportTool
{
    ...
    public static void ExportOperatorCapabilities()
    {
        ...
        NcnnOperatorCapabilities.WriteStableJson(path, NcnnOperatorCapabilities.CreateDocument());
    ...
    public static void PreflightReferenceModels()
    {
        ...
        NcnnOperatorCapabilities.WriteStableJson(
            Path.Combine(outputDirectory, "operator-capabilities.json"),
            NcnnOperatorCapabilities.CreateDocument());
        ExportPreflight(
    ...
    public static void RunFromCommandLine()
    {
        ...
        NcnnOperatorCapabilities.WriteStableJson(capabilityOutput, NcnnOperatorCapabilities.CreateDocument());
    ...
    private static void ExportPreflight(string modelPath, string outputPath, NcnnPreflightTensorDescriptor[] inputs)
    {
        ...
        var report = NcnnModelPreflight.Analyze(model, new NcnnModelPreflightRequest
        {
            ...
            targetDtype = "FP32",
            strict = true,
            inputs = inputs ?? Array.Empty<NcnnPreflightTensorDescriptor>()
        });
    ...
    private static List<NcnnPreflightTensorDescriptor> ParseInputs(string[] values)
    {
        var inputs = new List<NcnnPreflightTensorDescriptor>();
        if (values == null)
            return inputs;
