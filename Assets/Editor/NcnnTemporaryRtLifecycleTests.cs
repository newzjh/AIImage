...
public sealed class NcnnTemporaryRtLifecycleTests
{
    ...
    {
        var previousEnabled = NcnnGpuResourceTracker.Enabled;
        ...
        try
        {
            ...
            var snapshot = NcnnGpuResourceTracker.GetStatsSnapshot();
            var timeline = string.Join("\n", NcnnGpuResourceTracker.GetTimelineSnapshot());
            ...
            Assert.That(NcnnGpuResourceTracker.GetUnreleasedTemporaryTextureDiagnostics(), Is.Empty);
        }
        finally
        {
            repro.Dispose();
            NcnnGpuResourceTracker.Reset("cmd-temporary-rt-branch-cleanup");
            NcnnGpuResourceTracker.Enabled = previousEnabled;
        }
    }
    ...
    public void TemporaryRtBudgetError_ReportsRequestPeakAndNode()
    {
        ...
        try
        {
            ...
            var allocation = new NcnnTemporaryRtDescriptor(shape, shape, descriptor, "test-owner", "budget-node", "budget-allocation");
            var requiredBytes = NcnnGpuResourceTracker.EstimateTemporaryTextureBytes(allocation);
            NcnnGpuResourceTracker.RegisterTemporaryTextureHandle(1234, allocation);

            var error = Assert.Throws<InvalidOperationException>(
                () => NcnnGpuResourceTracker.EnsureTemporaryTextureBudget(requiredBytes, requiredBytes, "budget-node"));
            ...
            NcnnGpuResourceTracker.ReleaseTextureHandle(1234, "budget-test-release");
        }
        finally
        {
            NcnnGpuResourceTracker.Reset("temporary-rt-budget-cleanup");
            NcnnGpuResourceTracker.Enabled = previousEnabled;
        }
    }
