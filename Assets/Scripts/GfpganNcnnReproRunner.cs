...
public sealed class GfpganNcnnReproRunner : MonoBehaviour
{
    ...
    public async UniTask<GfpganResult> ProcessAsync(Texture2D src, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var composedTex = ComposeWithSoftMask(inputTex, restoredCropTex, faceMask, rect, Mathf.Clamp01(faceMaskThreshold));
            if (composedTex == null)
    ...
    private static Texture2D ComposeWithSoftMask(Texture2D baseTex, Texture2D overlayCrop, Texture2D mask, RectInt rect, float maskThreshold01)
    {
    ...
    {
        if (src == null || dst == null || kernelOffsets == null)
            return;
