...
#pragma warning restore CS0618
    ...
    internal static class NcnnPack4LayerHelpers
    {
        public static void ExecuteShapePreservingRenderTexture(
            ...
            string opName,
            Action<RenderTexture, NcnnRepro.BufferShape, RenderTexture> execute)
        {
            if (owner == null)
                throw new ArgumentNullException(nameof(owner));
            if (layer == null)
            ...
            var strictLinear = NcnnRepro.IsStrictLinearMatTexture(src);
            if (!strictLinear && !NcnnRepro.BufferShapeEquals(srcShape, storageShape))
            ...
            try
            {
                ...
                outRt = owner.RentTempArray(input.width, input.height, outputDepth, NcnnRepro.ResolveTensorTextureFormat(srcShape.dims));
        ...
        public static void ExecuteShapePreservingCommandBuffer(
            ...
            string opName,
            Action<CommandBuffer, ComputeTexture, NcnnRepro.BufferShape, ComputeTexture> execute)
        {
            if (owner == null)
                throw new ArgumentNullException(nameof(owner));
            if (layer == null)
            ...
            try
            {
                ...
                outArr = owner.RentTempArray(cmd, input.width, input.height, outputDepth, NcnnRepro.ResolveTensorTextureFormat(srcShape.dims));
        ...
        private static int ResolvePack4SliceCount(NcnnRepro.BufferShape shape)
        {
