...
namespace NcnnCompute
{
    ...
    public sealed class NcnnShortConvLayerRepro : NcnnBaseLayerRepro
    {
        ...
        {
            owner._extraPacks[layer.name] = new NcnnRepro.ShortConvPack();
            return default;
        }
        ...
        public override void ExecuteRenderTexturePath(NcnnRepro owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var mixedShape = NcnnRepro.GetTextureShape(context.textureShapes, mixedRef, layer.bottomNames[1]);
            var cacheShape = NcnnRepro.GetTextureShape(context.textureShapes, cacheRef, layer.bottomNames[2]);
        ...
        public override void ExecuteCommandBuffer(NcnnRepro owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var mixedShape = NcnnRepro.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[1]);
        ...
        private static NcnnRepro.TensorRef RequireTexture(NcnnLayerBufferContext context, string name, string layer)
        {
        ...
        private static TextureView ToArray(NcnnRepro owner, NcnnRepro.TensorRef source, Dictionary<string, NcnnRepro.BufferShape> shapes, string name)
        {
            if (source.texture.dimension == TextureDimension.Tex2DArray)
                return new TextureView { texture = source.texture, temporary = false };
            var logical = NcnnRepro.GetTextureShape(shapes, source, name);
            var storage = NcnnRepro.GetTextureStorageShape(source, logical);
        ...
        private static NcnnRepro.BufferShape StorageShape(NcnnRepro.BufferShape logical, RenderTexture texture)
        {
            return logical.dims <= 2
                ? new NcnnRepro.BufferShape(3, texture.width, texture.height, 1, Mathf.Max(1, texture.volumeDepth) * 4)
                : new NcnnRepro.BufferShape(logical.dims, texture.width, texture.height, logical.d, Mathf.Max(1, texture.volumeDepth) * 4);
        }
    }
}
