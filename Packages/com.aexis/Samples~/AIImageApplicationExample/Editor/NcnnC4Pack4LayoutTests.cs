...
public sealed class NcnnC4Pack4LayoutTests
{
    ...
    public void CapabilityMetadata_RecordsC4ProfilesAndRejectsRotaryProductionPlaceholder()
    {
        ...
        foreach (var operatorName in new[] { "Reshape", "Flatten", "Squeeze", "ExpandDims", "Permute", "Slice", "Tile", "Packing", "Cast" })
        {
            ...
            Assert.That(capability.status, Is.EqualTo(NcnnOperatorCapabilityStatus.Partial), operatorName);
            Assert.That(capability.profiles.Single(profile => profile.backend == NcnnOperatorCapabilityBackend.CommandBuffer).shapeProfile, Does.Contain("descriptor alias"), operatorName);
        ...
        Assert.That(rotary.status, Is.Not.EqualTo(NcnnOperatorCapabilityStatus.Supported));
    ...
    {
        var input = new NcnnTexturePlanTensorDescriptor
        {
            blob = "in",
        ...
        var identityPlan = NcnnTextureExecutionPlanner.Compile(
    ...
    public void C4LayersPublishDescriptorsAndDoNotPublishCmdPlaceholders()
    {
        ...
        foreach (var file in new[]
        {
            ...
            "NcnnPermuteLayerRepro.cs", "NcnnSliceLayerRepro.cs", "NcnnTileLayerRepro.cs", "NcnnPackingLayerRepro.cs", "NcnnCastLayerRepro.cs"
        })
        {
        ...
        Assert.That(File.ReadAllText(Path.Combine(layers, "NcnnFlattenLayerRepro.cs")), Does.Contain("ReshapePack4ToPack4"));
        Assert.That(File.ReadAllText(Path.Combine(layers, "NcnnPermuteLayerRepro.cs")), Does.Contain("placeholder publication is prohibited"));
        Assert.That(File.ReadAllText(Path.Combine(layers, "NcnnSliceLayerRepro.cs")), Does.Contain("placeholder publication is prohibited"));
        Assert.That(File.ReadAllText(Path.Combine(layers, "NcnnPackingLayerRepro.cs")), Does.Contain("RequiresPackingTransform"));
    }
