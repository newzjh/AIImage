...
public sealed class ClipNcnnReproRunner : MonoBehaviour
{
    ...
    private AexisGraphSession _imageRepro;
    private AexisGraphSession _textRepro;
    private AexisGraphSession _projectionRepro;
    ...
    private void ApplyOptions(AexisGraphSession repro)
    {
        if (repro == null)
            return;
    ...
    private async UniTask EnsureLoaded(CancellationToken ct)
    {
        ...
        long buildTextMs = 0;
        AexisGraphSession.ModelLoadProfile imageProfile = null;
        AexisGraphSession.ModelLoadProfile textProfile = null;
        AexisGraphSession.ModelLoadProfile projectionProfile = null;
    ...
    private async UniTask<float[]> EncodeTextAsync(string prompt, CancellationToken ct)
    {
        ...
        using var infer = _textRepro.InferWithMultiInputs(null, new System.Collections.Generic.Dictionary<string, AexisTensorBuffer>
        {
        ...
        var rowView = new AexisTensorBuffer(rowBuffer, 2, EmbeddingSize, 1, 1, 1, false);
        using var projInfer = _projectionRepro.InferWithMultiInputs(null, new System.Collections.Generic.Dictionary<string, AexisTensorBuffer>
        {
    ...
    private void ReportLoadProgress(string modelLabel, float progressStart, float progressEnd, AexisGraphSession.LoadProgress progress)
    {
    ...
    private async UniTask<float[]> ReadImageEmbeddingAsync(AexisGraphSession.InferResult infer, CancellationToken ct)
    {
        if (infer == null)
            return null;
    ...
    private async UniTask<float[]> EncodeImageWithCommandBufferAsync(Texture resizedInput, int targetSize, CancellationToken ct)
    {
        ...
        var outputLogicalShape = default(AexisGraphSession.BufferShape);
        ...
        try
        {
            ...
            outputCmd = _imageRepro.ForwardPack4(
                cmd,
                inputCmd,
                new AexisGraphSession.BufferShape(3, targetSize, targetSize, 1, 3),
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            using (var infer = _imageRepro.InferWithMultiInputs(
                ...
                {
                    [InputBlobName] = new AexisGraphSession.BufferShape(3, targetSize, targetSize, 1, 3)
                },
                probeBlob))
            {
                _imageRepro.Ops.DebugSyncGpu();
    ...
    private static float[] ReadTextureLogicalValues(RenderTexture texture, AexisGraphSession.BufferShape logicalShape, CancellationToken ct)
    {
        if (texture == null)
            return null;
    ...
    private bool ShouldUseTextureReadbackForImageOutput(AexisGraphSession.InferResult infer)
    {
        if (infer == null)
            return false;
