...
public sealed class MONAINcnnReproRunner : MonoBehaviour
{
    ...
    private AexisGraphSession.LayerRuntimeProfile _lastLayerRuntimeProfile;
    private string _lastLayerRuntimeProfileText;
    private AexisGraphSession.TempResourceStatsSnapshot _lastInferenceTempResourceStats;
    ...
    private sealed class OutputHeadInfo
    {
        ...
        public AexisGraphSession.ConvPack conv;
    ...
    {
        public readonly AexisGraphSession.InferResult infer;
        public readonly RenderTexture outputTexture;
        public readonly InferOutputShape? outputShape;
        private readonly AexisGraphSession _repro;
        ...
        public PatchInferHandle(AexisGraphSession.InferResult infer, AexisGraphSession repro, RenderTexture ownedInputTexture)
        {
            this.infer = infer;
            outputTexture = null;
            outputShape = null;
        ...
        public PatchInferHandle(CommandBuffer ownedCommandBuffer, AexisGraphSession repro, RenderTexture ownedInputTexture, RenderTexture ownedOutputTexture, InferOutputShape ownedOutputShape)
        {
            infer = null;
    ...
    private void CaptureLatestLayerRuntimeProfile(string pathMode)
    {
        ...
        _lastLayerRuntimeProfileText = _lastLayerRuntimeProfile != null
            ? AexisGraphSession.FormatLayerRuntimeProfile(_lastLayerRuntimeProfile, 256)
            : null;
    }

    private InferenceRunResult RunSinglePassInference(ResolvedRequest resolved, PreparedInput prepared)
    {
        using var inputTensor = new AexisTensorBuffer(
    ...
    private async UniTask<InferenceRunResult> RunSlidingWindowInferenceAsync(ResolvedRequest resolved, PreparedInput prepared, CancellationToken ct)
    {
        ...
        using var inputTensor = new AexisTensorBuffer(roiW, roiH, roiD, channelCount);
    ...
    private bool TryResolveLinearOutputHead(string outputBlobName, out OutputHeadInfo head)
    {
        ...
        for (var i = model.layers.Count - 1; i >= 0; i--)
        {
            ...
            if (layer?.topNames == null || Array.IndexOf(layer.topNames, outputBlobName) < 0)
                continue;
            if (layer.type != AexisLayerTypes.Convolution3D)
                return false;
        ...
        CancellationToken ct)
    {
        ...
        using var inputTensor = new AexisTensorBuffer(roiW, roiH, roiD, inputChannels);
        ...
        string stopAfterTopName)
    {
        ...
        if (!useTextureInputForMonaiPatches)
        {
            return new PatchInferHandle(
                ...
                null,
                new Dictionary<string, AexisTensorBuffer>(StringComparer.Ordinal) { { resolved.inputBlobName, inputTensor } },
                pinnedBlobNames,
                null,
                stopAfterTopName),
                _repro,
        ...
        var inputPack4 = _repro.RentTempArray(width, height, sliceCount, AexisGraphSession.ResolveTensorTextureFormat(4));
        try
        {
            ...
            if (useCommandBufferForMonaiPatches)
            {
                ...
                var cmdInput = _repro.RentTempArray(cmd, width, height, sliceCount, AexisGraphSession.ResolveTensorTextureFormat(4));
                ...
                var cmdOutput = _repro.ForwardPack4(
                    cmd,
                    cmdInput,
                    new AexisGraphSession.BufferShape(4, width, height, depth, resolved.inputChannels),
            ...
            {
                { resolved.inputBlobName, new AexisGraphSession.BufferShape(4, width, height, depth, resolved.inputChannels) }
            };
    ...
    private InferOutputShape GetInferOutputShape(AexisGraphSession.InferResult infer, string blobName, string missingPrefix)
    {
        if (infer == null)
    ...
    private float[] ExtractInferOutputData(AexisGraphSession.InferResult infer, string blobName, InferOutputShape outputView)
    {
        if (infer == null)
            throw new ArgumentNullException(nameof(infer));
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            for (var packOffset = 0; packOffset < outputPackCount; packOffset += chunkPackCount)
            {
                ...
                logitsRt = _repro.RentTempArray(width, height, checked(depth * activePackCount), AexisGraphSession.ResolveTensorTextureFormat(4));
        ...
        CancellationToken ct)
    {
        ...
        {
            var unreleased = AexisGpuResourceTracker.GetUnreleasedTemporaryTextureDiagnostics();
    ...
    private string BuildSlidingWindowResourceSnapshot(string stage, int patchIndex, int patchCount)
    {
        ...
        if (_repro != null)
            text += " | gpu=" + AexisGpuResourceTracker.BuildSummary();
        return text;
    }
        ...
        CancellationToken ct)
    {
        ...
        using var featureTensor = new AexisTensorBuffer(width, height, baseChunkDepth, featureChannels);
        using var logitsTensor = new AexisTensorBuffer(width, height, baseChunkDepth, outputChannels);
        ...
        for (var relativeStartDepth = 0; relativeStartDepth < depthCount; relativeStartDepth += baseChunkDepth)
        {
            ...
            else
            {
                ...
                using var featureTensorTail = new AexisTensorBuffer(width, height, chunkDepth, featureChannels);
                using var logitsTensorTail = new AexisTensorBuffer(width, height, chunkDepth, outputChannels);
                for (var c = 0; c < featureChannels; c++)
                {
                    var srcOffset = checked(((c * sourceDepthCapacity) + localStartDepth + relativeStartDepth) * plane);
                    var dstOffset = c * chunkVoxelCount;
    ...
    public string RunPack4RoundtripSelfTest(string outputDir, RenderTextureFormat textureFormat)
    {
        ...
        AexisGpuResourceTracker.Reset("MONAI.pack4_roundtrip_selftest");
        ...
        try
        {
            ...
            using (var tensor = new AexisTensorBuffer(width, height, depth, channels))
            {
            ...
            using (var chwTensor = new AexisTensorBuffer(width, height, chwChannels))
            {
            ...
            sb.AppendLine("gpu_summary=" + AexisGpuResourceTracker.BuildSummary());
            summary = sb.ToString();
            ...
            AexisGpuResourceTracker.WriteReport(outputDir);
            return summary;
        }
        finally
        {
            ...
            if (rt != null)
            {
                AexisGpuResourceTracker.ReleaseTexture(rt, "RunPack4RoundtripSelfTest.manualRt");
                rt.Release();
            ...
            if (chwRt != null)
            {
                AexisGpuResourceTracker.ReleaseTexture(chwRt, "RunPack4RoundtripSelfTest.manualChwRt");
                chwRt.Release();
            ...
            if (rgbPack4 != null)
            {
                AexisGpuResourceTracker.ReleaseTexture(rgbPack4, "RunPack4RoundtripSelfTest.manualRgbRt");
                rgbPack4.Release();
    ...
    private static RenderTexture CreateArrayRenderTextureForSelfTest(int width, int height, int depth, RenderTextureFormat format, string label)
    {
        ...
        rt.Create();
        AexisGpuResourceTracker.RegisterTexture(rt, "MONAI." + (label ?? "pack4_selftest_array"));
        return rt;
    }
    ...
    private void DumpPinnedBlobOutputs(AexisGraphSession.InferResult infer, string[] pinnedBlobNames)
    {
    ...
    private void RunPatchTimingSplitDiagnostics(ResolvedRequest resolved, AexisTensorBuffer inputTensor, int depth, int height, int width)
    {
