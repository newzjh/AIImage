...
public sealed class SDNcnnReproRunner : MonoBehaviour
{
    ...
    private AexisGraphSession _clipRepro;
    private AexisGraphSession _unetRepro;
    private AexisGraphSession _decoderRepro;
    private AexisGraphSession _encoderRepro;
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            var condView = new AexisTensorBuffer(condBuf, 2, TextEmbeddingWidth, cond.Length / TextEmbeddingWidth, 1, 1, false);
            var uncondView = new AexisTensorBuffer(uncondBuf, 2, TextEmbeddingWidth, uncond.Length / TextEmbeddingWidth, 1, 1, false);
    ...
    private async UniTask<float[]> BuildConditioningAsync(string prompt, CancellationToken ct)
    {
        ...
        var dummyCondView = new AexisTensorBuffer(dummyCondBuffer, 2, TextEmbeddingWidth, 1, 1, 1, false);
        ...
        for (var i = 0; i < chunks.Count; i++)
        {
            ...
            var tokenView = new AexisTensorBuffer(tokenBuffer, 1, tokens77.Length, 1, 1, 1, false);
            var multiplierView = new AexisTensorBuffer(multiplierBuffer, 1, multipliers77.Length, 1, 1, 1, false);
    ...
    private async UniTask<ComputeBuffer> RunCfgDenoiserAsync(ComputeBuffer latentBuf, int width, int height, float sigma, AexisTensorBuffer condView, AexisTensorBuffer uncondView, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var cInView = new AexisTensorBuffer(cInBuf, 1, 1, 1, 1, 1, false);
            var cOutView = new AexisTensorBuffer(cOutBuf, 1, 1, 1, 1, 1, false);
    ...
    private ComputeBuffer RunUnetOnce(
        ...
        AexisTensorBuffer cInView,
        AexisTensorBuffer cOutView,
        int width,
        int height,
        string dumpTag = null,
        ...
        IReadOnlyList<UnetCacheBlob> reuseCache = null)
    {
        ...
        Dictionary<string, AexisGraphSession.BufferShape> textureInputShapes = null;
        if (reuseCache != null)
        {
            textureInputShapes = new Dictionary<string, AexisGraphSession.BufferShape>(StringComparer.Ordinal);
    ...
    private AexisGraphSession CreateSession()
    {
    ...
    private void DumpLayerRuntimeProfile(AexisGraphSession repro, string prefix)
    {
    ...
    private static void LogLoadProfile(string label, AexisGraphSession.ModelLoadProfile profile)
    {
        if (profile == null)
            return;
        if (string.IsNullOrWhiteSpace(label))
        ...
        var items = new List<KeyValuePair<string, AexisGraphSession.LayerTypeLoadProfile>>(profile.layerTypes);
    ...
    private void TryDumpAnyBlob(AexisGraphSession.InferResult infer, string blobName, string path, bool useUnetOwner = true, AexisGraphSession ownerOverride = null)
    {
