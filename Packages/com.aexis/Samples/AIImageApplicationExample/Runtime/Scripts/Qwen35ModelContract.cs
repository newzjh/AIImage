    ...
    public sealed class Qwen35ModelContract
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (contract.Files.TryGetValue("qwen3.5_decoder.ncnn.param", out var decoderParam))
            {
                ...
                {
                    if (model.layers[i].type == AexisLayerTypes.ShortConv) contract.ShortConvCount++;
                    if (model.layers[i].type == AexisLayerTypes.GatedDeltaRule) contract.GatedDeltaRuleCount++;
                }
