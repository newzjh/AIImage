...
public sealed class SDInpaintingNcnnReproRunner : MonoBehaviour
{
    ...
    private AexisGraphSession _textRepro;
    private AexisGraphSession _unetRepro;
    private AexisGraphSession _vaeRepro;
    private AexisGraphSession _vaeEncoderRepro;
        ...
        CancellationToken ct)
    {
        ...
        AexisTensorBuffer condPromptView = null;
        AexisTensorBuffer uncondPromptView = null;
        try
        {
            ...
            LogStageTrace("prompt encode end | positive | tex=" + (condTex != null
                ? (condTex.width.ToString(CultureInfo.InvariantCulture) + "x" + condTex.height.ToString(CultureInfo.InvariantCulture) + "x" + Mathf.Max(1, condTex.volumeDepth).ToString(CultureInfo.InvariantCulture))
                : "<null>"));
            LogStageTrace("prompt encode begin | negative");
            uncondTex = await EncodePromptAsync(negativePrompt ?? string.Empty, "negative", ct);
            LogStageTrace("prompt encode end | negative | tex=" + (uncondTex != null
                ? (uncondTex.width.ToString(CultureInfo.InvariantCulture) + "x" + uncondTex.height.ToString(CultureInfo.InvariantCulture) + "x" + Mathf.Max(1, uncondTex.volumeDepth).ToString(CultureInfo.InvariantCulture))
                : "<null>"));
            ...
            if (!string.IsNullOrWhiteSpace(_lastDumpDir))
            {
                DumpTextureRefSafe(Path.Combine(_lastDumpDir, "prompt_cond_tex_ref.txt"), condTex);
                DumpTextureRefSafe(Path.Combine(_lastDumpDir, "prompt_uncond_tex_ref.txt"), uncondTex);
                DumpLatentTextureStatsSafe(Path.Combine(_lastDumpDir, "prompt_cond_tex_stats.txt"), condTex, 1);
                DumpLatentTextureStatsSafe(Path.Combine(_lastDumpDir, "prompt_uncond_tex_stats.txt"), uncondTex, 1);
            ...
            if (cleanLatentTex != null)
            {
                ReleasePersistentTensorTexture(ref cleanLatentTex, "SDInpaint.CleanLatentsTex");
                cleanLatentTex = null;
            }
            ...
            for (var i = 0; i < activeTimesteps.Length; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(_lastDumpDir))
                {
                    DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, "latent_before_unet_step_" + i.ToString(CultureInfo.InvariantCulture) + "_f32.bin"), latentsTex, LatentChannels);
                ...
                if (!string.IsNullOrWhiteSpace(_lastDumpDir))
                {
                    DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, "latent_after_unet_step_" + i.ToString(CultureInfo.InvariantCulture) + "_f32.bin"), latentsTex, LatentChannels);
                    DumpTextureRefSafe(Path.Combine(_lastDumpDir, "latent_after_unet_step_" + i.ToString(CultureInfo.InvariantCulture) + "_ref.txt"), latentsTex);
                    DumpTextureRefSafe(Path.Combine(_lastDumpDir, "epsilon_step_" + i.ToString(CultureInfo.InvariantCulture) + "_ref.txt"), epsilonTex);
                }
            ...
            {
                ReleasePersistentTensorTexture(ref maskedLatentTex, "SDInpaint.MaskedLatentsTex");
            }
            if (latentMaskTex != null)
            {
                ReleasePersistentTensorTexture(ref latentMaskTex, "SDInpaint.LatentMaskTex");
            }
            ...
            {
                ReleasePersistentTensorTexture(ref decodeLatentsTex, "SDInpaint.DecodeLatentsTex");
            }
        ...
        finally
        {
            ...
            ReleasePersistentTensorTexture(ref latentsTex, "SDInpaint.FinalLatentsTex");
            ReleasePersistentTensorTexture(ref decodeLatentsTex, "SDInpaint.DecodeLatentsTex");
            ReleasePersistentTensorTexture(ref maskedLatentTex, "SDInpaint.MaskedLatentsTex");
            ReleasePersistentTensorTexture(ref latentMaskTex, "SDInpaint.LatentMaskTex");
            ReleasePersistentTensorTexture(ref cleanLatentTex, "SDInpaint.CleanLatentsTex");
    ...
    private static void ReleaseTempArrayFallback(ref RenderTexture texture, AexisGraphSession owner, string label)
    {
        if (texture == null)
            return;
        ...
        AexisGpuResourceTracker.ReleaseTexture(texture, label ?? "SDInpaint.TempArrayFallback");
        try { RenderTexture.ReleaseTemporary(texture); } catch { }
        texture = null;
    }

    private static void ReleasePersistentTensorTexture(ref RenderTexture texture, string label)
    {
        if (texture == null)
            return;

        AexisGpuResourceTracker.ReleaseTexture(texture, label ?? "SDInpaint.PersistentTensorTexture");
        try { texture.Release(); } catch { }
        try { DestroyImmediate(texture); } catch { }
        texture = null;
    }
    ...
    private void ApplyCommonOptions(AexisGraphSession repro)
    {
        if (repro == null)
            return;
    ...
    private void ApplySpatialModelOptions(AexisGraphSession repro)
    {
        if (repro == null)
            return;
    ...
    private void AttachDebugLog(AexisGraphSession repro, string key)
    {
        if (repro == null)
            return;
    ...
    private VaeEncoderTailInfo ResolveVaeEncoderTailInfo()
    {
        ...
        for (var i = 0; i < modelLayers.Count; i++)
        {
            ...
            if (layer?.topNames == null || layer.bottomNames == null || layer.bottomNames.Length == 0)
                continue;
            if (layer.type != AexisLayerTypes.Slice)
                continue;
    ...
    private VaeDecoderEndpointInfo ResolveVaeDecoderEndpointInfo()
    {
        ...
        for (var i = 0; i < modelLayers.Count; i++)
        {
            ...
            if (layer?.topNames == null || layer.topNames.Length == 0)
                continue;
            if (layer.type != AexisLayerTypes.Input)
                continue;
    ...
    private async UniTask LoadModelAsync(AexisGraphSession repro, string paramPath, string binPath, string label, CancellationToken ct)
    {
        if (repro == null)
    ...
    private async UniTask<RenderTexture> EncodePromptAsync(string prompt, string label, CancellationToken ct)
    {
        ct.ThrowIfCancellationRequested();
        if (_tokenizer == null)
        ...
        try
        {
            ...
            if (dims != 2 || w != TextEmbeddingWidth || h != TokenCount || d != 1 || c != 1)
            {
                throw new InvalidOperationException(
                    "Text encoder output shape mismatch | dims=" + dims.ToString(CultureInfo.InvariantCulture)
            ...
            if (!string.IsNullOrWhiteSpace(_lastDumpDir))
            {
                var dumpStem = string.Equals(label, "negative", StringComparison.OrdinalIgnoreCase)
                    ? "prompt_uncond"
                    : "prompt_cond";
                DumpInferTextureContractSafe(infer, TextEncoderOutputBlobName, Path.Combine(_lastDumpDir, dumpStem + "_contract.txt"));
                try
                {
                    var outputData = infer.GetExistingTextureData(TextEncoderOutputBlobName);
                    WriteFloatArrayStatsSafe(Path.Combine(_lastDumpDir, dumpStem + "_stats.txt"), outputData);
                    WriteFloatArrayRawF32Safe(Path.Combine(_lastDumpDir, dumpStem + "_f32.bin"), outputData);
                }
                catch
                {
                }
            ...
            if (!infer.TryGetExistingTexture(TextEncoderOutputBlobName, out var outputTex) || outputTex == null)
                throw new InvalidOperationException("Text encoder output texture is missing.");

            resultTex = CloneTensorTexturePersistent(
                _textRepro,
                outputTex,
                "SDInpaint.Prompt." + (string.IsNullOrWhiteSpace(label) ? "prompt" : label));

            LogStageTrace("text texture clone end"
                + (string.IsNullOrWhiteSpace(label) ? string.Empty : " | " + label)
                + " | size=" + resultTex.width.ToString(CultureInfo.InvariantCulture)
                + "x" + resultTex.height.ToString(CultureInfo.InvariantCulture)
                + "x" + Mathf.Max(1, resultTex.volumeDepth).ToString(CultureInfo.InvariantCulture));
            await UniTask.Yield();
            var result = resultTex;
            resultTex = null;
            return result;
        }
        finally
    ...
    private async UniTask<RenderTexture> EncodeImageLatentsPack4Async(Texture source, INormalRng rng, CancellationToken ct, string debugLabel = null)
    {
        ...
        try
        {
            ...
            await UniTask.Yield();
            ct.ThrowIfCancellationRequested();
            resultTex = CloneTensorTexturePersistent(_vaeEncoderRepro, scaledLatentTex, "SDInpaint.EncodedLatentsPersistent");
            var result = resultTex;
            resultTex = null;
            return result;
        }
        finally
        {
            ...
            if (resultTex != null)
                ReleasePersistentTensorTexture(ref resultTex, "SDInpaint.EncodedLatentsPersistent");
        }
    }
    ...
    private RenderTexture CreateLatentNoisePack4(AexisGraphSession owner, INormalRng rng, bool temporary = true, string label = null)
    {
        if (owner == null)
    ...
    private RenderTexture CreateMaskPack4Texture(Texture2D mask)
    {
        ...
        return CreateTensorPack4Texture(
            ...
            tensorTextureFormat,
            temporary: false,
            label: "SDInpaint.LatentMaskPersistent");
    }
        ...
        string label = null)
    {
        ...
        var actualFormat = format == RenderTextureFormat.ARGBHalf ? owner.TensorTextureFormat : format;
        var rt = temporary
            ? owner.RentTempArray(width, height, sliceCount, actualFormat)
            : CreatePersistentTensorTexture(owner, width, height, sliceCount, actualFormat, label);
        var textureFormat = actualFormat == RenderTextureFormat.ARGBFloat ? TextureFormat.RGBAFloat : TextureFormat.RGBAHalf;
        var wh = width * height;
        ...
        catch
        {
            if (temporary)
                ...
                ReleasePersistentTensorTexture(ref rt, label);
            throw;
        }
        finally
        {
        ...
        string label)
    {
        ...
        var desc = new RenderTextureDescriptor(Mathf.Max(1, width), Mathf.Max(1, height), format, 0)
        {
            ...
            volumeDepth = Mathf.Max(1, sliceCount),
        ...
        var rt = new RenderTexture(desc)
        {
            filterMode = FilterMode.Point,
            wrapMode = TextureWrapMode.Clamp,
            name = label ?? "SDInpaint.PersistentTensorTexture"
        };
        rt.Create();
        AexisGpuResourceTracker.RegisterTexture(rt, label ?? "SDInpaint.PersistentTensorTexture");
        return rt;
    }
    ...
    private async UniTask<ComputeBuffer> RunCfgUnetAsync(
        ...
        int timestep,
        AexisTensorBuffer condView,
        AexisTensorBuffer uncondView,
        float guidanceScale,
        CancellationToken ct)
    {
        RenderTexture inputTex = null;
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            if (condView == null || uncondView == null)
                throw new InvalidOperationException("UNet pack4 path requires prompt condition views.");
        ...
        CancellationToken ct)
    {
        ...
        RenderTexture condInputTex = null;
        RenderTexture uncondInputTex = null;
        RenderTexture condTimestepTex = null;
        RenderTexture uncondTimestepTex = null;
        ...
        try
        {
            condInputTex = BuildUnetInputPack4(latentsTex, maskTex, maskedLatentTex);
            uncondInputTex = BuildUnetInputPack4(latentsTex, maskTex, maskedLatentTex);
            condTimestepTex = CreateTensorPack4Texture(_unetRepro, new[] { (float)timestep }, 1, 1, 1, 1, 1, tensorTextureFormat);
            uncondTimestepTex = CreateTensorPack4Texture(_unetRepro, new[] { (float)timestep }, 1, 1, 1, 1, 1, tensorTextureFormat);
            condPromptTex = CloneTensorTexture(_unetRepro, condTex, "SDInpaint.CmdCondPromptTex");
            uncondPromptTex = CloneTensorTexture(_unetRepro, uncondTex, "SDInpaint.CmdUncondPromptTex");
            ...
            {
                { UnetInputBlobName, condInputCmd },
                { UnetTimestepBlobName, condTimestepCmd },
                { UnetTextBlobName, condPromptCmd }
            };
            var condShapes = new Dictionary<string, AexisGraphSession.BufferShape>(StringComparer.Ordinal)
            {
                { UnetInputBlobName, new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, UnetInputChannels) },
                { UnetTimestepBlobName, new AexisGraphSession.BufferShape(1, 1, 1, 1, 1) },
                { UnetTextBlobName, new AexisGraphSession.BufferShape(2, TextEmbeddingWidth, TokenCount, 1, 1) }
            };
            ...
            var uncondShapes = new Dictionary<string, AexisGraphSession.BufferShape>(StringComparer.Ordinal)
            {
                { UnetInputBlobName, new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, UnetInputChannels) },
                { UnetTimestepBlobName, new AexisGraphSession.BufferShape(1, 1, 1, 1, 1) },
                { UnetTextBlobName, new AexisGraphSession.BufferShape(2, TextEmbeddingWidth, TokenCount, 1, 1) }
        ...
        finally
        {
            ...
            if (uncondInputTex != null)
                _unetRepro?.ReturnTempArray(uncondInputTex);
            if (condTimestepTex != null)
                _unetRepro?.ReturnTempArray(condTimestepTex);
            if (uncondTimestepTex != null)
                _unetRepro?.ReturnTempArray(uncondTimestepTex);
            if (condPromptTex != null)
                _unetRepro?.ReturnTempArray(condPromptTex);
            if (uncondPromptTex != null)
                _unetRepro?.ReturnTempArray(uncondPromptTex);
    ...
    private ComputeTexture BindExternalTexture(CommandBuffer cmd, RenderTexture texture, string label)
    {
        ...
        if (_unetRepro == null)
            throw new InvalidOperationException("UNet command-buffer texture binding requires an AexisGraphSession owner.");
    ...
    private static RenderTexture CloneTensorTexture(AexisGraphSession owner, RenderTexture source, string label)
    {
        if (owner == null)
            throw new ArgumentNullException(nameof(owner));
        if (source == null)
    ...
    private static RenderTexture CloneTensorTexturePersistent(AexisGraphSession owner, RenderTexture source, string label)
    {
        if (owner == null)
            throw new ArgumentNullException(nameof(owner));
        if (source == null)
    ...
    private static RenderTexture CreatePersistentTexture2D(int width, int height, RenderTextureFormat format, string label)
    {
        ...
        rt.Create();
        AexisGpuResourceTracker.RegisterTexture(rt, label ?? "SDInpaint.PersistentTensorTexture2D");
        return rt;
    }

    private AexisTensorBuffer CreatePromptConditionView(RenderTexture texture, string label, out ComputeBuffer buffer, out RenderTexture linearTexture)
    {
        if (texture == null)
            throw new ArgumentNullException(nameof(texture));
        ...
        {
            throw new InvalidOperationException(
                "Prompt condition texture shape mismatch | len="
                + (data != null ? data.Length.ToString(CultureInfo.InvariantCulture) : "<null>")
                + " expected=" + (TextEmbeddingWidth * TokenCount).ToString(CultureInfo.InvariantCulture));
        }
        ...
        return new AexisTensorBuffer(buffer, 2, TextEmbeddingWidth, TokenCount, 1, 1, false);
    }
    ...
    private RenderTexture CreatePersistentLinearMatTexture(ComputeBuffer buffer, int width, int height, string label)
    {
        ...
        if (_ops == null)
            throw new InvalidOperationException("AexisOps is required to upload prompt LinearMat texture.");
        ...
        try
        {
            ...
            rt.Create();
            AexisGpuResourceTracker.RegisterTexture(rt, label ?? "SDInpaint.PersistentLinearMatTexture");
        ...
        catch
        {
            if (rt != null)
            {
                AexisGpuResourceTracker.ReleaseTexture(rt, label ?? "SDInpaint.PersistentLinearMatTexture");
    ...
    private ComputeBuffer RunUnetOnce(
        ...
        AexisTensorBuffer timestepView,
        AexisTensorBuffer textView,
        string dumpTag,
        ...
        IReadOnlyList<UnetCacheBlob> reuseCache)
    {
        ...
        var textureInputShapes = new Dictionary<string, AexisGraphSession.BufferShape>(StringComparer.Ordinal)
        {
            { UnetInputBlobName, new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, UnetInputChannels) }
        };
        ...
        IReadOnlyList<UnetCacheBlob> reuseCache)
    {
        ...
        HashSet<string> pinned = null;
        var shouldDumpThisPass = enableDebugDump
            && !_unetDebugDumped
        ...
        var safeDumpTag = shouldDumpThisPass
            ? SanitizeFileName(string.IsNullOrWhiteSpace(dumpTag) ? "pass" : dumpTag)
            : null;
        var debugUnetBlobNames = shouldDumpThisPass ? ResolveDebugUnetBlobNames() : Array.Empty<string>();
        var textureInputs = new Dictionary<string, RenderTexture>(StringComparer.Ordinal)
        {
            { UnetInputBlobName, inputTex },
        ...
        if (captureCache != null)
        {
            ...
            for (var i = 0; i < OfficialUnetCacheBlobNames.Length; i++)
                pinned.Add(OfficialUnetCacheBlobNames[i]);
            pinned.Add(UnetOutputBlobName);
        }

        if (shouldDumpThisPass)
        {
            pinned ??= new HashSet<string>(StringComparer.Ordinal);
            DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_text_input_tex_f32.bin"), textTex, 1);
            DumpTextureRefSafe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_text_input_tex_ref.txt"), textTex);
        ...
        if (shouldDumpThisPass)
        {
            DumpInferTextureContractSafe(infer, UnetOutputBlobName, Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_out0_contract.txt"));
            DumpInferExistingTextureDataRawSafe(infer, UnetOutputBlobName, Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_out0_existing_f32.bin"));
            for (var i = 0; i < debugUnetBlobNames.Length; i++)
            {
                var blobName = debugUnetBlobNames[i];
                TryDumpAnyBlob(infer, blobName, Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_blob_" + blobName + ".txt"));
                DumpInferTextureContractSafe(infer, blobName, Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_blob_" + blobName + "_contract.txt"));
                DumpInferExistingTextureDataRawSafe(infer, blobName, Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_blob_" + blobName + "_existing_f32.bin"));
                if (string.Equals(dumpTag, "cond", StringComparison.Ordinal))
                {
                    TryDumpAnyBlob(infer, blobName, Path.Combine(_lastDumpDir, "unity_unet_blob_" + blobName + ".txt"));
                    DumpInferTextureContractSafe(infer, blobName, Path.Combine(_lastDumpDir, "unity_unet_blob_" + blobName + "_contract.txt"));
                    DumpInferExistingTextureDataRawSafe(infer, blobName, Path.Combine(_lastDumpDir, "unity_unet_blob_" + blobName + "_existing_f32.bin"));
                }
            }
        }

        var extractedOutTex = infer.ExtractTexture(UnetOutputBlobName);
        if (extractedOutTex == null)
            return null;
        ...
        try
        {
            if (shouldDumpThisPass)
            {
                DumpTextureRefSafe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_text_input_tex_after_ref.txt"), textTex);
                DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_text_input_tex_after_f32.bin"), textTex, 1);
                DumpTextureRefSafe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_out0_extracted_ref.txt"), extractedOutTex);
                DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_out0_extracted_f32.bin"), extractedOutTex, LatentChannels);
            }

            var resultTex = extractedOutTex;
            extractedOutTex = null;
            if (shouldDumpThisPass && resultTex != null)
            {
                DumpTextureRefSafe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_out0_returned_ref.txt"), resultTex);
                DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_out0_returned_f32.bin"), resultTex, LatentChannels);
            }

            resultTex.name = "SDInpaint.UnetOutput." + (string.IsNullOrWhiteSpace(dumpTag) ? "pass" : dumpTag);
            return resultTex;
        }
        finally
        {
            if (extractedOutTex != null)
                _unetRepro?.ReturnTempArray(extractedOutTex);
        }
    }

    private RenderTexture RunUnetOnceTextureMixedInputs(
        RenderTexture inputTex,
        AexisTensorBuffer timestepView,
        RenderTexture textTex,
        AexisTensorBuffer textView,
        ...
        IReadOnlyList<UnetCacheBlob> reuseCache)
    {
        if (inputTex == null)
            throw new ArgumentNullException(nameof(inputTex));
        if (timestepView == null)
            throw new ArgumentNullException(nameof(timestepView));
        ...
        if (textTex != null)
        {
            ...
            textureInputShapes[UnetTextBlobName] = new AexisGraphSession.BufferShape(2, TextEmbeddingWidth, TokenCount, 1, 1);
        }
        if (captureCache != null)
        {
            pinned = new HashSet<string>(StringComparer.Ordinal);
        ...
        {
            pinned ??= new HashSet<string>(StringComparer.Ordinal);
            if (textView != null && textView.buffer != null)
                DumpBufferToFile(Path.Combine(_lastDumpDir, "unity_unet_" + SanitizeFileName(string.IsNullOrWhiteSpace(dumpTag) ? "pass" : dumpTag) + "_text_input_buf.txt"), textView.buffer, textView.w * textView.h * textView.d * textView.c);
            if (textTex != null)
            {
                var safeDumpTag = SanitizeFileName(string.IsNullOrWhiteSpace(dumpTag) ? "pass" : dumpTag);
                DumpTextureRefSafe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_text_input_tex_ref.txt"), textTex);
                DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, "unity_unet_" + safeDumpTag + "_text_input_tex_f32.bin"), textTex, 1);
            }
            for (var i = 0; i < debugUnetBlobNames.Length; i++)
        ...
        try
        {
            extractedOutTex.name = "SDInpaint.UnetOutput." + (string.IsNullOrWhiteSpace(dumpTag) ? "pass" : dumpTag);
            var resultTex = extractedOutTex;
            extractedOutTex = null;
            return resultTex;
        }
    ...
    private bool TryCaptureOfficialUnetCache(AexisGraphSession.InferResult infer, List<UnetCacheBlob> dst)
    {
        ...
        try
        {
            for (var i = 0; i < OfficialUnetCacheBlobNames.Length; i++)
            {
                ...
                captured.Add(new UnetCacheBlob
                {
                    name = name,
                    texture = tex,
                    shape = new AexisGraphSession.BufferShape(dims, w, h, d, c)
                });
            }
    ...
    private Dictionary<string, AexisGraphSession.BufferShape> BuildReplayTextureInputShapes(
        Dictionary<string, RenderTexture> textureInputs,
        Dictionary<string, ReplayBlobInput> inputs)
    {
        var result = new Dictionary<string, AexisGraphSession.BufferShape>(StringComparer.Ordinal);
        if (inputs != null)
        {
            foreach (var kv in inputs)
            {
        ...
        if (textureInputs != null)
        {
            foreach (var kv in textureInputs)
            {
                if (result.ContainsKey(kv.Key))
                    continue;
                result[kv.Key] = new AexisGraphSession.BufferShape(3, kv.Value.width, kv.Value.height, 1, Mathf.Max(1, kv.Value.volumeDepth) * 4);
            }
        }
    ...
    private string ResolveReplayAliasSourceBlob(string blobName)
    {
        ...
        while (!string.IsNullOrWhiteSpace(current) && visited.Add(current))
        {
            ...
            if (producer.type == AexisLayerTypes.Input)
                return current;

            if (producer.bottomNames == null || producer.bottomNames.Length == 0 || string.IsNullOrWhiteSpace(producer.bottomNames[0]))
                return null;

            if (producer.type == AexisLayerTypes.Split
                || producer.type == AexisLayerTypes.AtenTo
                || producer.type == AexisLayerTypes.Noop)
            {
    ...
    private AexisGraphSession.BufferShape ResolveReplayBlobShape(string blobName, int valueCount)
    {
        ...
        if (string.Equals(blobName, UnetInputBlobName, StringComparison.Ordinal))
            return new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, UnetInputChannels);
        if (string.Equals(blobName, UnetTextBlobName, StringComparison.Ordinal))
            return new AexisGraphSession.BufferShape(2, TextEmbeddingWidth, TokenCount, 1, 1);
        ...
        if (valueCount == LatentSize * LatentSize * LatentChannels)
            return new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, LatentChannels);
        if (valueCount == LatentSize * LatentSize)
            return new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, 1);
        if ((valueCount % (LatentSize * LatentSize)) == 0)
            return new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, valueCount / (LatentSize * LatentSize));
    ...
    private static bool TryResolveKnownSdReplayBlobShape(string blobName, int valueCount, out AexisGraphSession.BufferShape shape)
    {
        shape = default;
        if (string.IsNullOrWhiteSpace(blobName) || valueCount <= 0)
            return false;

        static bool TryMatch(string candidate, string blobName, int valueCount, AexisGraphSession.BufferShape candidateShape, out AexisGraphSession.BufferShape resolved)
        {
            resolved = default;
        ...
        if (TryMatch("197", blobName, valueCount, new AexisGraphSession.BufferShape(3, 320, 64, 1, 64), out shape))
            return true;

        var shape2d320x4096 = new AexisGraphSession.BufferShape(2, 320, 4096, 1, 1);
        ...
        var shape3d40x4096x8 = new AexisGraphSession.BufferShape(3, 40, 4096, 1, 8);
        ...
        var shape3d40x8x4096 = new AexisGraphSession.BufferShape(3, 40, 8, 1, 4096);
    ...
    private bool TryResolveReplayBlobShapeFromModelGraph(string blobName, out AexisGraphSession.BufferShape shape)
    {
        shape = default;
        var model = _unetRepro?.Model;
        ...
        var known = new Dictionary<string, AexisGraphSession.BufferShape>(StringComparer.Ordinal)
        {
            [UnetInputBlobName] = new AexisGraphSession.BufferShape(3, LatentSize, LatentSize, 1, UnetInputChannels),
            [UnetTextBlobName] = new AexisGraphSession.BufferShape(2, TextEmbeddingWidth, TokenCount, 1, 1),
            [UnetTimestepBlobName] = new AexisGraphSession.BufferShape(1, 1, 1, 1, 1)
    ...
    private static List<AexisGraphSession.BufferShape> BuildReplayBottomShapes(
        AexisGraphModel.Layer layer,
        Dictionary<string, AexisGraphSession.BufferShape> known)
    {
        var result = new List<AexisGraphSession.BufferShape>();
    ...
    private static bool TryResolveReplayLayerOutputShapes(
        ...
        out List<AexisGraphSession.BufferShape> topShapes)
    {
        topShapes = null;
        ...
        switch (layer.type)
        {
            case var t when t == AexisLayerTypes.Input:
                return false;

            case var t when t == AexisLayerTypes.Split:
            case var t2 when t2 == AexisLayerTypes.AtenTo:
            case var t3 when t3 == AexisLayerTypes.GroupNorm:
            case var t4 when t4 == AexisLayerTypes.LayerNorm:
            case var t5 when t5 == AexisLayerTypes.Swish:
            case var t6 when t6 == AexisLayerTypes.GELU:
            case var t7 when t7 == AexisLayerTypes.UnaryOp:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
                topShapes = RepeatShape(bottomShapes[0], layer.topNames.Length);
                return true;

            case var t when t == AexisLayerTypes.PnnxExpression:
                topShapes = RepeatShape(new AexisGraphSession.BufferShape(1, 1, 1, 1, 1), layer.topNames.Length);
                return true;

            case var t when t == AexisLayerTypes.Convolution:
            case var t2 when t2 == AexisLayerTypes.ConvolutionDepthWise:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
            ...
            case var t when t == AexisLayerTypes.BinaryOp:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
            ...
            case var t when t == AexisLayerTypes.InnerProduct:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
            ...
            case var t when t == AexisLayerTypes.Gemm:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
            ...
            case var t when t == AexisLayerTypes.Reshape:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
                var reshapeShape = AexisGraphSession.ResolveReshapeShape(bottomShapes[0], layer, bottomShapes);
                topShapes = RepeatShape(reshapeShape, layer.topNames.Length);
                return true;

            case var t when t == AexisLayerTypes.Permute:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
                var dims = Mathf.Clamp(bottomShapes[0].dims, 2, 4);
                var axes = AexisGraphSession.ResolvePermuteAxes(dims, layer.GetInt(0, 0), layer.name);
                var permuteShape = AexisGraphSession.ResolvePermuteShape(bottomShapes[0], dims, axes);
                topShapes = RepeatShape(permuteShape, layer.topNames.Length);
                return true;

            case var t when t == AexisLayerTypes.SDPA:
                if (bottomShapes == null || bottomShapes.Count < 3)
                    return false;
                var queryShape = bottomShapes[0];
                var valueShape = bottomShapes[2];
                topShapes = RepeatShape(new AexisGraphSession.BufferShape(3, valueShape.w, queryShape.h, 1, queryShape.c), layer.topNames.Length);
                return true;

            case var t when t == AexisLayerTypes.Slice:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
                return TryResolveReplaySliceOutputShapes(layer, bottomShapes[0], out topShapes);

            case var t when t == AexisLayerTypes.Concat:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
            ...
            case var t when t == AexisLayerTypes.Tile:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
            ...
            case var t when t == AexisLayerTypes.ExpandDims:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
            ...
            case var t when t == AexisLayerTypes.Interp:
                if (bottomShapes == null || bottomShapes.Count < 1)
                    return false;
    ...
    private static List<AexisGraphSession.BufferShape> RepeatShape(AexisGraphSession.BufferShape shape, int count)
    {
        var result = new List<AexisGraphSession.BufferShape>(Mathf.Max(1, count));
        ...
        out AexisGraphSession.BufferShape shape)
    {
        ...
        shape = new AexisGraphSession.BufferShape(3, outW, outH, 1, outC);
        return true;
    }

    private static AexisGraphSession.BufferShape ResolveReplayBinaryOutputShape(
        AexisGraphSession.BufferShape a,
        AexisGraphSession.BufferShape b)
    {
        ...
        out AexisGraphSession.BufferShape shape)
    {
        ...
        if (srcShape.dims <= 1)
        {
            shape = new AexisGraphSession.BufferShape(1, outFeatures, 1, 1, 1);
            return true;
        }

        var rows = Mathf.Max(1, srcShape.h);
        shape = new AexisGraphSession.BufferShape(2, outFeatures, rows, 1, 1);
        return true;
    }
        ...
        out AexisGraphSession.BufferShape shape)
    {
        ...
        if (srcShape.dims == 1)
        {
            shape = new AexisGraphSession.BufferShape(1, n, 1, 1, 1);
            return true;
        }

        shape = new AexisGraphSession.BufferShape(2, n, m, 1, 1);
        return true;
    }
    ...
    private static bool TryResolveReplaySliceOutputShapes(
        AexisGraphModel.Layer layer,
        AexisGraphSession.BufferShape srcShape,
        out List<AexisGraphSession.BufferShape> shapes)
    {
        shapes = null;
        if (layer == null || layer.topNames == null || layer.topNames.Length == 0)
            return false;
        ...
        shapes = new List<AexisGraphSession.BufferShape>(layer.topNames.Length);
        var begin = 0;
        for (var i = 0; i < layer.topNames.Length; i++)
        {
            ...
            shapes.Add(new AexisGraphSession.BufferShape(srcShape.dims, Mathf.Max(1, w), Mathf.Max(1, h), Mathf.Max(1, d), Mathf.Max(1, c)));
            begin = end;
        }
        ...
        out AexisGraphSession.BufferShape shape)
    {
        ...
        var axis = AexisGraphSession.MapNcnnAxisToTensorAxis(first.dims, layer.GetInt(0, 0));
        var w = first.w;
        var h = first.h;
        var d = first.d;
        var c = first.c;
        ...
        shape = new AexisGraphSession.BufferShape(first.dims, w, h, d, c);
        return true;
    }
        ...
        out AexisGraphSession.BufferShape shape)
    {
        ...
        if (axis < 0)
            axis += srcShape.dims;
        var tensorAxis = AexisGraphSession.MapNcnnAxisToTensorAxis(srcShape.dims, axis);
        var w = srcShape.w;
        var h = srcShape.h;
        var d = srcShape.d;
        var c = srcShape.c;
        ...
        shape = new AexisGraphSession.BufferShape(srcShape.dims, w, h, d, c);
        return true;
    }
        ...
        out AexisGraphSession.BufferShape shape)
    {
        ...
        if (dims.Count == 1)
            shape = new AexisGraphSession.BufferShape(1, dims[0], 1, 1, 1);
        else if (dims.Count == 2)
            shape = new AexisGraphSession.BufferShape(2, dims[1], dims[0], 1, 1);
        else if (dims.Count == 3)
            shape = new AexisGraphSession.BufferShape(3, dims[2], dims[1], 1, dims[0]);
        else
            shape = new AexisGraphSession.BufferShape(4, dims[3], dims[2], dims[1], dims[0]);
        ...
        out AexisGraphSession.BufferShape shape)
    {
        ...
        shape = new AexisGraphSession.BufferShape(srcShape.dims, outW, outH, srcShape.d, srcShape.c);
        return true;
    }
    ...
    private AexisGraphSession.BufferShape ResolveReplayBlobShape(string blobName, int valueCount, DebugMatDump dump)
    {
        if (dump != null && dump.hasHeader)
        {
            ...
            if (dims == 2)
                return new AexisGraphSession.BufferShape(2, w, h, 1, 1);
            if (dims == 3)
                return new AexisGraphSession.BufferShape(3, w, h, 1, logicalC);
            if (dims == 4)
                return new AexisGraphSession.BufferShape(4, w, h, d, logicalC);
            return new AexisGraphSession.BufferShape(1, w * Mathf.Max(1, dump.elempack), 1, 1, 1);
        }
    ...
    private RenderTexture BuildNoisyLatentsPack4(ComputeBuffer cleanLatentBuf, ComputeBuffer noiseBuf, int timestep)
    {
        ...
        try
        {
            ...
            resultTex = CloneTensorTexturePersistent(_unetRepro, outputTex, "SDInpaint.NoisyLatentsPersistent");
            var result = resultTex;
            resultTex = null;
            return result;
        }
        finally
        {
            ...
            if (resultTex != null)
                ReleasePersistentTensorTexture(ref resultTex, "SDInpaint.NoisyLatentsPersistent");
        }
    }
    ...
    private RenderTexture DdimStepPack4(RenderTexture sampleTex, RenderTexture epsilonTex, int timestep, int prevTimestep)
    {
        ...
        try
        {
            ...
            _ops.DebugSyncGpu();
            DumpDdimIntermediateTexture(timestep, "sample", sampleTex);
            DumpDdimIntermediateTexture(timestep, "epsilon", epsilonTex);
            DumpDdimIntermediateTexture(timestep, "scaled_epsilon", scaledEpsilonTex);
            DumpDdimIntermediateTexture(timestep, "pred_original_numerator", predOriginalNumeratorTex);
            DumpDdimIntermediateTexture(timestep, "pred_original", predOriginalTex);
            DumpDdimIntermediateTexture(timestep, "pred_direction", predDirectionTex);
            DumpDdimIntermediateTexture(timestep, "scaled_original", scaledOriginalTex);
            DumpDdimIntermediateTexture(timestep, "output", outputTex);

            resultTex = CloneTensorTexturePersistent(_unetRepro, outputTex, "SDInpaint.DDIMOutput");
            DumpDdimIntermediateTexture(timestep, "result", resultTex);
            var result = resultTex;
            resultTex = null;
            return result;
        }
        finally
        {
            ...
            if (outputTex != null)
                _unetRepro?.ReturnTempArray(outputTex);
            if (resultTex != null)
                ReleasePersistentTensorTexture(ref resultTex, "SDInpaint.DDIMOutput");
        }
    }

    private void DumpDdimIntermediateTexture(int timestep, string label, RenderTexture texture)
    {
        if (texture == null || string.IsNullOrWhiteSpace(_lastDumpDir))
            return;

        var safeLabel = SanitizeFileName(string.IsNullOrWhiteSpace(label) ? "tensor" : label);
        var prefix = "ddim_t" + timestep.ToString(CultureInfo.InvariantCulture) + "_" + safeLabel;
        DumpLatentTextureStatsSafe(Path.Combine(_lastDumpDir, prefix + "_stats.txt"), texture, LatentChannels);
        DumpTextureRawF32Safe(Path.Combine(_lastDumpDir, prefix + "_f32.bin"), texture, LatentChannels);
    }
    ...
    private void DumpLayerRuntimeProfile(AexisGraphSession repro, string label)
    {
    ...
    private static ComputeBuffer NewTrackedBuffer(int count, int stride, ComputeBufferType type, string label)
    {
        ...
        AexisGpuResourceTracker.RegisterBuffer(buffer, count, stride, label ?? "SDInpaint.StandaloneBuffer");
        return buffer;
    }
    ...
    private static RenderTexture GetTemporaryRt(int width, int height, RenderTextureFormat format, bool enableRandomWrite, string label = null)
    {
        ...
        AexisGpuResourceTracker.RegisterTexture(rt, label ?? "SDInpaint.TempRt");
        return rt;
    }
    ...
    private static void ReleaseTemporaryRt(RenderTexture rt, string label = null)
    {
        if (rt == null)
            return;
        AexisGpuResourceTracker.ReleaseTexture(rt, label ?? "SDInpaint.TempRt");
    ...
    private static void DisposeBuffer(ComputeBuffer buffer, string label = null)
    {
        if (buffer == null)
            return;
        AexisGpuResourceTracker.ReleaseBuffer(buffer, label ?? "SDInpaint.StandaloneBuffer");
    ...
    private void DumpTextureRawF32Safe(string path, RenderTexture texture, int channelCount)
    {
        ...
        {
            _ops?.DebugSyncGpu();
            var data = ReadTextureChwDataStable(texture, channelCount);
            if (data != null && data.Length > 0)
                WriteFloatArrayRawF32Safe(path, data);
        }
        catch
    ...
    private static void DumpTextureRefSafe(string path, RenderTexture texture)
    {
        ...
        {
            var text =
                "instance_id=" + texture.GetInstanceID().ToString(CultureInfo.InvariantCulture) + Environment.NewLine
                + "name=" + (texture.name ?? string.Empty) + Environment.NewLine
                + "width=" + texture.width.ToString(CultureInfo.InvariantCulture) + Environment.NewLine
                + "height=" + texture.height.ToString(CultureInfo.InvariantCulture) + Environment.NewLine
                + "volume_depth=" + Mathf.Max(1, texture.volumeDepth).ToString(CultureInfo.InvariantCulture) + Environment.NewLine
                + "dimension=" + texture.dimension + Environment.NewLine
                + "format=" + texture.format + Environment.NewLine
                + "created=" + texture.IsCreated().ToString(CultureInfo.InvariantCulture);
            WriteAllTextSafe(path, text);
        }
        catch
        {
        }
    ...
    private void DumpLatentTextureStatsSafe(string path, RenderTexture texture, int channelCount)
    {
        ...
        try
        {
            ...
            var data = ReadTextureChwDataStable(texture, channelCount);
            if (data != null && data.Length > 0)
                WriteFloatArrayStatsSafe(path, data);
        }
        catch
        {
        }
    ...
    private static float[] ReadTextureChwDataStable(RenderTexture texture, int channelCount)
    {
        if (texture == null || channelCount <= 0)
            return null;

        var plane = Mathf.Max(1, texture.width) * Mathf.Max(1, texture.height);
        var packCount = Mathf.Max(1, Mathf.CeilToInt(channelCount / 4f));
        var availableSlices = Mathf.Max(1, texture.volumeDepth > 0 ? texture.volumeDepth : 1);
        if (packCount > availableSlices)
            throw new InvalidOperationException(
                "texture slice count too small for channel dump"
                + " | channels=" + channelCount.ToString(CultureInfo.InvariantCulture)
                + " | packs=" + packCount.ToString(CultureInfo.InvariantCulture)
                + " | slices=" + availableSlices.ToString(CultureInfo.InvariantCulture)
                + " | dimension=" + texture.dimension);

        var values = new float[checked(plane * channelCount)];
        ...
        try
        {
            ...
            {
                ReadRenderTextureSlice(texture, readback, pack);
                ...
                for (var i = 0; i < plane; i++)
                {
                    ...
                    for (var lane = 0; lane < 4; lane++)
                    {
                        ...
                        values[channel * plane + i] = raw[srcBase + lane];
                    }
                }
            }
    ...
    private void TryDumpAnyBlob(AexisGraphSession.InferResult infer, string blobName, string path)
    {
        ...
        try
        {
            ...
            {
                WriteFloatArraySafe(path, textureData);
                return;
            }
        }
        catch
    ...
    private static void DumpInferTextureContractSafe(AexisGraphSession.InferResult infer, string blobName, string path)
    {
        if (infer == null || string.IsNullOrWhiteSpace(blobName) || string.IsNullOrWhiteSpace(path))
            return;
        ...
        try
        {
            ...
            sb.Append("blob=").AppendLine(blobName);
            if (infer.TryGetLogicalShape(blobName, out var dims, out var w, out var h, out var d, out var c))
            {
                sb.Append("logical_dims=").Append(dims.ToString(CultureInfo.InvariantCulture))
                    .Append(" w=").Append(w.ToString(CultureInfo.InvariantCulture))
                    .Append(" h=").Append(h.ToString(CultureInfo.InvariantCulture))
                    .Append(" d=").Append(d.ToString(CultureInfo.InvariantCulture))
                    .Append(" c=").AppendLine(c.ToString(CultureInfo.InvariantCulture));
            }
            else
            {
                sb.AppendLine("logical_shape=<missing>");
            }

            if (infer.TryGetExistingTexture(blobName, out var tex) && tex != null)
            {
                sb.Append("texture_width=").Append(tex.width.ToString(CultureInfo.InvariantCulture))
                    .Append(" height=").Append(tex.height.ToString(CultureInfo.InvariantCulture))
                    .Append(" depth=").Append(Mathf.Max(1, tex.volumeDepth).ToString(CultureInfo.InvariantCulture))
                    .Append(" instance_id=").Append(tex.GetInstanceID().ToString(CultureInfo.InvariantCulture))
                    .Append(" dimension=").Append(tex.dimension.ToString())
                    .Append(" format=").Append(tex.format.ToString())
                    .Append(" created=").AppendLine(tex.IsCreated().ToString(CultureInfo.InvariantCulture));
            }
            else
            {
    ...
    private void DumpInferExistingTextureDataRawSafe(AexisGraphSession.InferResult infer, string blobName, string path)
    {
        if (infer == null || string.IsNullOrWhiteSpace(blobName) || string.IsNullOrWhiteSpace(path))
            return;
        ...
        try
        {
            if (infer.TryGetExistingTextureData(blobName, out var data) && data != null)
            {
                WriteFloatArrayRawF32Safe(path, data);
                return;
            }
    ...
    private static bool TryReadExistingInferBufferData(AexisGraphSession.InferResult infer, string blobName, out float[] data)
    {
#if UNITY_EDITOR || AIIMAGE_INFERENCE_DEBUG_ORACLE
        data = null;
    ...
    private AexisGraphSession CreateSession(bool diffusionStatePrecisionGuard = false)
    {
        ...
        var sourceManifest = AexisModelManifestLoader.ResolveRunnerManifest("sd-inpainting", precisionMode);
    ...
    private void DumpVaeEncoderStopBlobDebugSafe(RenderTexture inputPack4, string stopAfterTopName, string blobName, string pathStem)
    {
        ...
        try
        {
            ...
            {
                [VaeEncoderInputBlobName] = new AexisGraphSession.BufferShape(3, inputPack4.width, inputPack4.height, 1, 3)
            };
    ...
    private float[] ExtractBlobDataByLogicalShape(AexisGraphSession.InferResult infer, string blobName)
    {
        if (infer == null || string.IsNullOrWhiteSpace(blobName))
            return null;
    ...
    private bool TryDumpTextureBlobByLogicalShape(AexisGraphSession.InferResult infer, string blobName, string path)
    {
        if (infer == null || string.IsNullOrWhiteSpace(blobName) || string.IsNullOrWhiteSpace(path))
            return false;
    ...
    private static void LogLoadProgress(string label, AexisGraphSession.LoadProgress progress)
    {
    ...
    private void LogLoadProfile(string label, AexisGraphSession.ModelLoadProfile profile)
    {
        if (profile == null)
            return;
