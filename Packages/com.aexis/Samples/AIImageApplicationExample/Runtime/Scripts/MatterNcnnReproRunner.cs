...
public sealed class MatterNcnnReproRunner : MonoBehaviour
{
    ...
    private UniTask<RenderTexture> ForwardMattingWithCommandBufferAsync(Texture resizedInput, int width, int height, CancellationToken ct)
    {
        ...
        try
        {
            ...
            outputCmd = _repro.ForwardPack4(
                cmd,
                inputCmd,
                new AexisGraphSession.BufferShape(3, width, height, 1, 3),
                out _,
                "input");
            if (outputCmd == null)
    ...
    private void DumpPinnedBlobStats(AexisGraphSession.InferResult infer, string dir, ICollection<string> blobNames)
    {
