...
namespace AIImage.Qwen35
{
    public sealed class Qwen35OwnedTexture : IDisposable
    {
        ...
        public AexisGraphSession.BufferShape LogicalShape { get; }

        internal Qwen35OwnedTexture(AexisGraphSession owner, RenderTexture texture, AexisGraphSession.BufferShape logicalShape)
        {
    ...
    public sealed class Qwen35DecoderState : IDisposable
    {
        ...
        private readonly Dictionary<string, AexisGraphSession.BufferShape> _shapes =
            new Dictionary<string, AexisGraphSession.BufferShape>(StringComparer.Ordinal);

        internal Qwen35DecoderState(AexisGraphSession owner, int sequenceLength, int positionId)
        {
        ...
        internal Dictionary<string, AexisGraphSession.BufferShape> Shapes => _shapes;

        internal void Add(string inputName, RenderTexture texture, AexisGraphSession.BufferShape shape)
        {
        ...
        internal void TransferTo(Qwen35DecoderState destination, string inputName, AexisGraphSession.BufferShape nextShape)
        {
    ...
    public sealed class Qwen35DecoderSession : IDisposable
    {
        ...
        private readonly AexisGraphSession _embed;
        private readonly AexisGraphSession _decoder;
        private readonly AexisGraphSession _projection;
        ...
        public AexisGraphSession.ModelLoadProfile TokenEmbeddingLoadProfile => _embed.LastLoadProfile;
        public AexisGraphSession.ModelLoadProfile DecoderLoadProfile => _decoder.LastLoadProfile;
        public AexisGraphSession.ModelLoadProfile ProjectionLoadProfile => _projection.LastLoadProfile;
        // Diagnostics only. Normal Qwen execution does not allocate or time layer profiles.
        public AexisGraphSession.LayerRuntimeProfile PrefillDecoderRuntimeProfile { get; private set; }
        public AexisGraphSession.LayerRuntimeProfile FirstDecodeRuntimeProfile { get; private set; }
        public AexisGraphSession.LayerRuntimeProfile LastDecodeRuntimeProfile { get; private set; }
        ...
        private static Qwen35Progress MapNetworkProgress(
            string network,
            AexisGraphSession.LoadProgress progress,
            float start,
            float end)
        {
            ...
            try
            {
                for (var i = 0; i < ConvCacheCount; i++)
                {
                    state.Add(
                        ...
                        new AexisGraphSession.BufferShape(2, 1536, 4, 1, 1));
                    state.Add(
                        "cache_gdr" + i,
                        CreateZeroPack4Texture(32, 128, 16),
                        new AexisGraphSession.BufferShape(3, 32, 128, 1, 16));
                }
                return state;
            }
            catch
            ...
            out int imagePadIndex)
        {
            ...
            using (var text = EmbedTokens(prefixTokenIds))
            {
                try
                {
                    ...
                    {
                        imageLinear = _decoder.RentTempMat(HiddenSize, vision.EmbeddingCount, AexisGraphSession.ResolveLinearMatTextureFormat());
                    ...
                    merged = _decoder.RentTempMat(HiddenSize, expandedRows, AexisGraphSession.ResolveLinearMatTextureFormat());
                    if (imagePadIndex > 0)
                    ...
                    var result = new Qwen35OwnedTexture(
                        _decoder,
                        merged,
                        new AexisGraphSession.BufferShape(2, HiddenSize, expandedRows, 1, 1));
                    merged = null;
                    return result;
                }
                finally
        ...
        private static void ExtractCache(
            AexisGraphSession.InferResult result,
            Qwen35DecoderState state,
            string inputName,
            string outputName)
        {
        ...
        private void DebugReadbackPackedCaches(AexisGraphSession.InferResult result, int decodeInvocation)
        {
        ...
        private static AexisGraphSession.BufferShape GetShape(AexisGraphSession.InferResult result, string name)
        {
        ...
        {
            var repro = new AexisGraphSession(_ops)
            {
