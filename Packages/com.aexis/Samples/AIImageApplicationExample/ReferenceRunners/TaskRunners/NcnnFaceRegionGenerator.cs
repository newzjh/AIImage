...
public sealed class NcnnFaceRegionGenerator : MonoBehaviour
{
    ...
    public async UniTask<FaceRegionResult> GenerateAsync(Texture2D src, bool dumpDebug, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (dumpDebug)
            {
                ...
                AexisGpuResourceTracker.Reset("NcnnFaceRegionGenerator");
            }
    ...
    private void AppendBlobPreview(AexisGraphSession.InferResult infer, List<string> lines, string blobName, int previewCount)
    {
    ...
    private static string CreateDumpDir()
    {
        ...
        var dir = Path.Combine(root, "Aexis_AexisFaceRegion_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
