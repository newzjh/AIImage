...
public sealed class CodeFormerNcnnReproRunner2 : MonoBehaviour
{
    ...
    private AexisGraphSession _generatorRepro;
    ...
    public async UniTask<CodeFormerResult> ProcessAsync(Texture2D src, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (enableDebugDump)
            {
                ...
                AexisGpuResourceTracker.Reset("CodeFormerNcnnReproRunner2");
            }
            await EnsureLoaded();
            ...
            finally
            {
                ...
                if (enableDebugDump && !string.IsNullOrWhiteSpace(_lastDumpDir))
                    AexisGpuResourceTracker.WriteReport(_lastDumpDir, "codeformer_gpu_resources.txt");
            }
        }
        catch (Exception e)
        {
    ...
    private async UniTask<CodeFormer512RunResult> RunCodeFormer512Async(RenderTexture face512, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                { "input", new AexisGraphSession.BufferShape(2, 1024, 256, 1, 1) }
            };
            ...
            else
            {
                using (var generatorResult = _generatorRepro.InferWithMultiInputs(textureInputs, null, generatorPinned, textureInputShapes))
                {
                    ...
                    AexisGpuResourceTracker.UpdateTextureLabel(outputTex, "CodeFormer.generatorOut");
                    if (outputTex == null)
                    {
    ...
    private async UniTask<Texture2D> RunGeneratorWithCommandBufferAsync(
        ...
        Dictionary<string, AexisGraphSession.BufferShape> textureInputShapes,
        ...
        try
        {
            foreach (var kv in textureInputs)
            {
                ...
                var logicalShape = textureInputShapes != null && textureInputShapes.TryGetValue(kv.Key, out var suppliedShape)
                    ? suppliedShape
                    : new AexisGraphSession.BufferShape(3, kv.Value.width, kv.Value.height, 1, Mathf.Max(1, (kv.Value.volumeDepth > 0 ? kv.Value.volumeDepth : 1) * 4));
    ...
    private async UniTask DumpInferBlobAsync(AexisGraphSession.InferResult inferResult, string dir, string fileName, string blobName, CancellationToken ct)
    {
    ...
    private async UniTask DumpInferBlobStatsAsync(AexisGraphSession.InferResult inferResult, string dir, string blobName)
    {
        if (!enableDebugDump || inferResult == null || string.IsNullOrWhiteSpace(dir))
            return;
    ...
    private async UniTask DumpBinaryTensorAsImageAsync(string dir, string fileName, AexisTensorBuffer tensor, int width, int height, CancellationToken ct)
    {
    ...
    private async UniTask DumpBufferBlobAsNormalizedImageAsync(AexisGraphSession.InferResult inferResult, string dir, string fileName, string blobName, int width, int height, CancellationToken ct)
    {
        if (!enableDebugDump || inferResult == null || string.IsNullOrWhiteSpace(dir))
            return;
    ...
    private static void AppendBinaryPatternStatsLine(string dir, string blobName, AexisTensorBuffer tensor, int width, int height)
    {
    ...
    private static string CreateDumpDir()
    {
        ...
        var dir = Path.Combine(root, "Aexis_CodeFormerRepro2_" + DateTime.Now.ToString("yyyyMMdd_HHmmss"));
    ...
    private static void TryAppendInferBufferPreview(AexisGraphSession.InferResult inferResult, string dir, string blobName, int maxCount = 16)
    {
    ...
    private static RenderTexture ComposeWithMask(Texture2D src, RenderTexture restored, Texture2D mask, RectInt rect)
    {
        ...
        var cs = Resources.Load<ComputeShader>("Aexis/Samples/ImageProcessing");
        if (cs == null)
            return null;
