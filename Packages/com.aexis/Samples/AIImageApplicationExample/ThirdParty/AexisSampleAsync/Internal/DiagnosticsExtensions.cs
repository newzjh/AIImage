...
namespace Aexis.Samples.Async.Internal
{
    internal static class DiagnosticsExtensions
    {
        ...
        static string BeautifyType(Type t, bool shortName)
        {
            ...
            if (!t.IsGenericType) return shortName ? t.Name : t.FullName.Replace("Aexis.Samples.Async.Triggers.", "").Replace("Aexis.Samples.Async.Internal.", "").Replace("Aexis.Samples.Async.", "") ?? t.Name;
            ...
            return typeBeautifyRegex.Replace(genericType, "").Replace("Aexis.Samples.Async.Triggers.", "").Replace("Aexis.Samples.Async.Internal.", "").Replace("Aexis.Samples.Async.", "") + "<" + innerFormat + ">";
        }
        ...
        static bool IgnoreLine(MethodBase methodInfo)
        {
            ...
            else if (declareType.StartsWith("Aexis.Samples.Async.CompilerServices"))
            {
                return true;
            }
            ...
            else if (declareType.StartsWith("Aexis.Samples.Async.UniTaskCompletionSourceCore"))
            {
                return true;
            }
            else if (declareType.StartsWith("Aexis.Samples.Async.AwaiterActions"))
            {
                return true;
            }
