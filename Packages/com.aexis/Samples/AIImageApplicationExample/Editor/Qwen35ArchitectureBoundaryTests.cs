...
public sealed class Qwen35ArchitectureBoundaryTests
{
    ...
    public void ReusablePackage_KeepsOnlyGenericRecurrentOperatorLayers()
    {
        var runtimeRoot = Path.Combine(
            ...
            "Runtime",
            "Ncnn");
        Assert.That(File.Exists(Path.Combine(runtimeRoot, "Layers", "AexisShortConvLayer.cs")), Is.True);
        Assert.That(File.Exists(Path.Combine(runtimeRoot, "Layers", "AexisGatedDeltaRuleLayer.cs")), Is.True);
    }
