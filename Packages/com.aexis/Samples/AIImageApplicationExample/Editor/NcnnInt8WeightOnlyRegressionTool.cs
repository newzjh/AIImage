...
public static class NcnnInt8WeightOnlyRegressionTool
{
    ...
    private static NcnnInt8WeightOnlyRegressionCase RunYoloConv(string modelDirectory)
    {
        ...
        using var reader = new NcnnBinReader(stream);
        var weights = AexisGraphSession.ReadPackedOrRawWeightArray(reader, weightCount, layer.name);
        ...
        float activationScale)
    {
        ...
        AexisGraphSession.Int8WeightOnlyUpload int8 = null;
        ComputeBuffer fp16 = null;
        try
        {
            var inputPacks = (inputChannels + 3) / 4;
            var outputPacks = (outputChannels + 3) / 4;
            var packedWeights = AexisGraphSession.PackWeightsToO4I4K(weights, outputChannels, inputChannels, kernel, outputPacks, inputPacks);
            var packedBias = AexisGraphSession.PackBiasToO4(bias, outputChannels, outputPacks);
            using var ops = new AexisOps();
            ...
            fp16 = AexisGraphSession.NewFp16Vector4Buffer(packedWeights, "NcnnInt8WeightOnlyRegressionTool.MattingFp16");
            int8 = AexisGraphSession.NewInt8WeightOnlyUpload(
                weights,
                outputChannels,
            ...
            return CreateAvailableGpuTiming(
                MeasureMattingGpuMode(repro, ops, input, inputWidth, inputHeight, inputChannels, outputWidth, outputHeight, outputChannels, kernel, stride, pad, packedWeightBuffer, packedBiasBuffer, rawBiasBuffer, fp16, int8, AexisPrecisionMode.FP32),
                MeasureMattingGpuMode(repro, ops, input, inputWidth, inputHeight, inputChannels, outputWidth, outputHeight, outputChannels, kernel, stride, pad, packedWeightBuffer, packedBiasBuffer, rawBiasBuffer, fp16, int8, AexisPrecisionMode.FP16),
                MeasureMattingGpuMode(repro, ops, input, inputWidth, inputHeight, inputChannels, outputWidth, outputHeight, outputChannels, kernel, stride, pad, packedWeightBuffer, packedBiasBuffer, rawBiasBuffer, fp16, int8, AexisPrecisionMode.INT8Selective, activationScale));
        }
        ...
        finally
        {
            ...
            {
                AexisGpuResourceTracker.ReleaseBuffer(int8.packedWeights, "NcnnInt8WeightOnlyRegressionTool.MattingInt8");
                int8.packedWeights.Dispose();
            }
            if (int8 != null && int8.scales != null)
            {
                AexisGpuResourceTracker.ReleaseBuffer(int8.scales, "NcnnInt8WeightOnlyRegressionTool.MattingInt8");
                int8.scales.Dispose();
            }
            if (fp16 != null)
            {
                AexisGpuResourceTracker.ReleaseBuffer(fp16, "NcnnInt8WeightOnlyRegressionTool.MattingFp16");
                fp16.Dispose();
            }
        }
    }
    ...
    private static NcnnInt8GpuModeMeasurement MeasureMattingGpuMode(
        ...
        AexisGraphSession.Int8WeightOnlyUpload int8Weights,
        AexisPrecisionMode precision,
        float activationScale = 0f)
    {
        var previousTracker = AexisGpuResourceTracker.Enabled;
        AexisGpuResourceTracker.Enabled = true;
        AexisGpuResourceTracker.Reset("NcnnInt8SelectiveRegression.Conv." + precision);
        try
        {
            var format = precision == AexisPrecisionMode.FP32 ? RenderTextureFormat.ARGBFloat : RenderTextureFormat.ARGBHalf;
            var inputPacks = (inputChannels + 3) / 4;
            var outputPacks = (outputChannels + 3) / 4;
            ...
            try
            {
                var useInt8 = precision == AexisPrecisionMode.INT8Selective;
                ops.SetFp16ConvWeights(precision == AexisPrecisionMode.FP16 ? fp16Weights : null);
                ops.SetInt8ConvWeights(useInt8 ? int8Weights.packedWeights : null, useInt8 ? int8Weights.scales : null);
                ops.SetInt8ActivationQuantization(precision == AexisPrecisionMode.INT8Selective && activationScale > 0f
        ...
        {
            AexisGpuResourceTracker.Enabled = previousTracker;
        }
    }
        ...
        float activationScale)
    {
        ...
        try
        {
            ...
            fp16 = AexisGraphSession.NewFp16Buffer(weights, "NcnnInt8WeightOnlyRegressionTool.ClipFp16");
            int8 = AexisGraphSession.NewInt8WeightOnlyUpload(
                weights,
                outputFeatures,
                inputFeatures,
            ...
            var fp32Ms = MeasureFrozenClipGpuMode(
                ...
                fp32, biasBuffer, fp16, int8, unusedWeightBinding, AexisPrecisionMode.FP32);
            var fp16Ms = MeasureFrozenClipGpuMode(
                repro, ops, input, outputFeatures, inputFeatures, RenderTextureFormat.ARGBHalf,
                fp32, biasBuffer, fp16, int8, unusedWeightBinding, AexisPrecisionMode.FP16);
            var int8Ms = MeasureFrozenClipGpuMode(
                repro, ops, input, outputFeatures, inputFeatures, RenderTextureFormat.ARGBHalf,
                fp32, biasBuffer, fp16, int8, unusedWeightBinding, AexisPrecisionMode.INT8Selective, activationScale);
        ...
        finally
        {
            if (int8 != null && int8.packedWeights != null)
            {
                AexisGpuResourceTracker.ReleaseBuffer(int8.packedWeights, "NcnnInt8WeightOnlyRegressionTool.ClipInt8");
                int8.packedWeights.Dispose();
            }
            if (int8 != null && int8.scales != null)
            {
                AexisGpuResourceTracker.ReleaseBuffer(int8.scales, "NcnnInt8WeightOnlyRegressionTool.ClipInt8");
                int8.scales.Dispose();
            }
            if (fp16 != null)
            {
                AexisGpuResourceTracker.ReleaseBuffer(fp16, "NcnnInt8WeightOnlyRegressionTool.ClipFp16");
                fp16.Dispose();
            }
        }
    }
        ...
        float activationScale = 0f)
    {
        var previousTracker = AexisGpuResourceTracker.Enabled;
        AexisGpuResourceTracker.Enabled = true;
        AexisGpuResourceTracker.Reset("NcnnInt8SelectiveRegression.Gemm." + precision);
        try
        {
            ...
            {
                var useInt8 = precision == AexisPrecisionMode.INT8Selective;
                ops.SetFp16GemmWeights(precision == AexisPrecisionMode.FP16 ? fp16Weights : null);
                ops.SetInt8GemmWeights(useInt8 ? int8Weights.packedWeights : null, useInt8 ? int8Weights.scales : null);
                ops.SetInt8ActivationQuantization(precision == AexisPrecisionMode.INT8Selective
                    ? new QuantizedNodePlan
                    {
                        mode = QuantizedNodeMode.Int8W8A8,
                        activationScale = activationScale,
    ...
    private static void RegisterConvModeBuffers(
        ...
        AexisGraphSession.Int8WeightOnlyUpload int8Weights)
    {
        if (precision == AexisPrecisionMode.FP32)
        {
        ...
        else if (precision == AexisPrecisionMode.FP16)
        {
        ...
        else if (precision == AexisPrecisionMode.INT8Selective)
        {
    ...
    {
        if (buffer != null)
            AexisGpuResourceTracker.RegisterBuffer(buffer, buffer.count, buffer.stride, label);
    }
    ...
    private static RenderTexture[] CreatePersistentOutputs(int count, int width, int height, RenderTextureFormat format, string label)
    {
        ...
        for (var index = 0; index < outputs.Length; index++)
        {
            ...
            AexisGpuResourceTracker.RegisterTexture(outputs[index], label + "." + index);
        }
        return outputs;
    }
    ...
    private static void ReleasePersistentOutputs(RenderTexture[] outputs, string label)
    {
        ...
        foreach (var output in outputs ?? Array.Empty<RenderTexture>())
        {
            ...
            AexisGpuResourceTracker.ReleaseTexture(output, label + "." + index);
            if (output.IsCreated())
                output.Release();
    ...
    {
        return AexisNumericUtils.ToHalfRoundedFloat(value);
    }
}
#endif
