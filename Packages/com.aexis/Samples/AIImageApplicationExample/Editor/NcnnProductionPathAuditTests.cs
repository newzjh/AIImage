...
public sealed class NcnnProductionPathAuditTests
{
    ...
    public void InferResult_DoesNotExposeProductionBufferReadback()
    {
        ...
        Assert.That(source, Does.Contain("public AexisInferenceExecutionMode ExecutionMode { get; set; } = AexisInferenceExecutionMode.ProductionTextureOnly;"));
    ...
    public void ProductionBoundary_AllowsFixedBufferInputsAndRejectsIntermediateMaterializationWithTensorContract()
    {
        ...
        Assert.That(factory, Does.Contain("if (!IsDebugOracleExecution)"));
        Assert.That(factory, Does.Contain("UploadFixedInputTexture(kv.Key, kv.Value.buffer, kv.Value)"));
        Assert.That(factory, Does.Contain("fixedBufferInputBlobs.Add(kv.Key)"));
        ...
        Assert.That(factory, Does.Contain("DebugOracle is the only mode that retains legacy Buffer inputs."));
        Assert.That(factory, Does.Contain("SetCurrentBufferExecutionContext(context)"));
        Assert.That(repro, Does.Contain("FillLinearMatFromIntBuffer"));
        Assert.That(repro, Does.Contain("ForwardPack4WithFixedInputs"));
        Assert.That(repro, Does.Contain("UploadFixedInputCmdTexture"));
        Assert.That(repro, Does.Contain("EnsureProductionDebugOverridesRejected"));
        Assert.That(repro, Does.Contain("fixedInputUpload = fixedBufferUpload"));
        ...
        Assert.That(reshape, Does.Contain("owner.ExecutionMode == AexisInferenceExecutionMode.ProductionTextureOnly"));
    ...
    public void TensorDescriptor_CompatibleViewsKeepRenderTextureOwnerAndRemainImmutable()
    {
        ...
        try
        {
            var storage = new AexisGraphSession.BufferShape(3, 4, 4, 1, 8);
            var source = AexisGraphSession.CreateTextureRef(texture, storage, storage, owned: false, blobName: "source");
            var descriptor = source.Descriptor;

            var reshape = AexisGraphSession.CreateTextureAlias(
                source,
                new AexisGraphSession.BufferShape(2, 16, 8, 1, 1),
                storage);
            var squeeze = AexisGraphSession.CreateTextureAlias(
                source,
                new AexisGraphSession.BufferShape(2, 16, 8, 1, 1),
                storage);
            var flatten = AexisGraphSession.CreateTextureAlias(
                source,
                new AexisGraphSession.BufferShape(1, 128, 1, 1, 1),
                storage);
            ...
            source.logicalShape = new AexisGraphSession.BufferShape(3, 1, 1, 1, 1);
            source.storageShape = new AexisGraphSession.BufferShape(3, 1, 1, 1, 1);
    ...
    public void TensorDescriptor_IncompatibleAliasRequiresTextureTransformWithoutBufferFallback()
    {
        ...
        try
        {
            var sourceStorage = new AexisGraphSession.BufferShape(3, 4, 4, 1, 8);
            var source = AexisGraphSession.CreateTextureRef(texture, sourceStorage, sourceStorage, owned: false, blobName: "source");
            var targetLogical = new AexisGraphSession.BufferShape(3, 8, 2, 1, 8);
            var targetStorage = new AexisGraphSession.BufferShape(3, 8, 2, 1, 8);

            var error = Assert.Throws<TensorAliasTransformRequiredException>(
                () => AexisGraphSession.CreateTextureAlias(source, targetLogical, targetStorage));
    ...
    public void TensorDescriptor_CommandTextureAliasRetainsOwnerAndDescriptor()
    {
        ...
        var storage = new AexisGraphSession.BufferShape(3, 4, 4, 1, 8);
        var source = AexisGraphSession.CreateCmdTensorRef(texture, storage, storage, owned: false, blobName: "cmd-source");
        var alias = AexisGraphSession.CreateCmdTensorAlias(
            source,
            new AexisGraphSession.BufferShape(1, 128, 1, 1, 1),
            storage);
