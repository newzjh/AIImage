...
public sealed class NcnnInt8WeightOnlyGpuTests
{
    ...
    private static void AssertConvKernel()
    {
        ...
        var upload = AexisGraphSession.NewInt8WeightOnlyUpload(weights, 2, 1, true, "NcnnInt8WeightOnlyGpuTests.Conv");
        using var ops = new AexisOps();
        using var repro = new AexisGraphSession(ops) { TensorTextureFormat = RenderTextureFormat.ARGBFloat };
    ...
    private static void AssertGemmKernel()
    {
        ...
        var bias = new[] { 0.1f, -0.2f };
        var upload = AexisGraphSession.NewInt8WeightOnlyUpload(weights, 2, 4, true, "NcnnInt8WeightOnlyGpuTests.Gemm");
        using var ops = new AexisOps();
        using var repro = new AexisGraphSession(ops) { TensorTextureFormat = RenderTextureFormat.ARGBFloat };
    ...
    private static void AssertGemmW8A8Kernel()
    {
        ...
        var upload = AexisGraphSession.NewInt8WeightOnlyUpload(weights, 2, 4, true, "NcnnInt8WeightOnlyGpuTests.GemmW8A8");
        using var ops = new AexisOps();
        using var repro = new AexisGraphSession(ops) { TensorTextureFormat = RenderTextureFormat.ARGBFloat };
    ...
    private static void ReleaseUpload(AexisGraphSession.Int8WeightOnlyUpload upload, string label)
    {
        if (upload?.packedWeights != null)
        {
            AexisGpuResourceTracker.ReleaseBuffer(upload.packedWeights, label);
            upload.packedWeights.Dispose();
        }
        if (upload?.scales != null)
        {
            AexisGpuResourceTracker.ReleaseBuffer(upload.scales, label);
