...
namespace Aexis.Ncnn
{
    ...
    {
        public string blob;
        // Shape encoding is [dims, w, h, d, c], matching NcnnGraphSession.BufferShape.
    ...
    public static class NcnnTextureExecutionPlanner
    {
            ...
            out string reason)
        {
            ...
            else if (string.Equals(operatorName, "Flatten", StringComparison.Ordinal))
            {
                ...
                outputLogicalShape = ToShapeArray(new NcnnGraphSession.BufferShape(1, ElementCount(input), 1, 1, 1));
            }
            else if (string.Equals(operatorName, "Squeeze", StringComparison.Ordinal))
            {
                ...
                try
                {
                    outputLogicalShape = ToShapeArray(NcnnGraphSession.ResolveSqueezeShape(input, layer));
                }
        ...
        private static bool TryResolveReshape(int[] sourceShape, NcnnParamModel.Layer layer, out int[] outputShape, out string reason)
        {
            ...
            try
            {
                var output = NcnnGraphSession.ResolveReshapeShape(source, layer);
        ...
        private static bool TryResolveExpandDims(int[] sourceShape, NcnnParamModel.Layer layer, out int[] outputShape, out string reason)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < axes.Length; i++)
                {
                    ...
                    var axis = NcnnGraphSession.MapNcnnAxisToTensorAxis(outDims, ncnnAxis);
        ...
        private static bool TryToBufferShape(int[] shape, out NcnnGraphSession.BufferShape value)
        {
            value = default;
            if (shape == null || shape.Length != 5 || shape[0] < 1 || shape[0] > 4
                || shape[1] <= 0 || shape[2] <= 0 || shape[3] <= 0 || shape[4] <= 0)
                return false;
            value = new NcnnGraphSession.BufferShape(shape[0], shape[1], shape[2], shape[3], shape[4]);
            return true;
        }

        private static int[] ToShapeArray(NcnnGraphSession.BufferShape shape)
        {
        ...
        private static int ElementCount(NcnnGraphSession.BufferShape shape)
        {
