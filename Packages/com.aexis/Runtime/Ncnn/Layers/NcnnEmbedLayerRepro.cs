    ...
    public sealed class NcnnEmbedLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        var ep = new NcnnGraphSession.EmbedPack();
                                        ...
                                        if (useSharedWeights)
                                            ...
                                            w = NcnnGraphSession.ReadClipArrayAsFloat32(br, ep.weightSize, 0);
                                        float[] b = null;
                                        if (ep.biasTerm != 0)
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var logicalShape = new NcnnGraphSession.BufferShape(2, Mathf.Max(1, ep.numOutput), words, 1, 1);
            var storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(logicalShape);
            var output = owner.RentTempMat(storageShape.w, storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
            ...
            {
                var indexShape = NcnnGraphSession.GetTextureShape(textureShapes, indexTexture, layer.bottomNames[0]);
                var indexStorage = NcnnGraphSession.GetTextureStorageShape(indexTexture, indexShape);
                var isLinear = NcnnGraphSession.IsStrictLinearMatTexture(indexTexture);
            ...
            NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, logicalShape, storageShape);
            owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
        }
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var logicalShape = new NcnnGraphSession.BufferShape(2, Mathf.Max(1, ep.numOutput), words, 1, 1);
            var storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(logicalShape);
            var output = owner.RentTempMat(cmd, storageShape.w, storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
            var src = NcnnGraphSession.GetCmdTensor(blobs, layer.bottomNames[0]);
            var srcStorage = NcnnGraphSession.GetCmdStorageShape(src, srcShape);
            var isLinear = NcnnGraphSession.IsStrictLinearMatTexture(src);
            ...
            blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(output, logicalShape, storageShape, owned: true);
            if (shapes != null)
            ...
            Dictionary<string, NcnnTensorBuffer> bufferViews)
        {
            ...
            {
                var shape = NcnnGraphSession.GetTextureShape(textureShapes, texture, name);
                return Mathf.Max(1, shape.w) * Mathf.Max(1, shape.h) * Mathf.Max(1, shape.d) * Mathf.Max(1, shape.c);
            }

            var view = NcnnGraphSession.TryGetBufferView(name, bufferBlobs, bufferViews);
            if (view != null)
