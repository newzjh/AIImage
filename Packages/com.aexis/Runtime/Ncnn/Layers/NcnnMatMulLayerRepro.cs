...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnMatMulLayerRepro : NcnnBaseLayerRepro
    {
        ...
        private sealed class VistaTailPack4Plan
        {
            ...
            public NcnnGraphSession.BufferShape featureShape;
            public NcnnGraphSession.BufferShape outputShape;
...
#pragma warning restore CS0618
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var bShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[1]);
        ...
        private static bool TryExecutePack4AttentionMatMulPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            {
                outShape = new NcnnGraphSession.BufferShape(2, Mathf.Max(1, n), Mathf.Max(1, aRows), 1, 1);
            }
            else if (maxDims >= 4 || outBatchD > 1)
            {
                outShape = new NcnnGraphSession.BufferShape(4, Mathf.Max(1, n), Mathf.Max(1, aRows), Mathf.Max(1, outBatchD), Mathf.Max(1, outBatchC));
            }
            else
            {
                outShape = new NcnnGraphSession.BufferShape(3, Mathf.Max(1, n), Mathf.Max(1, aRows), 1, Mathf.Max(1, outBatchC));
            }
            ...
            owner.Ops.MatMulPack4Cdhw(
                ...
                outShape.c,
                outRt);
            NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outRt, outShape, outShape);
        ...
        private static bool TryExecutePack4AttentionMatMulPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            if (owner == null || layer == null || context == null)
                return false;
            if (owner.ShouldForceCurrentLayerBufferPath())
            ...
            var aTex = NcnnGraphSession.GetCmdTensor(context.blobs, layer.bottomNames[0]);
            var bTex = NcnnGraphSession.GetCmdTensor(context.blobs, layer.bottomNames[1]);
            var aShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[0]);
            var bShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[1]);
            if (!CanUsePack4AttentionMatMul(aTex, aShape) || !CanUsePack4AttentionMatMul(bTex, bShape))
                return false;
            ...
            var outPacks = Mathf.Max(1, Mathf.CeilToInt(outShape.c / 4f));
            var outSlices = Mathf.Max(1, outShape.d) * outPacks;
            var outRt = owner.RentTempArray(context.commandBuffer, outShape.w, outShape.h, outSlices, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
            owner.Ops.MatMulPack4Cdhw(
        ...
        private static bool TryExecuteVistaTailPack4Path(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            if (owner == null || layer == null || context == null)
                return false;
            ...
            var promptView = NcnnGraphSession.TryGetBufferView(plan.promptBlobName, context.bufferBlobs, context.bufferViews);
            ...
            if (!usedPromptRt && promptBuf == null)
            {
                promptBuf = owner.GetOrConvertToBuffer(
                    ...
                    context.bufferViews,
                    context.tempOwned);
                promptView = NcnnGraphSession.TryGetBufferView(plan.promptBlobName, context.bufferBlobs, context.bufferViews);
            }
            ...
            NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outRt, plan.outputShape);
            owner.DebugLog?.Invoke(
            ...
            out VistaTailPack4Plan plan)
        {
            ...
            plan = new VistaTailPack4Plan
            {
                ...
                outputShape = new NcnnGraphSession.BufferShape(4, featureShape.w, featureShape.h, featureShape.d, 1),
        ...
        private static void GetMatrixShape(NcnnGraphSession.BufferShape shape, out int rows, out int cols)
        {
            if (shape.dims == 1)
            {
                rows = 1;
        ...
        private static bool CanUsePack4AttentionMatMul(NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape shape)
        {
            return src != null
                && src.texture != null
        ...
        private static bool CanUsePack4AttentionMatMul(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape shape)
        {
            return src != null
                && src.texture != null
                && (shape.dims == 3 || shape.dims == 4)
                ...
                && shape.c > 0
                && (shape.dims != 3 || shape.d == 1)
                && NcnnGraphSession.MatchesPack4TextureStorage(src, shape);
        }
    }
}
