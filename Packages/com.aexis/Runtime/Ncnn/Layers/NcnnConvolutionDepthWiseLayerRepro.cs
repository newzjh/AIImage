                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        else if (needDepthWiseTexturePack && !owner.UsesQuantizedWeightsForLayer(layer))
                                        {
                                            ...
                                            if (owner.UsesFp16WeightStorage)
                                                pack.packedDepthWiseWeight4Fp16 = NcnnGraphSession.NewFp16Vector4Buffer(w4, "NcnnGraphSession.ConvPackedDepthWiseWeight4Fp16:" + layer.name);
                                            phaseSw.Stop();
                                            packMs += phaseSw.ElapsedMilliseconds;
                                        }
...
#pragma warning restore CS0618
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            else
            {
                ...
                src = new NcnnGraphSession.TensorRef
                {
            ...
            var outWTex = NcnnGraphSession.ComputeConvOut(src.width, conv.kernelW, conv.dilationW, conv.strideW, conv.padLeft, conv.padRight);
            var outHTex = NcnnGraphSession.ComputeConvOut(src.height, conv.kernelH, conv.dilationH, conv.strideH, conv.padTop, conv.padBottom);
            ...
            textureShapes[layer.topNames[0]] = new NcnnGraphSession.BufferShape(3, outWTex, outHTex, 1, conv.outC);
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            if (!owner._conv.TryGetValue(layer.name, out var conv))
                throw new InvalidOperationException("ConvolutionDepthWise not found: " + layer.name);
        ...
        private static bool CanExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context, NcnnGraphSession.ConvPack conv)
        {
            ...
            var forceBufferThisConv = owner.ForceBufferConvolutionAll
                                      || (conv.useBufferPath && !canUseDepthWiseTexturePath && !canUseQuantizedTexturePath)
        ...
        private static bool SupportsDepthWiseTexturePath(NcnnGraphSession.ConvPack conv)
        {
            return conv != null
                && conv.isDepthWise
                && conv.group > 0
        ...
        private static string BuildUnsupportedMessage(NcnnParamModel.Layer layer, NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape logicalShape, string reason)
        {
            ...
            return "ConvolutionDepthWise CommandBuffer Pack4 rejected"
