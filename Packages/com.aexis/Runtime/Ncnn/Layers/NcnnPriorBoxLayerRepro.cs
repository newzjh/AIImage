...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnPriorBoxLayerRepro : NcnnBaseLayerRepro
    {
        ...
        {
            var pack = new NcnnGraphSession.PriorBoxPack
            {
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not NcnnGraphSession.PriorBoxPack pp)
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var spec = ResolvePriorBoxSpec(
                layer,
                pp,
                (string name, out NcnnGraphSession.BufferShape shape) =>
                {
            ...
            try
            {
                output = owner.RentTempMat(spec.storageShape.w, spec.storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.PriorBox(
                    output,
                    spec.mode,
                    spec.featW,
                    ...
                    spec.stepH,
                    pp.offset);
                NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, spec.logicalShape, spec.storageShape);
                output = null;
            }
            finally
            {
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var spec = ResolvePriorBoxSpec(
                ...
                (string name, out NcnnGraphSession.BufferShape shape) =>
                {
                    shape = NcnnGraphSession.GetCmdShape(shapes, blobs, name);
                    return true;
                });
            ...
            try
            {
                output = owner.RentTempMat(cmd, spec.storageShape.w, spec.storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.PriorBox(
                    cmd,
                    output,
                    spec.mode,
                    ...
                    spec.stepH,
                    pp.offset);
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(output, spec.logicalShape, spec.storageShape, owned: true);
                output = null;
                if (shapes != null)
        ...
        private readonly struct PriorBoxSpec
        {
            ...
            public PriorBoxSpec(int mode, int featW, int featH, int imageW, int imageH, int numPrior, float stepW, float stepH, NcnnGraphSession.BufferShape logicalShape)
            {
                this.mode = mode;
                this.featW = featW;
                this.featH = featH;
                ...
                storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(logicalShape);
            }
        }

        private delegate bool TryGetShapeDelegate(string name, out NcnnGraphSession.BufferShape shape);

        private static PriorBoxSpec ResolvePriorBoxSpec(NcnnParamModel.Layer layer, NcnnGraphSession.PriorBoxPack pp, TryGetShapeDelegate tryGetShape)
        {
            ...
            if (useMxnet)
            {
                ...
                return new PriorBoxSpec(0, featW, featH, 1, 1, numPrior, stepW, stepH, new NcnnGraphSession.BufferShape(1, total, 1, 1, 1));
            }
            ...
            return new PriorBoxSpec(1, featW, featH, imageW, imageH, caffeNumPrior, stepWidth, stepHeight, new NcnnGraphSession.BufferShape(2, caffeTotal, 2, 1, 1));
        }

        private static ComputeBuffer NewParamBuffer(float[] values)
        {
            return NcnnGraphSession.NewBuffer(values != null && values.Length > 0 ? values : new[] { 0f });
        }
        ...
        private static float[] BuildMxnetPrior(int w, int h, NcnnGraphSession.PriorBoxPack pp)
        {
        ...
        private static float[] BuildCaffePrior(int w, int h, int imageW, int imageH, NcnnGraphSession.PriorBoxPack pp)
        {
