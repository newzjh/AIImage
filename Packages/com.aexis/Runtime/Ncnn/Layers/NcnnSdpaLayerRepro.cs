...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnSdpaLayerRepro : NcnnBaseLayerRepro
    {
        ...
        {
            owner._extraPacks[layer.name] = new NcnnGraphSession.SdpaPack
            {
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not NcnnGraphSession.SdpaPack sp)
            ...
            var queryView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[0], bufferBlobs, bufferViews);
            var keyView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[1], bufferBlobs, bufferViews);
            var valueView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[2], bufferBlobs, bufferViews);
            ...
            if (sp.attnMask)
            {
                ...
                attnMaskView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[3], bufferBlobs, bufferViews);
            ...
            if (sp.kvCache)
            {
                ...
                var pastKeyView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[pastKeyBottom], bufferBlobs, bufferViews);
                var pastValueView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[pastValueBottom], bufferBlobs, bufferViews);
...
#pragma warning restore CS0618
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var queryShape = NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[0]);
            var keyShape = NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[1]);
            var valueShape = NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[2]);
            ...
            {
                var pastKeyBottom = sp.attnMask ? 4 : 3;
                var pastKeyShape = NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[pastKeyBottom]);
                dstSeqLen += pastKeyShape.h;
            }

            var outShape = new NcnnGraphSession.BufferShape(3, valueShape.w, queryShape.h, 1, queryShape.c);
        ...
        private static bool TryExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            try
            {
                ...
                if (ResolveSdpaFastPathEnabled() || plan.hasAttnMask || plan.causal)
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], output, plan.outputShape, plan.outputStorageShape);
                    output = null;
        ...
        private static bool TryExecuteCommandBufferTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            if (owner == null || layer == null || context == null)
                return false;
            if (owner.ShouldForceCurrentLayerBufferPath())
            ...
            try
            {
                ...
                if (ResolveSdpaFastPathEnabled() || plan.hasAttnMask || plan.causal)
                {
                    ...
                    context.blobs[layer.topNames[0]] = new NcnnGraphSession.CmdTensorRef
                    {
                        texture = output,
            ...
            ref RenderTexture valueCache)
        {
            ...
            if (inPlaceKvCache)
            {
                var inPlaceKeyStorageShape = NcnnGraphSession.GetTextureStorageShape(plan.pastKey, plan.keyShape);
                var inPlaceValueStorageShape = NcnnGraphSession.GetTextureStorageShape(plan.pastValue, plan.valueShape);
                context.textureBlobs[layer.topNames[1]] = NcnnGraphSession.CreateTextureAlias(plan.pastKey, plan.keyShape, inPlaceKeyStorageShape);
                context.textureBlobs[layer.topNames[2]] = NcnnGraphSession.CreateTextureAlias(plan.pastValue, plan.valueShape, inPlaceValueStorageShape);
            ...
            {
                var keyCapacityStorageShape = new NcnnGraphSession.BufferShape(3, plan.keyShape.w, keyCache.height, plan.keyShape.d, plan.keyShape.c);
                var valueCapacityStorageShape = new NcnnGraphSession.BufferShape(3, plan.valueShape.w, valueCache.height, plan.valueShape.d, plan.valueShape.c);
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[1], keyCache, plan.keyShape, keyCapacityStorageShape);
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[2], valueCache, plan.valueShape, valueCapacityStorageShape);
                keyCache = null;
                valueCache = null;
                return;
            }

            var keyStorageShape = NcnnGraphSession.GetTextureStorageShape(plan.key, plan.keyCurrentShape);
            var valueStorageShape = NcnnGraphSession.GetTextureStorageShape(plan.value, plan.valueCurrentShape);
            context.textureBlobs[layer.topNames[1]] = NcnnGraphSession.CreateTextureAlias(plan.key, plan.keyShape, keyStorageShape);
            context.textureBlobs[layer.topNames[2]] = NcnnGraphSession.CreateTextureAlias(plan.value, plan.valueShape, valueStorageShape);
            context.textureShapes[layer.topNames[1]] = plan.keyShape;
            context.textureShapes[layer.topNames[2]] = plan.valueShape;
        }

        private static bool CanAppendInPlace(NcnnGraphSession owner, in SdpaRtPlan plan)
        {
            ...
            ref ComputeTexture valueCache)
        {
            ...
            if (plan.hasPastCache)
            {
                var keyCapacityStorageShape = new NcnnGraphSession.BufferShape(3, plan.keyShape.w, keyCache.height, plan.keyShape.d, plan.keyShape.c);
                var valueCapacityStorageShape = new NcnnGraphSession.BufferShape(3, plan.valueShape.w, valueCache.height, plan.valueShape.d, plan.valueShape.c);
                context.blobs[layer.topNames[1]] = NcnnGraphSession.CreateCmdTensorRef(keyCache, plan.keyShape, keyCapacityStorageShape, owned: true);
                context.blobs[layer.topNames[2]] = NcnnGraphSession.CreateCmdTensorRef(valueCache, plan.valueShape, valueCapacityStorageShape, owned: true);
                keyCache = null;
                valueCache = null;
            }
            else
            {
                var keyStorageShape = NcnnGraphSession.GetCmdStorageShape(plan.key, plan.keyCurrentShape);
                var valueStorageShape = NcnnGraphSession.GetCmdStorageShape(plan.value, plan.valueCurrentShape);
                context.blobs[layer.topNames[1]] = NcnnGraphSession.CreateCmdTensorAlias(plan.key, plan.keyShape, keyStorageShape);
                context.blobs[layer.topNames[2]] = NcnnGraphSession.CreateCmdTensorAlias(plan.value, plan.valueShape, valueStorageShape);
            }
            ...
            out SdpaRtPlan plan)
        {
            ...
            NcnnGraphSession.TensorRef pastKey = null;
            NcnnGraphSession.TensorRef pastValue = null;
            NcnnGraphSession.BufferShape pastKeyShape = default;
            NcnnGraphSession.BufferShape pastValueShape = default;
            ...
            if (common.sp.kvCache)
            {
                ...
                var hasPastKey = layer.bottomNames.Length > pastKeyBottom
                    && NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[pastKeyBottom], out pastKey, out pastKeyShape);
                var hasPastValue = layer.bottomNames.Length > pastValueBottom
                    && NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[pastValueBottom], out pastValue, out pastValueShape);
                ...
                if (hasPastKey)
                {
                    if (!TryValidatePastCache(keyShape, valueShape, pastKey, pastKeyShape, pastValue, pastValueShape))
                        return false;
                    attentionKeyShape = new NcnnGraphSession.BufferShape(3, keyShape.w, pastKeyShape.h + keyShape.h, 1, keyShape.c);
                    attentionValueShape = new NcnnGraphSession.BufferShape(3, valueShape.w, pastValueShape.h + valueShape.h, 1, valueShape.c);
                }
            }
            ...
            NcnnGraphSession.TensorRef attnMask = null;
            NcnnGraphSession.BufferShape attnMaskShape = default;
            if (common.sp.attnMask)
            {
                if (layer.bottomNames.Length <= 3
                    ...
                    || attnMask.packs != 1
                    || NcnnGraphSession.IsStrictLinearMatTexture(attnMask))
                    return false;
            }
            ...
            out SdpaCmdRtPlan plan)
        {
            ...
            NcnnGraphSession.CmdTensorRef pastKey = null;
            NcnnGraphSession.CmdTensorRef pastValue = null;
            NcnnGraphSession.BufferShape pastKeyShape = default;
            NcnnGraphSession.BufferShape pastValueShape = default;
            var attentionKeyShape = keyShape;
            var attentionValueShape = valueShape;
            ...
            NcnnGraphSession.CmdTensorRef attnMask = null;
            NcnnGraphSession.BufferShape attnMaskShape = default;
            if (common.sp.attnMask)
            {
                if (layer.bottomNames.Length <= 3
        ...
        private static bool TryResolveCommonPlan(NcnnGraphSession owner, NcnnParamModel.Layer layer, out SdpaCommonPlan plan)
        {
            plan = default;
        ...
        private static bool TryValidateTextureInputs(
            object queryTex,
            NcnnGraphSession.BufferShape queryShape,
            object keyTex,
            NcnnGraphSession.BufferShape keyShape,
            object valueTex,
            NcnnGraphSession.BufferShape valueShape,
            in SdpaCommonPlan common,
            out NcnnGraphSession.BufferShape outputShape,
            out NcnnGraphSession.BufferShape outputStorageShape)
        {
            outputShape = default;
            ...
            outputShape = new NcnnGraphSession.BufferShape(3, valueShape.w, queryShape.h, 1, queryShape.c);
            outputStorageShape = outputShape;
            return true;
        }

        private static bool TryValidatePastCache(
            NcnnGraphSession.BufferShape currentKeyShape,
            NcnnGraphSession.BufferShape currentValueShape,
            object pastKey,
            NcnnGraphSession.BufferShape pastKeyShape,
            object pastValue,
            NcnnGraphSession.BufferShape pastValueShape)
        {
            ...
            out NcnnGraphSession.BufferShape shape)
        {
            ...
            shape = NcnnGraphSession.GetCmdShape(shapes, blobs, blobName);
            return shape.dims >= 1 && shape.dims <= 4;
        }

        private static void ReturnTemp(NcnnGraphSession owner, ref RenderTexture texture)
        {
        ...
        private static void ReturnTemp(NcnnGraphSession owner, CommandBuffer cmd, ref ComputeTexture texture)
        {
        ...
        {
            public readonly NcnnGraphSession.SdpaPack sp;
            public readonly float scale;

            public SdpaCommonPlan(NcnnGraphSession.SdpaPack sp)
            {
                this.sp = sp;
                scale = sp.scale;
            }
        ...
        private readonly struct SdpaRtPlan
        {
            ...
            public readonly NcnnGraphSession.TensorRef query;
            public readonly NcnnGraphSession.BufferShape queryShape;
            public readonly NcnnGraphSession.BufferShape queryStorageShape;
            public readonly int querySlices;
            public readonly NcnnGraphSession.TensorRef key;
            public readonly NcnnGraphSession.BufferShape keyCurrentShape;
            public readonly NcnnGraphSession.BufferShape keyShape;
            public readonly NcnnGraphSession.TensorRef value;
            public readonly NcnnGraphSession.BufferShape valueCurrentShape;
            public readonly NcnnGraphSession.BufferShape valueShape;
            public readonly NcnnGraphSession.TensorRef pastKey;
            public readonly NcnnGraphSession.BufferShape pastKeyShape;
            public readonly NcnnGraphSession.TensorRef pastValue;
            public readonly NcnnGraphSession.BufferShape pastValueShape;
            public readonly bool hasPastCache;
            public readonly NcnnGraphSession.TensorRef attnMask;
            public readonly bool hasAttnMask;
            public readonly NcnnGraphSession.BufferShape keyTransposedShape;
            public readonly NcnnGraphSession.BufferShape keyTransposedStorageShape;
            public readonly int keyTransposedSlices;
            public readonly NcnnGraphSession.BufferShape scoresShape;
            public readonly NcnnGraphSession.BufferShape scoresStorageShape;
            public readonly int scoresSlices;
            public readonly NcnnGraphSession.BufferShape outputShape;
            public readonly NcnnGraphSession.BufferShape outputStorageShape;
            ...
            public SdpaRtPlan(
                float scale,
                bool causal,
                NcnnGraphSession.TensorRef query,
                NcnnGraphSession.BufferShape queryShape,
                NcnnGraphSession.TensorRef key,
                NcnnGraphSession.BufferShape keyShape,
                NcnnGraphSession.TensorRef value,
                NcnnGraphSession.BufferShape valueShape,
                NcnnGraphSession.TensorRef pastKey,
                NcnnGraphSession.BufferShape pastKeyShape,
                NcnnGraphSession.TensorRef pastValue,
                NcnnGraphSession.BufferShape pastValueShape,
                NcnnGraphSession.BufferShape attentionKeyShape,
                NcnnGraphSession.BufferShape attentionValueShape,
                NcnnGraphSession.TensorRef attnMask,
                NcnnGraphSession.BufferShape outputShape,
                NcnnGraphSession.BufferShape outputStorageShape)
            {
                ...
                keyTransposedShape = new NcnnGraphSession.BufferShape(4, keyShape.h, keyShape.w, Mathf.Max(1, keyShape.d), keyShape.c);
                ...
                scoresShape = new NcnnGraphSession.BufferShape(4, keyShape.h, queryShape.h, Mathf.Max(1, queryShape.d), queryShape.c);
                ...
                pack4TextureFormat = NcnnGraphSession.ResolveTensorTextureFormat(4);
            }
        }
        ...
        private readonly struct SdpaCmdRtPlan
        {
            public readonly float scale;
            public readonly bool causal;
            public readonly NcnnGraphSession.CmdTensorRef query;
            public readonly NcnnGraphSession.BufferShape queryShape;
            public readonly NcnnGraphSession.BufferShape queryStorageShape;
            public readonly int querySlices;
            public readonly NcnnGraphSession.CmdTensorRef key;
            public readonly NcnnGraphSession.BufferShape keyCurrentShape;
            public readonly NcnnGraphSession.BufferShape keyShape;
            public readonly NcnnGraphSession.CmdTensorRef value;
            public readonly NcnnGraphSession.BufferShape valueCurrentShape;
            public readonly NcnnGraphSession.BufferShape valueShape;
            public readonly NcnnGraphSession.CmdTensorRef pastKey;
            public readonly NcnnGraphSession.BufferShape pastKeyShape;
            public readonly NcnnGraphSession.CmdTensorRef pastValue;
            public readonly NcnnGraphSession.BufferShape pastValueShape;
            public readonly bool hasPastCache;
            public readonly NcnnGraphSession.CmdTensorRef attnMask;
            public readonly bool hasAttnMask;
            public readonly NcnnGraphSession.BufferShape keyTransposedShape;
            public readonly NcnnGraphSession.BufferShape keyTransposedStorageShape;
            public readonly int keyTransposedSlices;
            ...
            public SdpaCmdRtPlan(
                float scale,
                bool causal,
                NcnnGraphSession.CmdTensorRef query,
                NcnnGraphSession.BufferShape queryShape,
                NcnnGraphSession.CmdTensorRef key,
                NcnnGraphSession.BufferShape keyShape,
                NcnnGraphSession.CmdTensorRef value,
                NcnnGraphSession.BufferShape valueShape,
                NcnnGraphSession.CmdTensorRef pastKey,
                NcnnGraphSession.BufferShape pastKeyShape,
                NcnnGraphSession.CmdTensorRef pastValue,
                NcnnGraphSession.BufferShape pastValueShape,
                NcnnGraphSession.BufferShape attentionKeyShape,
                NcnnGraphSession.BufferShape attentionValueShape,
                NcnnGraphSession.CmdTensorRef attnMask,
                NcnnGraphSession.BufferShape outputShape,
                NcnnGraphSession.BufferShape outputStorageShape)
            {
                this.scale = Mathf.Approximately(scale, 0f) ? 1f / Mathf.Sqrt(Mathf.Max(1, queryShape.w)) : scale;
