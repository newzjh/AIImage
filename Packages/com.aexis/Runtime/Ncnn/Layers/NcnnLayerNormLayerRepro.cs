    ...
    public sealed class NcnnLayerNormLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        var lp = new NcnnGraphSession.LayerNormPack();
        ...
        public override void ExecuteBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            if (!owner.ShouldForceCurrentLayerBufferPath()
                ...
                && NcnnGraphSession.TryGetExistingTexture(context.textureBlobs, context.textureShapes, layer.bottomNames[0], out var src, out var srcShape)
...
#pragma warning restore CS0618
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var isStrictLinear = NcnnGraphSession.IsStrictLinearMatTexture(srcTex);
            ...
            try
            {
                ...
                {
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outRt, srcShape, srcShape);
                    outRt = null;
                }
            }
            finally
        ...
        private static bool ShouldPromoteAttentionPrepTexture(NcnnGraphSession owner, NcnnParamModel.Layer layer)
        {
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            if (CanUsePack4WidthCmdPath(src, srcShape, lp))
            {
                ...
                var isStrictLinear = NcnnGraphSession.IsStrictLinearMatTexture(src);
                var packCount = Mathf.Max(1, Mathf.CeilToInt(Mathf.Max(1, srcShape.c) / 4f));
                var logicalDepth = srcShape.dims == 4 ? Mathf.Max(1, srcShape.d) : 1;
                var sliceCount = logicalDepth * packCount;
                ...
                try
                {
                    ...
                    {
                        blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outArr, srcShape, srcShape, owned: true);
                        outArr = null;
                    }
                }
                finally
        ...
        private static RenderTextureFormat ResolveLayerNormOutputFormat(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnGraphSession.BufferShape srcShape)
        {
            if (ShouldPromoteAttentionPrepTexture(owner, layer))
                return RenderTextureFormat.ARGBFloat;
            return NcnnGraphSession.ResolveTensorTextureFormat(srcShape.dims);
        }

        private static bool TryGetPack4WidthTexture(
            NcnnGraphSession owner,
            NcnnParamModel.Layer layer,
            NcnnGraphSession.LayerNormPack lp,
        ...
        private static bool CanUsePack4WidthPath(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.LayerNormPack lp)
        {
            if (CanUsePack4Linear2DPath(srcTex, srcShape, lp))
                return true;

            if (srcTex != null && NcnnGraphSession.IsStrictLinearMatTexture(srcTex))
            {
                var storageShape = NcnnGraphSession.GetTextureStorageShape(srcTex, srcShape);
                return lp != null
                    && lp.affine
        ...
        private static bool CanUsePack4WidthCmdPath(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.LayerNormPack lp)
        {
            if (CanUsePack4Linear2DPath(src, srcShape, lp))
                return true;

            if (src != null && NcnnGraphSession.IsStrictLinearMatTexture(src))
            {
                var storageShape = NcnnGraphSession.GetCmdStorageShape(src, srcShape);
        ...
        private static bool CanUsePack4Linear2DPath(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.LayerNormPack lp)
        {
            return srcTex != null
                && srcTex.texture != null
                && lp != null
                ...
                && srcShape.h > 0
                && srcShape.w == lp.affineSize
                && NcnnGraphSession.IsPack4LinearMatTexture(srcTex, srcShape);
        }

        private static bool CanUsePack4Linear2DPath(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.LayerNormPack lp)
        {
            return src != null
                && src.texture != null
                && lp != null
