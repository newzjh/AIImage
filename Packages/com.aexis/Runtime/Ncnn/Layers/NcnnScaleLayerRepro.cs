...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnScaleLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
            ...
            var sp = new NcnnGraphSession.ScalePack
            {
            ...
            if (!sp.dynamic)
            {
                ...
                phaseSw.Restart();
                sp.scale = NcnnGraphSession.NewBuffer(sp.scaleCpu);
                if (sp.biasCpu != null)
                    sp.bias = NcnnGraphSession.NewBuffer(sp.biasCpu);
                phaseSw.Stop();
        ...
        {
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not NcnnGraphSession.ScalePack sp)
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (sp.dynamic)
            {
                ...
                var scaleView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[1], bufferBlobs, bufferViews);
