...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnNormalizeLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
            ...
            var np = new NcnnGraphSession.NormalizePack
            {
            ...
            if (np.scaleDataSize > 0)
            {
                ...
                phaseSw.Restart();
                np.scale = NcnnGraphSession.NewBuffer(np.scaleCpu);
                phaseSw.Stop();
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not NcnnGraphSession.NormalizePack np)
