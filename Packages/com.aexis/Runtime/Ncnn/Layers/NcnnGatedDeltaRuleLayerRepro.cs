...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnGatedDeltaRuleLayerRepro : NcnnBaseLayerRepro
    {
        ...
        {
            owner._extraPacks[layer.name] = new NcnnGraphSession.GatedDeltaRulePack
            {
                epsilon = 1e-6f
            };
            return default;
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var qShape = NcnnGraphSession.GetTextureShape(context.textureShapes, q, layer.bottomNames[4]);
            var vShape = NcnnGraphSession.GetTextureShape(context.textureShapes, v, layer.bottomNames[6]);
            var stateShape = NcnnGraphSession.GetTextureShape(context.textureShapes, state, layer.bottomNames[7]);
            ...
            try
            {
                ...
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], output, vShape, NcnnGraphSession.GetTextureStorageShape(v, vShape));
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[1], finalState, stateShape, NcnnGraphSession.GetTextureStorageShape(state, stateShape));
                output = null;
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var qShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[4]);
            var vShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[6]);
            var stateShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[7]);
            var heads = Mathf.Max(1, qShape.h);
            var sequence = Mathf.Max(1, qShape.c);
            ...
            context.blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(output, vShape, new NcnnGraphSession.BufferShape(vShape.dims, output.width, output.height, vShape.d, output.depth * 4), owned: true);
            context.blobs[layer.topNames[1]] = NcnnGraphSession.CreateCmdTensorRef(finalState, stateShape, new NcnnGraphSession.BufferShape(stateShape.dims, finalState.width, finalState.height, stateShape.d, finalState.depth * 4), owned: true);
        ...
        private static ScalarView RequireScalar(NcnnGraphSession owner, NcnnLayerBufferContext context, string name, string layer)
        {
            ...
            var shape = NcnnGraphSession.GetTextureShape(context.textureShapes, tensor, name);
            var elements = Mathf.Max(1, shape.w) * Mathf.Max(1, shape.h) * Mathf.Max(1, shape.d) * Mathf.Max(1, shape.c);
            var linear = owner.RentTempMat(elements, 1, NcnnGraphSession.ResolveLinearMatTextureFormat());
            owner.Ops.ReshapePack4ToLinearMat(
                ...
                shape.dims,
                linear,
                inputPack4Linear: NcnnGraphSession.IsPack4LinearMatTexture(tensor, shape));
            return new ScalarView { texture = linear, temporary = true };
        }

        private static NcnnGraphSession.CmdTensorRef RequireCmd2D(NcnnLayerCommandBufferContext context, string name, string layer)
        {
        ...
        private static NcnnGraphSession.TensorRef RequireArray(NcnnLayerBufferContext context, string name, string layer)
        {
