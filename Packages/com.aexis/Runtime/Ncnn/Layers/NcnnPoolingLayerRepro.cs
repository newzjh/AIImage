...
#pragma warning restore CS0618
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            try
            {
                ...
                var outShape = new NcnnGraphSession.BufferShape(3, outW, outH, 1, srcShape.c);
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
                        ...
                        do
                        {
                                                ...
                                                var outW = globalPooling != 0 ? 1 : Mathf.Max(1, NcnnGraphSession.ComputeConvOut(srcShape.w, kernelW, 1, strideW, padLeft, padRight));
                                                var outH = globalPooling != 0 ? 1 : Mathf.Max(1, NcnnGraphSession.ComputeConvOut(srcShape.h, kernelH, 1, strideH, padTop, padBottom));
                                                ...
                                                blobs[layer.topNames[0]] = new NcnnGraphSession.CmdTensorRef { texture = outArr, width = outW, height = outH, packs = src.packs, refs = 1, owned = true };
                                                if (shapes != null)
        ...
        private static bool CanUsePack4CmdPath(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
                && srcShape.dims == 3
        ...
        private static NcnnGraphSession.BufferShape ResolveCmdOutputShape(
            ...
            bool adaptivePooling)
        {
            ...
            if (srcShape.dims != 3)
                return new NcnnGraphSession.BufferShape(srcShape.dims, Mathf.Max(1, srcShape.w), Mathf.Max(1, srcShape.h), Mathf.Max(1, srcShape.d), outC);

            if (globalPooling)
                return new NcnnGraphSession.BufferShape(3, 1, 1, 1, outC);
            ...
            if (adaptivePooling)
            {
                ...
                return new NcnnGraphSession.BufferShape(3, Mathf.Max(1, outW), Mathf.Max(1, outH), 1, outC);
            }

            var resolvedW = Mathf.Max(1, NcnnGraphSession.ComputeConvOut(srcShape.w, kernelW, 1, strideW, padLeft, padRight));
            var resolvedH = Mathf.Max(1, NcnnGraphSession.ComputeConvOut(srcShape.h, kernelH, 1, strideH, padTop, padBottom));
            return new NcnnGraphSession.BufferShape(3, resolvedW, resolvedH, 1, outC);
        }
    }
}
