...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnSliceLayerRepro : NcnnBaseLayerRepro
    {
        private readonly struct SliceSpec
        {
            ...
            public readonly NcnnGraphSession.BufferShape shape;

            public SliceSpec(int axis, int begin, NcnnGraphSession.BufferShape shape)
            {
                this.axis = axis;
                this.begin = begin;
                this.shape = shape;
        ...
        {
            if (NcnnGraphSession.TryGetExistingTexture(context.textureBlobs, context.textureShapes, layer.bottomNames[0], out var existingTex, out var existingShape))
            {
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (bufferViews.TryGetValue(layer.bottomNames[0], out var existingView) && existingView != null)
                srcShape = NcnnGraphSession.GetShapeOf(existingView);
            else if (textureBlobs.TryGetValue(layer.bottomNames[0], out var existingTex) && existingTex != null && existingTex.texture != null)
                srcShape = NcnnGraphSession.GetTextureShape(textureShapes, existingTex, layer.bottomNames[0]);
            else
            {
                ...
                var srcViewTmp = NcnnGraphSession.TryGetBufferView(layer.bottomNames[0], bufferBlobs, bufferViews);
                if (srcBufTmp == null || srcViewTmp == null)
                    throw new InvalidOperationException("Slice source not found: " + layer.name);
                srcShape = NcnnGraphSession.GetShapeOf(srcViewTmp);
            }
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (!NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[0], out var srcTex, out var texShape)
            ...
            for (var i = 0; i < layer.topNames.Length; i++)
            {
                ...
                if (canUseLinearMat)
                {
                    var storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(spec.shape);
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[i], outMat, spec.shape, storageShape);
                }
                else if (canUsePack4LinearMat)
                {
                    var srcStorageShape = NcnnGraphSession.GetTextureStorageShape(srcTex, texShape);
                    var outStorageShape = NcnnGraphSession.ResolvePack4LinearMatStorageShape(spec.shape);
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[i], outRt, spec.shape, outStorageShape);
                }
                else if (texShape.dims == 4)
                {
                    ...
                    var outRt = owner.RentTempArray(spec.shape.w, spec.shape.h, outSlices, NcnnGraphSession.ResolveTensorTextureFormat(spec.shape.dims));
                    owner.Ops.SlicePack4Cdhw(
                        ...
                        spec.shape.c,
                        outRt);
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[i], outRt, spec.shape, spec.shape);
                }
                else
                {
                    var outPacks = Mathf.Max(1, Mathf.CeilToInt(spec.shape.c / 4f));
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[i], outRt, spec.shape);
                }
            }
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            for (var i = 0; i < layer.topNames.Length; i++)
            {
                ...
                if (canUseLinearMat || canUsePack4LinearMat || canUsePack4)
                {
                    ...
                    {
                        blobs[layer.topNames[i]] = NcnnGraphSession.CreateCmdTensorAlias(src, spec.shape, sourceContract.StorageShape);
                        if (shapes != null)
                    ...
                    if (canUseLinearMat)
                    {
                        ...
                        blobs[layer.topNames[i]] = NcnnGraphSession.CreateCmdTensorRef(outMat, spec.shape, storageShape, owned: true);
                        shapes[layer.topNames[i]] = spec.shape;
                        continue;
                    }
                    ...
                    {
                        var srcStorageShape = NcnnGraphSession.GetCmdStorageShape(src, srcShape);
                        var outStorageShape = NcnnGraphSession.ResolvePack4LinearMatStorageShape(spec.shape);
                        ...
                        blobs[layer.topNames[i]] = NcnnGraphSession.CreateCmdTensorRef(pack4LinearOut, spec.shape, outStorageShape, owned: true);
                        shapes[layer.topNames[i]] = spec.shape;
                        continue;
                    }
                    ...
                    var outFormat = srcShape.dims == 4 ? NcnnGraphSession.ResolveTensorTextureFormat(spec.shape.dims) : RenderTextureFormat.ARGBHalf;
                    ...
                    var directStorageShape = spec.shape.dims <= 2
                        ? new NcnnGraphSession.BufferShape(3, spec.shape.w, spec.shape.dims == 2 ? spec.shape.h : 1, 1, 1)
                        : spec.shape;
                    blobs[layer.topNames[i]] = NcnnGraphSession.CreateCmdTensorRef(outArr, spec.shape, directStorageShape, owned: true, blobName: layer.topNames[i]);
                    if (shapes != null)
                        shapes[layer.topNames[i]] = spec.shape;
                    continue;
                }
        ...
        private static SliceSpec[] ResolveSliceSpecs(NcnnParamModel.Layer layer, NcnnGraphSession.BufferShape srcShape)
        {
            ...
            for (var i = 0; i < layer.topNames.Length; i++)
            {
                ...
                specs[i] = new SliceSpec(axis, begin, new NcnnGraphSession.BufferShape(srcShape.dims, outW, outH, outD, outC));
                begin += sliceSize;
            }
        ...
        private static bool CanUsePack4Slice(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape)
        {
            return srcTex != null
                && srcTex.texture != null
                && !NcnnGraphSession.IsStrictLinearMatTexture(srcTex)
                && srcShape.dims >= 1
                && srcShape.dims <= 4
        ...
        private static bool CanUsePack4Slice(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
        ...
        private static bool CanUseLinearMatSliceSource(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape)
        {
        ...
        private static bool CanUseLinearMatSliceSource(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
        ...
        private static bool CanUseLinearMatSlice(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape, SliceSpec[] specs)
        {
        ...
        private static bool CanUseLinearMatSlice(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, SliceSpec[] specs)
        {
        ...
        private static bool CanUsePack4LinearMatSlice(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape, SliceSpec[] specs)
        {
            if (!NcnnGraphSession.IsPack4LinearMatTexture(srcTex, srcShape) || specs == null || specs.Length == 0)
                return false;
            for (var i = 0; i < specs.Length; i++)
            {
                var spec = specs[i];
        ...
        private static bool CanUsePack4LinearMatSlice(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, SliceSpec[] specs)
        {
            if (!NcnnGraphSession.IsPack4LinearMatTexture(src, srcShape) || specs == null || specs.Length == 0)
                return false;
            for (var i = 0; i < specs.Length; i++)
            {
                var spec = specs[i];
        ...
        private static bool TryCopyPack4LinearMatSlice(RenderTexture input, NcnnGraphSession.BufferShape storageShape, SliceSpec spec, RenderTexture output)
        {
        ...
        private static bool TryCopyPack4LinearMatSlice(CommandBuffer cmd, ComputeTexture input, NcnnGraphSession.BufferShape storageShape, SliceSpec spec, ComputeTexture output)
        {
        ...
        private static bool CanCopyPack4LinearMatSlice(NcnnGraphSession.BufferShape storageShape, int srcX, int srcY, int copyW, int copyH)
        {
        ...
        private static bool IsIdentitySlice(NcnnGraphSession.BufferShape srcShape, SliceSpec spec)
        {
            return spec.begin == 0
