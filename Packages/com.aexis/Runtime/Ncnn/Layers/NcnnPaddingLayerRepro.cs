    ...
    public sealed class NcnnPaddingLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
            ...
            if (pp.perChannelPadDataSize > 0)
            {
                ...
                phaseSw.Restart();
                pp.perChannelPadData = NcnnGraphSession.NewBuffer(pp.perChannelPadDataCpu);
                phaseSw.Stop();
...
#pragma warning restore CS0618
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            NcnnGraphSession.SetTextureBlob(
                ...
                layer.topNames[0],
                outRt,
                new NcnnGraphSession.BufferShape(3, srcShape.w + pp.left + pp.right, srcShape.h + pp.top + pp.bottom, 1, srcShape.c));
            owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
        }
        ...
        private static bool CanUseSimplePack4CmdPath(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, PaddingPack pp)
        {
            return src != null
                && src.texture != null
                && pp.front == 0
        ...
        private static NcnnGraphSession.BufferShape ResolveCmdOutputShape(NcnnGraphSession.BufferShape srcShape, PaddingPack pp)
        {
            var outW = Mathf.Max(1, srcShape.w + pp.left + pp.right);
            if (srcShape.dims == 1)
                return new NcnnGraphSession.BufferShape(1, outW, 1, 1, 1);

            var outH = Mathf.Max(1, srcShape.h + pp.top + pp.bottom);
            if (srcShape.dims == 2)
                return new NcnnGraphSession.BufferShape(2, outW, outH, 1, 1);

            if (srcShape.dims == 3)
                return new NcnnGraphSession.BufferShape(3, outW, outH, 1, Mathf.Max(1, srcShape.c + pp.front + pp.behind));

            return new NcnnGraphSession.BufferShape(4, outW, outH, Mathf.Max(1, srcShape.d + pp.front + pp.behind), srcShape.c);
        }
