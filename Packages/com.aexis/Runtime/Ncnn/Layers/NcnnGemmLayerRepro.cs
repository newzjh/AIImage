    ...
    public sealed class NcnnGemmLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        var gp = new NcnnGraphSession.GemmPack
                                        {
                                        ...
                                        if (gp.constantB)
                                        {
                                            ...
                                            if (useSharedWeights)
                                                ...
                                                gp.bDataCpu = NcnnGraphSession.ReadClipMatAsFloat32(br, bw, bh, 0, 0, 0);
                                            if (gp.constantC && gp.broadcastTypeC != -1)
                                            {
                                                ...
                                                var c = NcnnGraphSession.ReadClipMatAsFloat32(br, cw, ch, 0, 0, 0);
                                                gp.cDataCpu = c;
                                            }
                                            phaseSw.Stop();
                                            ...
                                            {
                                                var quantized = NcnnGraphSession.NewInt4WeightOnlyUpload(
                                                    gp.bDataCpu,
                                                    gp.constantN,
                                                    gp.constantK,
                                            ...
                                            else if (owner.UsesInt8WeightOnlyForLayer(layer))
                                            {
                                                var quantized = directQ8 != null
                                                    ? NcnnGraphSession.NewInt8WeightOnlyUpload(
                                                        directQ8,
                                                        gp.constantN,
                                                        "NcnnGraphSession.GemmInt8WeightOnlyDirect:" + layer.name)
                                                    : NcnnGraphSession.NewInt8WeightOnlyUpload(
                                                        gp.bDataCpu,
                                                        gp.constantN,
                                                        gp.constantK,
                                            ...
                                            {
                                                gp.bData = NcnnGraphSession.NewBuffer(gp.bDataCpu);
                                            }
                                            if (owner.UsesFp16WeightStorage)
                                                gp.bDataFp16 = NcnnGraphSession.NewFp16Buffer(gp.bDataCpu, "NcnnGraphSession.GemmWeightFp16:" + layer.name);
                                            if (gp.cDataCpu != null)
                                                gp.cData = NcnnGraphSession.NewBuffer(gp.cDataCpu);
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
                        ...
                        do
                        {
                                                ...
                                                if (gp.transA)
                                                    throw new InvalidOperationException("Gemm transA is not supported in NcnnGraphSession: " + layer.name);
                                                ...
                                                else
                                                {
                                                    ...
                                                    var bView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[1], bufferBlobs, bufferViews);
                                                ...
                                                else if (!gp.constantC && layer.bottomNames.Length > 2)
                                                {
                                                    ...
                                                    useC = cBuf != null;
                                                    if (useC)
                                                        cCpu = NcnnGraphSession.ReadFloatBuffer(cBuf);
                                                }
                                                ...
                                                if (owner.ForceCpuGemmAll)
                                                {
                                                    ...
                                                    var cpuOut = NcnnGraphSession.RunGemmCpu(srcBuf, srcView, gp, cCpu);
...
#pragma warning restore CS0618
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var aShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[0]);
        ...
        private static void ConfigureGemmWeightBindings(NcnnGraphSession owner, NcnnParamModel.Layer layer)
        {
        ...
        private static bool TryExecuteCommandBufferTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var srcTex = NcnnGraphSession.GetCmdTensor(context.blobs, layer.bottomNames[0]);
            var srcShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[0]);
            if (srcTex == null || srcTex.texture == null)
                return false;
            if (srcShape.dims != 2)
                return false;
            var srcIsStrictLinear = NcnnGraphSession.IsStrictLinearMatTexture(srcTex);
            ...
            var outShape = new NcnnGraphSession.BufferShape(2, Mathf.Max(1, n), Mathf.Max(1, m), 1, 1);
            ...
            var outStorageShape = usePack4TiledMat
                ? NcnnGraphSession.ResolvePack4TiledLinearMatStorageShape(outShape)
                : usePack4LinearMat
                ? NcnnGraphSession.ResolvePack4LinearMatStorageShape(outShape)
                : useStrictLinearMat
                ? NcnnGraphSession.ResolveLinearMatStorageShape(outShape)
            ...
            var outRt = usePack4LinearMat
                ...
                : useStrictLinearMat
                ? owner.RentTempMat(context.commandBuffer, outStorageShape.w, outStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat())
                : owner.RentTempArray(
                    context.commandBuffer,
                    outShape.w,
                    outShape.h,
            ...
            context.blobs[layer.topNames[0]].layoutKind = NcnnGraphSession.ResolveNcnnTextureLayoutKind(outShape, outStorageShape, 1);
        ...
        private static bool TryExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var outRt = usePack4LinearMat
                ? owner.RentTempArray(
                    ...
                    ShouldPromoteAttentionGemmOutputTexture(owner, layer) ? RenderTextureFormat.ARGBFloat : RenderTextureFormat.ARGBHalf)
                : useStrictLinearMat
                ? owner.RentTempMat(outStorageShape.w, outStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat())
                : owner.RentTempArray(
                    outShape.w,
                    outShape.h,
                    1,
            ...
            NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outRt, outShape, outStorageShape);
            owner.DebugLog?.Invoke(
            ...
            NcnnGraphSession.GemmPack gp)
        {
            ...
            var logicalOutShape = new NcnnGraphSession.BufferShape(2, Mathf.Max(1, n), Mathf.Max(1, m), 1, 1);
            var useC = gp.constantC && gp.broadcastTypeC != -1 && gp.cData != null;
            ...
            if (TryResolveAttentionQkvProjectionSpec(owner, layer, logicalOutShape, out var packedQkvShape))
            {
                ...
                var srcIsStrictLinear = NcnnGraphSession.IsStrictLinearMatTexture(srcTex);
                var srcIsPack4Linear = NcnnGraphSession.IsPack4LinearMatTexture(srcTex, srcShape);
                ...
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outRt, logicalOutShape, packedQkvShape);
                owner.DebugLog?.Invoke(
                    "[AttentionQkv][Gemm]"
                    + " | layer=" + layer.name
            ...
            var outStorageShape = usePack4LinearOut
                ? NcnnGraphSession.ResolvePack4LinearMatStorageShape(logicalOutShape)
                : NcnnGraphSession.ResolveLinearMatStorageShape(logicalOutShape);
            ...
            NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outMat, logicalOutShape, outStorageShape);
            owner.DebugLog?.Invoke(
            ...
            NcnnGraphSession.GemmPack gp)
        {
            ...
            if (TryResolveAttentionQkvProjectionSpec(owner, layer, logicalOutShape, out var packedQkvShape))
            {
                ...
                context.blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outArr, logicalOutShape, packedQkvShape, owned: true);
            ...
            context.blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outMat, logicalOutShape, outStorageShape, owned: true);
            context.shapes[layer.topNames[0]] = logicalOutShape;
            owner.DebugLog?.Invoke(
        ...
        private static bool TryResolveAttentionQkvProjectionSpec(
            NcnnGraphSession owner,
            NcnnParamModel.Layer layer,
            NcnnGraphSession.BufferShape logicalOutShape,
            out NcnnGraphSession.BufferShape packedShape)
        {
            packedShape = default;
            ...
            var reshapeOut = NcnnGraphSession.ResolveReshapeShape(logicalOutShape, reshape);
            ...
            var permuteOut = NcnnGraphSession.ResolvePermuteShape(reshapeOut, 3, NcnnGraphSession.ResolvePermuteAxes(3, 2, permute.name));
        ...
        {
            packedShape = default;
            if (source == null || source.texture == null || logicalShape.dims != 2 || NcnnGraphSession.IsStrictLinearMatTexture(source))
                return false;

            var storageShape = NcnnGraphSession.GetTextureStorageShape(source, logicalShape);
            ...
            out NcnnGraphSession.BufferShape packedShape)
        {
            ...
            var storageShape = NcnnGraphSession.GetCmdStorageShape(source, logicalShape);
            if (storageShape.dims != 3 || storageShape.d != 1 || storageShape.w <= 0 || storageShape.h <= 0 || storageShape.c <= 1)
                return false;
            if (source.width != storageShape.w || source.height != storageShape.h)
                return false;
            ...
            NcnnGraphSession.GemmPack gp)
        {
            ...
            var outShape = new NcnnGraphSession.BufferShape(2, Mathf.Max(1, n), Mathf.Max(1, m), 1, 1);
            var useStrictLinearOut = NcnnGraphSession.IsStrictLinearMatTexture(aTex) || NcnnGraphSession.IsStrictLinearMatTexture(bTex);
            ...
            try
            {
                ...
                {
                    var outMat = owner.RentTempMat(context.commandBuffer, outStorageShape.w, outStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                    ...
                    context.blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outMat, outShape, outStorageShape, owned: true);
                }
                else
                {
                    context.blobs[layer.topNames[0]] = new NcnnGraphSession.CmdTensorRef
            ...
            NcnnGraphSession.GemmPack gp)
        {
            ...
            try
            {
                ...
                if (useStrictLinearOut)
                {
                    var outMat = owner.RentTempMat(outStorageShape.w, outStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                    owner.Ops.ReshapePack4ToLinearMat(outRt, outShape.w, outShape.h, outShape.d, outShape.c, outShape.dims, outMat);
                    NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outMat, outShape, outStorageShape);
                }
                else
                {
                    NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outRt, outShape, outStorageShape);
        ...
        private static void ReturnTempUnique(NcnnGraphSession owner, ref RenderTexture texture, RenderTexture alias0)
        {
        ...
        private static void ReturnTempUnique(NcnnGraphSession owner, CommandBuffer cmd, ref ComputeTexture texture, ComputeTexture alias0)
        {
            if (texture == null)
                return;
        ...
        private static bool ShouldPromoteAttentionGemmOutputTexture(NcnnGraphSession owner, NcnnParamModel.Layer layer)
        {
            if (owner?.Model?.layers == null || layer?.topNames == null || layer.topNames.Length == 0)
                return false;
        ...
        private static bool IsAttentionContextOutputProjection(NcnnGraphSession owner, NcnnParamModel.Layer layer)
        {
