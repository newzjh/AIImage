...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            {
                var src = NcnnGraphSession.GetTexture(textureBlobs, layer.bottomNames[0]);
                ...
                var softShape = NcnnGraphSession.GetTextureShape(textureShapes, src, layer.bottomNames[0]);
                textureShapes[layer.topNames[0]] = new NcnnGraphSession.BufferShape(3, src.width, src.height, 1, softShape.c);
            }
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[0], out var scalarSrcTex, out var scalarSrcShape)
                && CanUseScalar2DSoftmax(scalarSrcTex, scalarSrcShape))
            {
                ...
                if (NcnnGraphSession.IsStrictLinearMatTexture(scalarSrcTex))
                {
                    var storageShape = NcnnGraphSession.GetTextureStorageShape(scalarSrcTex, scalarSrcShape);
                    var outScalarRt = owner.RentTempMat(storageShape.w, storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                    owner.Ops.SoftmaxLinearMat2D(scalarSrcTex.texture, scalarSrcShape.w, scalarSrcShape.h, outScalarRt, scalarAxis);
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outScalarRt, scalarSrcShape, storageShape);
                }
                else
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outScalarRt, scalarSrcShape);
                }
            ...
            var outRt4 = owner.RentTempArray(srcTex.width, srcTex.height, outSlices, NcnnGraphSession.ResolveTensorTextureFormat(srcShape.dims));
            owner.Ops.SoftmaxPack4Cdhw(srcTex.texture, srcShape.w, srcShape.h, srcShape.d, srcShape.c, outRt4, tensorAxis);
            NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outRt4, srcShape, srcShape);
            owner.Consume(textureBlobs, context.bufferBlobs, context.bufferRefs, context.bufferViews, context.remaining, layer.bottomNames, context.pinnedNames);
        }
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            if (CanUseScalar2DSoftmax(src, srcShape))
            {
                ...
                if (NcnnGraphSession.IsStrictLinearMatTexture(src))
                {
                    ...
                    blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outRt, srcShape, storageShape, owned: true);
                }
                else
                {
                    var outRt = owner.RentTempArray(cmd, src.width, src.height, src.packs, NcnnGraphSession.ResolveTensorTextureFormat(2));
            ...
            var outPack4 = owner.RentTempArray(cmd, src.width, src.height, outSlices, NcnnGraphSession.ResolveTensorTextureFormat(srcShape.dims));
        ...
        private static bool CanUseScalar2DSoftmax(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
                && srcShape.dims == 2
        ...
        private static bool CanUseScalar2DSoftmax(NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
                && srcShape.dims == 2
        ...
        private static bool CanUsePack4Softmax(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
        ...
        private static bool CanUsePack4Softmax(NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
                && (srcShape.dims == 3 || srcShape.dims == 4)
        ...
        private static bool TryResolvePack4SoftmaxWidthAxis(NcnnParamModel.Layer layer, NcnnGraphSession.BufferShape srcShape, out int tensorAxis)
        {
            tensorAxis = -1;
            if (layer == null)
                return false;
            ...
            tensorAxis = NcnnGraphSession.MapNcnnAxisToTensorAxis(srcShape.dims, axis);
