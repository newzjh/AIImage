...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnConvolution1DLayerRepro : NcnnBaseLayerRepro
    {
        ...
        {
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not NcnnGraphSession.ConvPack conv)
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var outW = NcnnGraphSession.ComputeConvOut(inputW, conv.kernelW, conv.dilationW, conv.strideW, conv.padLeft, conv.padRight);
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var outShape = new NcnnGraphSession.BufferShape(2, outW, Mathf.Max(1, conv.outC), 1, 1);
            ...
            try
            {
                ...
                scalarOutput = owner.RentTempArray(outShape.w, outShape.h, 1, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
                owner.Ops.ReshapePack4ToScalar2D(packOutput, outW, 1, 1, conv.outC, 3, scalarOutput);
                NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], scalarOutput, outShape);
                scalarOutput = null;
            }
            finally
            {
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            try
            {
                ...
                scalarOutput = owner.RentTempArray(cmd, outShape.w, outShape.h, 1, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
                owner.Ops.ReshapePack4ToScalar2D(cmd, packOutput, outW, 1, 1, conv.outC, 3, scalarOutput);
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(scalarOutput, outShape, outShape, owned: true);
                scalarOutput = null;
                if (shapes != null)
        ...
        private static bool CanUseTexturePath(NcnnParamModel.Layer layer, NcnnLayerBufferContext context, NcnnGraphSession.ConvPack conv)
        {
        ...
        private static bool CanUseTexturePath(NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.ConvPack conv)
        {
        ...
        private static bool CanUseTexturePath(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.ConvPack conv)
        {
            if (src == null || src.texture == null || conv == null)
                return false;
            if (srcShape.dims != 2 || srcShape.w <= 0 || srcShape.h <= 0)
