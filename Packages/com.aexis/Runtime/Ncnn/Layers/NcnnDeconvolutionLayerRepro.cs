                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        if (canUseGeneralTexturePack)
                                        {
                                            ...
                                            if (owner.UsesFp16WeightStorage)
                                                pack.packedWeight4Fp16 = NcnnGraphSession.NewFp16Vector4Buffer(w4, "NcnnGraphSession.DeconvPackedWeight4Fp16:" + layer.name);
                                            phaseSw.Stop();
...
#pragma warning restore CS0618
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var outWTex = NcnnGraphSession.ComputeDeconvOut(srcShape.w, deconv.kernelW, deconv.dilationW, deconv.strideW, deconv.padLeft, deconv.padRight, deconv.outputPadRight);
            var outHTex = NcnnGraphSession.ComputeDeconvOut(srcShape.h, deconv.kernelH, deconv.dilationH, deconv.strideH, deconv.padTop, deconv.padBottom, deconv.outputPadBottom);
            var outShape = new NcnnGraphSession.BufferShape(3, outWTex, outHTex, 1, deconv.outC);
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var outShape = new NcnnGraphSession.BufferShape(3, outW, outH, 1, Mathf.Max(1, deconv.outC));

            ComputeTexture outArr = null;
            try
            {
                outArr = owner.RentTempArray(cmd, outW, outH, deconv.outPacks, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
                if (deconv.group == 1
        ...
        private static bool CanUsePack4Deconvolution(NcnnGraphSession.DeconvPack deconv)
        {
            return deconv != null
                && deconv.group == 1
                && deconv.packedWeight4 != null
