    ...
    public sealed class NcnnExpandDimsLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (bufferBlobs.TryGetValue(layer.bottomNames[0], out var expandBuf) && expandBuf != null)
            {
                ...
                {
                    var sourceContract = NcnnGraphSession.GetTextureContract(textureShapes, expandTex, layer.bottomNames[0]);
                    var srcShape = sourceContract.LogicalShape;
                    var outShape = ExpandTextureShape(srcShape, axes);
                    textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(expandTex, outShape, sourceContract.StorageShape);
            ...
            else
            {
                ...
                var srcShape = sourceContract.LogicalShape;
                var outShape = ExpandTextureShape(srcShape, axes);
                textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(src, outShape, sourceContract.StorageShape);
                textureShapes[layer.topNames[0]] = outShape;
            }
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            static NcnnGraphSession.BufferShape ExpandShape(NcnnGraphSession.BufferShape input, int[] expandAxes)
            {
                var dims = input.dims;
                var w = input.w;
                var h = input.h;
                ...
                for (var i = 0; i < expandAxes.Length; i++)
                {
                    ...
                    var tensorAxis = NcnnGraphSession.MapNcnnAxisToTensorAxis(outDims, ncnnAxis);
        ...
        private static int[] ResolveAxes(NcnnParamModel.Layer layer)
        {
            var axes = layer.GetInts(-23303, null);
            if (axes == null || axes.Length == 0)
                axes = layer.GetInts(3, Array.Empty<int>());
        ...
        private static NcnnTensorBuffer ExpandBufferView(NcnnTensorBuffer input, int[] expandAxes)
        {
            if (input == null)
        ...
        private static NcnnGraphSession.BufferShape ExpandTextureShape(NcnnGraphSession.BufferShape input, int[] expandAxes)
        {
            var dims = input.dims;
            var w = input.w;
            var h = input.h;
