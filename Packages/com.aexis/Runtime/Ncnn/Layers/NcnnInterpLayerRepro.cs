...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            ResolveTargetShape(
                layer,
                new NcnnGraphSession.BufferShape(srcView.dims, srcView.w, srcView.h, srcView.d, srcView.c),
                textureBlobs,
                textureShapes,
                bufferViews,
...
#pragma warning restore CS0618
            ...
            var srcData = NcnnGraphSession.ReadFloatBuffer(srcBuf);
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (srcShape.dims == 4)
            {
                ...
                var outRt4 = owner.RentTempArray(outW, outH, outD * srcTex.packs, NcnnGraphSession.ResolveTensorTextureFormat(4));
                ...
                NcnnGraphSession.SetTextureBlob(
                    ...
                    layer.topNames[0],
                    outRt4,
                    new NcnnGraphSession.BufferShape(4, outW, outH, outD, outC));
                owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
                return;
            }
            ...
            var outShape = new NcnnGraphSession.BufferShape(3, outW, outH, 1, outC);
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            if (srcShape.dims == 4)
            {
                ...
                var outRt4 = owner.RentTempArray(cmd, outShape.w, outShape.h, outShape.d * outPacks, NcnnGraphSession.ResolveTensorTextureFormat(4));
                owner.Ops.InterpPack4CDHW(
                    cmd,
                    src.texture,
                    srcShape.w,
            ...
            {
                NcnnGraphSession.ResolveCmdTextureLayout(outShape, out var placeholderW, out var placeholderH, out var placeholderPacks);
            ...
            if (!alignCornersPack && Mathf.Abs(sx - 2f) < 1e-3f && Mathf.Abs(sy - 2f) < 1e-3f)
            {
                ...
                blobs[layer.topNames[0]] = new NcnnGraphSession.CmdTensorRef { texture = outArr, width = src.width * 2, height = src.height * 2, packs = src.packs, refs = 1, owned = true };
                if (shapes != null)
                    shapes[layer.topNames[0]] = outShape;
                owner.ConsumeCmd(cmd, blobs, remaining, layer.bottomNames, pinnedNames, shapes);
                return;
            ...
            if (!alignCornersPack && Mathf.Abs(sx - 0.5f) < 1e-3f && Mathf.Abs(sy - 0.5f) < 1e-3f)
            {
                ...
                blobs[layer.topNames[0]] = new NcnnGraphSession.CmdTensorRef { texture = outArr, width = Mathf.Max(1, src.width / 2), height = Mathf.Max(1, src.height / 2), packs = src.packs, refs = 1, owned = true };
                if (shapes != null)
                    shapes[layer.topNames[0]] = outShape;
                owner.ConsumeCmd(cmd, blobs, remaining, layer.bottomNames, pinnedNames, shapes);
                return;
            ...
            out int outD)
        {
            ...
            if (!string.IsNullOrWhiteSpace(sizeExpr))
            {
                ...
                for (var i = 0; i < layer.bottomNames.Length; i++)
                    bottomShapes.Add(NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[i]));

                var sizes = NcnnGraphSession.EvaluateExpressionList(sizeExpr, bottomShapes, layer);
        ...
        private static bool CanUsePack4Interp(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
                && srcShape.c > 0
        ...
        private static bool CanUsePack4Interp(NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape srcShape)
        {
            return src != null
                && src.texture != null
                && srcShape.c > 0
                && (srcShape.dims == 3
                    ? srcShape.d == 1 && NcnnGraphSession.MatchesPack4TextureStorage(src, srcShape)
                    : srcShape.dims == 4 && NcnnGraphSession.MatchesPack4TextureStorage(src, srcShape));
        }
        ...
        private static bool IsCmdInterpNoop(NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.BufferShape outShape)
        {
            if (srcShape.dims == 2)
        ...
        private static NcnnGraphSession.BufferShape ResolveCmdOutputShape(NcnnGraphSession.BufferShape srcShape, int outW, int outH, int outD)
        {
            if (srcShape.dims == 1)
                return new NcnnGraphSession.BufferShape(3, outW, outH, 1, srcShape.w);
            if (srcShape.dims == 2)
                return new NcnnGraphSession.BufferShape(2, outW, srcShape.h, 1, 1);
            if (srcShape.dims == 4)
                return new NcnnGraphSession.BufferShape(4, outW, outH, outD, srcShape.c);
            return new NcnnGraphSession.BufferShape(srcShape.dims, outW, outH, srcShape.d, srcShape.c);
        }
            ...
            out int outC)
        {
            ...
            if (!string.IsNullOrWhiteSpace(sizeExpr))
            {
                var bottomShapes = new List<NcnnGraphSession.BufferShape>(layer.bottomNames.Length);
                for (var i = 0; i < layer.bottomNames.Length; i++)
                {
            ...
            out NcnnGraphSession.BufferShape shape)
        {
            ...
            {
                shape = new NcnnGraphSession.BufferShape(exactView.dims, exactView.w, exactView.h, exactView.d, exactView.c);
                return true;
            }
            ...
            out NcnnGraphSession.BufferShape shape)
        {
            ...
            if (NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, name, out _, out shape))
                return true;
