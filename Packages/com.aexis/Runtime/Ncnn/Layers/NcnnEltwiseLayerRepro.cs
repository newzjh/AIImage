...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var isTargetSftResidualLayer = string.IsNullOrEmpty(owner.CodeFormerTargetSftResidualLayer)
                ? NcnnGraphSession.CodeFormerSftResidualLayers.Contains(layer.name)
            ...
            var firstView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[0], bufferBlobs, bufferViews);
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (!NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[0], out _, out var currentShape))
            ...
            try
            {
                ...
                textureShapes[layer.topNames[0]] = new NcnnGraphSession.BufferShape(3, a.width, a.height, 1, currentShape.c);
                accum = null;
            }
            finally
            {
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
                        ...
                        do
                        {
                                                ...
                                                for (var i = 1; i < layer.bottomNames.Length; i++)
                                                {
                                                    var b = NcnnGraphSession.GetCmdTensor(blobs, layer.bottomNames[i]);
                                                ...
                                                blobs[layer.topNames[0]] = new NcnnGraphSession.CmdTensorRef { texture = accum, width = a.width, height = a.height, packs = a.packs, refs = 1, owned = true };
                                                if (shapes != null)
        ...
        private static bool CanExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            NcnnGraphSession.TensorRef currentTex = null;
            NcnnGraphSession.BufferShape currentShape = default;
            ...
            for (var i = 1; i < layer.bottomNames.Length; i++)
            {
                if (!owner.TryGetPack4Texture(layer.bottomNames[i], context.textureBlobs, context.textureShapes, context.bufferBlobs, context.bufferViews, out var otherTex, out var otherShape)
                    ...
                    || !NcnnGraphSession.MatchesPack4TextureStorage(currentTex, currentShape)
                    || !NcnnGraphSession.MatchesPack4TextureStorage(otherTex, otherShape))
                {
                    return false;
                }
            }
