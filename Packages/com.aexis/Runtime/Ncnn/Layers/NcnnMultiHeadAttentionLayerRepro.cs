    ...
    public sealed class NcnnMultiHeadAttentionLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        var mp = new NcnnGraphSession.MultiHeadAttentionPack();
                                        ...
                                        phaseSw.Restart();
                                        var qW = NcnnGraphSession.ReadClipMatAsFloat32(br, mp.embedDim * mp.qdim, 0, 0, 0, 0);
                                        var qB = br.ReadNcnnMatAsFloat32(mp.embedDim, 0, 0, 0, 1);
                                        var kW = NcnnGraphSession.ReadClipMatAsFloat32(br, mp.embedDim * mp.kdim, 0, 0, 0, 0);
                                        var kB = br.ReadNcnnMatAsFloat32(mp.embedDim, 0, 0, 0, 1);
                                        var vW = NcnnGraphSession.ReadClipMatAsFloat32(br, mp.embedDim * mp.vdim, 0, 0, 0, 0);
                                        var vB = br.ReadNcnnMatAsFloat32(mp.embedDim, 0, 0, 0, 1);
                                        var oW = NcnnGraphSession.ReadClipMatAsFloat32(br, mp.qdim * mp.embedDim, 0, 0, 0, 0);
                                        var oB = br.ReadNcnnMatAsFloat32(mp.qdim, 0, 0, 0, 1);
                                        phaseSw.Stop();
                                        readMs += phaseSw.ElapsedMilliseconds;

                                        phaseSw.Restart();
                                        mp.qW = NcnnGraphSession.NewBuffer(qW);
                                        mp.qB = NcnnGraphSession.NewBuffer(qB);
                                        mp.kW = NcnnGraphSession.NewBuffer(kW);
                                        mp.kB = NcnnGraphSession.NewBuffer(kB);
                                        mp.vW = NcnnGraphSession.NewBuffer(vW);
                                        mp.vB = NcnnGraphSession.NewBuffer(vB);
                                        mp.oW = NcnnGraphSession.NewBuffer(oW);
                                        mp.oB = NcnnGraphSession.NewBuffer(oB);
                                        phaseSw.Stop();
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
                        ...
                        do
                        {
                                                ...
                                                if (attnMaskIndex >= 0)
                                                {
                                                    ...
                                                    attnMaskView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[attnMaskIndex], bufferBlobs, bufferViews);
...
#pragma warning restore CS0618
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var qShape = NcnnGraphSession.GetCmdShape(context.shapes, context.blobs, layer.bottomNames[0]);
            if (mp.kvCache)
        ...
        private static bool TryExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            try
            {
                ...
                {
                    NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], output, plan.outputLogicalShape, plan.outputScalarStorageShape);
                    output = null;
                }
                else
                {
                    NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outputPacked, plan.outputLogicalShape, plan.outputStorageShape);
                    outputPacked = null;
                }
                owner.Consume(
            ...
            out MultiHeadAttentionRtPlan plan)
        {
            ...
            if (kShape.h != rows || vShape.h != rows)
                return false;
            NcnnGraphSession.TensorRef attnMaskTex = null;
            ...
            out MultiHeadAttentionCmdRtPlan plan)
        {
            ...
            if (kShape.h != rows || vShape.h != rows)
                return false;
            NcnnGraphSession.CmdTensorRef attnMaskTex = null;
            NcnnGraphSession.BufferShape attnMaskShape = default;
            if (attnMaskIndex >= 0)
            {
        ...
        {
            if (!NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, blobName, out texture, out shape))
                return false;
            return IsScalarOrPack4Linear2DTexture(texture, shape);
        }

        private static Vector4Int ResolveAttentionMaskTileRepeats(NcnnGraphSession.BufferShape scoresShape)
        {
        ...
        private static bool IsScalarOrPack4Linear2DTexture(
            NcnnGraphSession.TensorRef texture,
            NcnnGraphSession.BufferShape shape)
        {
            return texture != null
                ...
                && ((texture.width == shape.w && texture.height == shape.h && texture.packs == 1)
                    || NcnnGraphSession.IsPack4LinearMatTexture(texture, shape));
        }

        private static bool IsScalarOrPack4Linear2DTexture(
            NcnnGraphSession.CmdTensorRef texture,
            NcnnGraphSession.BufferShape shape)
        {
            return texture != null
                && texture.texture != null
        ...
        private static bool RequiresScalar2DMaterialization(
            ...
            NcnnGraphSession.BufferShape shape)
        {
            return NcnnGraphSession.IsStrictLinearMatTexture(texture)
                || NcnnGraphSession.IsPack4LinearMatTexture(texture, shape);
        }
            ...
            ref RenderTexture materialized)
        {
            ...
            if (NcnnGraphSession.IsPack4LinearMatTexture(source, shape))
            {
            ...
            else if (NcnnGraphSession.IsStrictLinearMatTexture(source))
            {
        ...
        {
            public readonly NcnnGraphSession.MultiHeadAttentionPack pack;
            public readonly NcnnGraphSession.TensorRef qTex;
            public readonly NcnnGraphSession.BufferShape qShape;
            public readonly NcnnGraphSession.TensorRef kTex;
            public readonly NcnnGraphSession.BufferShape kShape;
            public readonly NcnnGraphSession.TensorRef vTex;
            public readonly NcnnGraphSession.BufferShape vShape;
            public readonly bool hasAttnMask;
            public readonly NcnnGraphSession.TensorRef attnMaskTex;
            public readonly NcnnGraphSession.BufferShape attnMaskShape;
            public readonly int rows;
            public readonly int embedDim;
            public readonly int numHeads;
            public readonly int headDim;
            public readonly NcnnGraphSession.BufferShape scalarStorageShape;
            public readonly NcnnGraphSession.BufferShape headShape;
            public readonly NcnnGraphSession.BufferShape headStorageShape;
            ...
            public readonly NcnnGraphSession.BufferShape contextPermutedShape;
            public readonly NcnnGraphSession.BufferShape contextPermutedStorageShape;
            public readonly int contextPermutedSlices;
            public readonly int contextFlattenOutChannels;
            public readonly NcnnGraphSession.BufferShape outputLogicalShape;
            public readonly NcnnGraphSession.BufferShape outputScalarStorageShape;
            ...
            public MultiHeadAttentionRtPlan(
                NcnnGraphSession.MultiHeadAttentionPack pack,
                NcnnGraphSession.TensorRef qTex,
                NcnnGraphSession.BufferShape qShape,
                NcnnGraphSession.TensorRef kTex,
                NcnnGraphSession.BufferShape kShape,
                NcnnGraphSession.TensorRef vTex,
                NcnnGraphSession.BufferShape vShape,
                int rows,
                NcnnGraphSession.TensorRef attnMaskTex,
                NcnnGraphSession.BufferShape attnMaskShape,
                RenderTextureFormat sensitiveTextureFormat)
            {
                ...
                scalarStorageShape = new NcnnGraphSession.BufferShape(3, Mathf.Max(1, embedDim), Mathf.Max(1, rows), 1, 1);
                headShape = new NcnnGraphSession.BufferShape(4, Mathf.Max(1, headDim), Mathf.Max(1, rows), 1, Mathf.Max(1, numHeads));
                headStorageShape = headShape;
                headSlices = Mathf.Max(1, headShape.d * Mathf.CeilToInt(headShape.c / 4f));
                keyTransposedShape = new NcnnGraphSession.BufferShape(4, Mathf.Max(1, rows), Mathf.Max(1, headDim), 1, Mathf.Max(1, numHeads));
                ...
                scoresShape = new NcnnGraphSession.BufferShape(4, Mathf.Max(1, rows), Mathf.Max(1, rows), 1, Mathf.Max(1, numHeads));
                scoresStorageShape = scoresShape;
                scoresSlices = Mathf.Max(1, scoresShape.d * Mathf.CeilToInt(scoresShape.c / 4f));
                contextPermutedShape = new NcnnGraphSession.BufferShape(4, Mathf.Max(1, numHeads), Mathf.Max(1, rows), 1, Mathf.Max(1, headDim));
                ...
                outputLogicalShape = new NcnnGraphSession.BufferShape(2, Mathf.Max(1, pack.qdim), Mathf.Max(1, rows), 1, 1);
                outputScalarStorageShape = new NcnnGraphSession.BufferShape(3, outputLogicalShape.w, outputLogicalShape.h, 1, 1);
                outputStorageShape = NcnnGraphSession.ResolvePack4LinearMatStorageShape(outputLogicalShape);
        ...
        {
            public readonly NcnnGraphSession.MultiHeadAttentionPack pack;
            public readonly NcnnGraphSession.CmdTensorRef qTex;
            public readonly NcnnGraphSession.BufferShape qShape;
            public readonly NcnnGraphSession.CmdTensorRef kTex;
            public readonly NcnnGraphSession.BufferShape kShape;
            public readonly NcnnGraphSession.CmdTensorRef vTex;
            public readonly NcnnGraphSession.BufferShape vShape;
            public readonly bool hasAttnMask;
            public readonly NcnnGraphSession.CmdTensorRef attnMaskTex;
            public readonly NcnnGraphSession.BufferShape attnMaskShape;
            public readonly int rows;
            public readonly int embedDim;
            public readonly int numHeads;
            ...
            public MultiHeadAttentionCmdRtPlan(
                NcnnGraphSession.MultiHeadAttentionPack pack,
                NcnnGraphSession.CmdTensorRef qTex,
                NcnnGraphSession.BufferShape qShape,
                NcnnGraphSession.CmdTensorRef kTex,
                NcnnGraphSession.BufferShape kShape,
                NcnnGraphSession.CmdTensorRef vTex,
                NcnnGraphSession.BufferShape vShape,
                int rows,
                NcnnGraphSession.CmdTensorRef attnMaskTex,
                NcnnGraphSession.BufferShape attnMaskShape,
                RenderTextureFormat sensitiveTextureFormat)
            {
                this.pack = pack;
