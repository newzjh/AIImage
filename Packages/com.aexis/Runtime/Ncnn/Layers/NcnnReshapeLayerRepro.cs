...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (bufferBlobs.TryGetValue(layer.bottomNames[0], out var reshapeBuf) && reshapeBuf != null)
            {
                ...
                if (srcTensor != null)
                {
                    var outView = NcnnGraphSession.ResolveReshapeTensor(srcTensor, layer, bottomShapes);
                    ...
                    {
                        var srcShape = NcnnGraphSession.GetTextureShape(textureShapes, reshapeTex, layer.bottomNames[0]);
                        var outShape = new NcnnGraphSession.BufferShape(outView.dims, outView.w, outView.h, outView.d, outView.c);
                        var canAliasTexture = CanAliasTextureLayout(owner, srcShape, outShape);
                        if (canAliasTexture)
                        {
                            var storageShape = NcnnGraphSession.GetTextureStorageShape(reshapeTex, srcShape);
                            textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(reshapeTex, outShape, storageShape);
            ...
            else
            {
                ...
                var outShape = NcnnGraphSession.ResolveReshapeShape(srcShape, layer, bottomShapes);
                var canAliasTexture = CanAliasTextureLayout(owner, srcShape, outShape);
                ...
                if (attentionTensor != null)
                {
                    ...
                    var outView = NcnnGraphSession.ResolveReshapeTensor(scratchTensor, layer, bottomShapes);
                    ...
                    {
                        var aliasLogicalShape = new NcnnGraphSession.BufferShape(outView.dims, outView.w, outView.h, outView.d, outView.c);
                        var storageShape = NcnnGraphSession.GetTextureStorageShape(src, srcShape);
                        textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(src, aliasLogicalShape, storageShape);
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var shapeExpr = layer.GetString(6, null);
            var bottomShapes = BuildBottomShapes(owner, layer, textureBlobs, textureShapes, bufferBlobs, bufferViews, tempOwned, !string.IsNullOrWhiteSpace(shapeExpr));
            if (!NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[0], out var src, out var srcShape))
            ...
            {
                var linearStorageShape = NcnnGraphSession.GetTextureStorageShape(src, srcShape);
                textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(src, outShape, linearStorageShape);
                textureShapes[layer.topNames[0]] = outShape;
                owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
                return;
            }
            ...
            var fallbackStorageShape = NcnnGraphSession.GetTextureStorageShape(src, srcShape);
            textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(src, outShape, fallbackStorageShape);
            textureShapes[layer.topNames[0]] = outShape;
            owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
        }
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var initialOutShape = !string.IsNullOrWhiteSpace(layer.GetString(6, null))
                ? NcnnGraphSession.EvaluateReshapeShapeExpression(layer.GetString(6, null), bottomShapes, layer)
                : NcnnGraphSession.ResolveReshapeShape(srcShape, layer, bottomShapes);
            if (ShouldKeepVistaTailFeatureTextureAlias(owner, layer, srcShape, initialOutShape))
            {
                ...
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorAlias(src, initialOutShape, storageShape);
                if (shapes != null)
            ...
            if (CanAliasLinearMatTextureLayout(src, srcShape, outShape))
            {
                var linearStorageShape = NcnnGraphSession.GetCmdStorageShape(src, srcShape);
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorAlias(src, outShape, linearStorageShape);
                if (shapes != null)
            ...
            var fallbackStorageShape = NcnnGraphSession.GetCmdStorageShape(src, srcShape);
            blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorAlias(src, outShape, fallbackStorageShape);
            if (shapes != null)
                shapes[layer.topNames[0]] = outShape;
            owner.ConsumeCmd(cmd, blobs, remaining, layer.bottomNames, pinnedNames, shapes);
        }
            ...
            UnityEngine.Rendering.CommandBuffer cmd)
        {
            if (owner == null || layer == null || src == null || src.texture == null
                || !NcnnGraphSession.IsPack4LinearMatTexture(src, srcShape))
                return false;

            var outShape = NcnnGraphSession.ResolveReshapeShape(srcShape, layer, BuildCmdBottomShapes(layer, blobs, shapes));
                ...
                && checked(outShape.h * outShape.d) <= GetMaxTextureSizeSafe())
            {
                ...
                storageShape = new NcnnGraphSession.BufferShape(4, outShape.w, outputHeight, 1, outShape.c);
            }
            var outRt = owner.RentTempArray(cmd, outShape.w, outputHeight, outputSlices, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
            ...
            UnityEngine.Rendering.CommandBuffer cmd)
        {
            ...
            var outRt = owner.RentTempMat(cmd, storageShape.w, storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
            owner.Ops.ReshapePack4ToLinearMat(
                ...
                srcShape.dims,
                outRt,
                inputPack4Linear: NcnnGraphSession.IsPack4LinearMatTexture(src, srcShape));
            blobs[layer.topNames[0]] = new NcnnGraphSession.CmdTensorRef
            {
                texture = outRt,
                width = outRt.width,
        ...
        private static bool TryResolveCmdOutputShape(
            ...
            out int outPacks)
        {
            outShape = NcnnGraphSession.InferCmdShape(src);
            outW = src.width;
            outH = src.height;
            outPacks = src.packs;
            ...
            {
                outShape = NcnnGraphSession.EvaluateReshapeShapeExpression(layer.GetString(6, null), bottomShapes, layer);
                NcnnGraphSession.ResolveCmdTextureLayout(outShape, out outW, out outH, out outPacks);
                return true;
            }

            outShape = NcnnGraphSession.ResolveReshapeShape(bottomShapes[0], layer, bottomShapes);
            NcnnGraphSession.ResolveCmdTextureLayout(outShape, out outW, out outH, out outPacks);
            return true;
        }

        private static System.Collections.Generic.List<NcnnGraphSession.BufferShape> BuildBottomShapes(
            NcnnGraphSession owner,
            NcnnParamModel.Layer layer,
            ...
            bool materializeAll)
        {
            var shapes = new System.Collections.Generic.List<NcnnGraphSession.BufferShape>(layer.bottomNames.Length);
            for (var i = 0; i < layer.bottomNames.Length; i++)
            {
                ...
                {
                    shapes.Add(new NcnnGraphSession.BufferShape(view.dims, view.w, view.h, view.d, view.c));
                    continue;
                }

                if (textureBlobs.TryGetValue(name, out var tr) && tr != null && tr.texture != null)
                {
                    shapes.Add(NcnnGraphSession.GetTextureShape(textureShapes, tr, name));
                    continue;
                }
        ...
        private static bool CanAliasTextureLayout(NcnnGraphSession owner, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.BufferShape outShape)
        {
        ...
        private static bool CanAliasLinearMatTextureLayout(NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.BufferShape outShape)
        {
            return NcnnGraphSession.IsStrictLinearMatTexture(src)
        ...
        private static bool CanAliasLinearMatTextureLayout(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.BufferShape outShape)
        {
            return NcnnGraphSession.IsStrictLinearMatTexture(src)
                && CanAliasLinearMatShape(srcShape, outShape)
                && GetShapeElementCount(outShape) <= Mathf.Max(1, src.texture.width) * Mathf.Max(1, src.texture.height);
        ...
        private static bool CanAliasLinearMatShape(NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.BufferShape outShape)
        {
        ...
        private static int GetShapeElementCount(NcnnGraphSession.BufferShape shape)
        {
            return checked(
                Mathf.Max(1, shape.w)
                * Mathf.Max(1, shape.h)
        ...
        private static void ResolvePack4TextureLayout(NcnnGraphSession.BufferShape shape, out int width, out int height, out int packs)
        {
        ...
        private static NcnnGraphSession.BufferShape ResolveTextureStorageShape(NcnnGraphSession.BufferShape logicalShape)
        {
            if (logicalShape.dims <= 1)
                return new NcnnGraphSession.BufferShape(3, logicalShape.w, 1, 1, 1);
            if (logicalShape.dims == 2)
                return new NcnnGraphSession.BufferShape(3, logicalShape.w, logicalShape.h, 1, 1);
            return logicalShape;
        }

        private static bool MatchesCmdTextureStorageShape(NcnnGraphSession.CmdTensorRef tensor, NcnnGraphSession.BufferShape storageShape)
        {
        ...
        private static bool CanAliasVistaTailTextureLayout(NcnnGraphSession owner, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.BufferShape outShape)
        {
        ...
        private static bool CanExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (!NcnnGraphSession.TryGetExistingTexture(context.textureBlobs, context.textureShapes, layer.bottomNames[0], out var src, out var srcShape))
            {
            ...
            if (ShouldAllowAttentionPack4ReshapeSpecializations(owner))
            {
                ...
                var attentionOutShape = NcnnGraphSession.ResolveReshapeShape(srcShape, layer, attentionBottomShapes);
            ...
            if (ShouldAllowGenericPack4ReshapeSpecializations(owner))
            {
                ...
                if (TryResolveScalar2DToPack4ReshapeShape(layer, srcShape, out _))
                    return src != null && src.texture != null;
                if (NcnnGraphSession.IsPack4LinearMatTexture(src, srcShape) && outShape.dims >= 3 && GetShapeElementCount(srcShape) == GetShapeElementCount(outShape))
                    return src != null && src.texture != null;
            }
        ...
        private static bool TryExecuteRenderTexturePack4Linear2DToPack4Reshape(
            ...
            NcnnGraphSession.TensorRef src,
            NcnnGraphSession.BufferShape srcShape,
            System.Collections.Generic.IReadOnlyList<NcnnGraphSession.BufferShape> bottomShapes,
            Dictionary<string, NcnnGraphSession.TensorRef> textureBlobs,
            Dictionary<string, NcnnGraphSession.BufferShape> textureShapes)
        {
            if (owner == null || layer == null || src == null || src.texture == null
            ...
            var outRt = owner.RentTempArray(outShape.w, outputHeight, outputSlices, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
        ...
        private static bool TryExecuteRenderTextureLinearMat2DReshape(
            ...
            Dictionary<string, NcnnGraphSession.BufferShape> textureShapes)
        {
            if (owner == null || layer == null || src == null || src.texture == null || !NcnnGraphSession.IsStrictLinearMatTexture(src))
                return false;
            var outShape = NcnnGraphSession.ResolveReshapeShape(srcShape, layer, bottomShapes);
            ...
            var storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(outShape);
            NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, outShape, storageShape);
            return true;
        }
            ...
            Dictionary<string, NcnnGraphSession.BufferShape> textureShapes)
        {
            ...
            var inputPack4Linear = NcnnGraphSession.IsPack4LinearMatTexture(src, srcShape);
            owner.Ops.ReshapePack4ToLinearMat(
                src.texture,
                srcShape.w,
                srcShape.h,
        ...
        private static NcnnTensorBuffer TryResolveImplicitAttentionReshape(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnTensorBuffer src)
        {
            if (src == null)
                return null;
            if (!TryResolveImplicitAttentionReshapeShape(owner, layer, new NcnnGraphSession.BufferShape(src.dims, src.w, src.h, src.d, src.c), out var outShape))
                return null;

            var source = NcnnGraphSession.ReadFloatBuffer(src.buffer);
        ...
        private static bool TryResolveImplicitAttentionReshapeShape(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnGraphSession.BufferShape srcShape, out NcnnGraphSession.BufferShape outShape)
        {
            outShape = default;
            ...
            outShape = new NcnnGraphSession.BufferShape(4, headDim, srcShape.h, srcShape.c, totalChannels);
            return true;
        }
            ...
            out NcnnGraphSession.BufferShape outShape)
        {
            ...
            outShape = NcnnGraphSession.ResolveReshapeShape(srcShape, layer);
            ...
            out NcnnTensorBuffer outTensor)
        {
            ...
            if (!TryResolveWindowPartitionPattern(
                    owner,
                    layer,
                    new NcnnGraphSession.BufferShape(src.dims, src.w, src.h, src.d, src.c),
                    out var outShape,
                    out var groupsA,
                    out var groupsB,
                    out var groupsC,
            ...
            out NcnnTensorBuffer outTensor)
        {
            ...
            if (src == null || src.buffer == null)
                return false;
            if (!TryResolveAttentionContextFlattenShape(owner, layer, new NcnnGraphSession.BufferShape(src.dims, src.w, src.h, src.d, src.c), out var outShape))
                return false;
        ...
        private static NcnnGraphSession.BufferShape ResolveAttentionContextFlattenStorageShape(NcnnGraphSession.BufferShape logicalShape)
        {
            if (logicalShape.dims == 2)
                return new NcnnGraphSession.BufferShape(3, logicalShape.w, logicalShape.h, 1, 1);
        ...
        private static bool ShouldPromoteGemmPrepTexture(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnTensorBuffer outView)
        {
            if (outView == null)
                return false;
            return ShouldPromoteGemmPrepTexture(
                owner,
                layer,
                new NcnnGraphSession.BufferShape(outView.dims, outView.w, outView.h, outView.d, outView.c));
        }

        private static bool ShouldPromoteGemmPrepTexture(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnGraphSession.BufferShape outShape)
        {
        ...
        private static bool ShouldAllowAttentionPack4ReshapeSpecializations(NcnnGraphSession owner)
        {
            return owner != null && owner.EnableAttentionMatMulPack4Specializations;
        }

        private static bool ShouldAllowGenericPack4ReshapeSpecializations(NcnnGraphSession owner)
        {
            return owner != null
        ...
        private static bool ShouldAllowPack4LinearMatReshapeSpecializations(NcnnGraphSession owner)
        {
            return ShouldAllowGenericPack4ReshapeSpecializations(owner);
        }

        private static bool ShouldAllowAnyPack4ReshapeSpecializations(NcnnGraphSession owner)
        {
            return owner != null
                && (owner.EnableAttentionMatMulPack4Specializations
        ...
        private static System.Collections.Generic.List<NcnnGraphSession.BufferShape> BuildCmdBottomShapes(
            NcnnParamModel.Layer layer,
            System.Collections.Generic.Dictionary<string, NcnnGraphSession.CmdTensorRef> blobs,
            System.Collections.Generic.Dictionary<string, NcnnGraphSession.BufferShape> cmdShapes)
        {
            var bottomShapes = new System.Collections.Generic.List<NcnnGraphSession.BufferShape>(layer.bottomNames.Length);
            for (var i = 0; i < layer.bottomNames.Length; i++)
            {
                bottomShapes.Add(NcnnGraphSession.GetCmdShape(cmdShapes, blobs, layer.bottomNames[i]));
            }
            return bottomShapes;
        }
    }
