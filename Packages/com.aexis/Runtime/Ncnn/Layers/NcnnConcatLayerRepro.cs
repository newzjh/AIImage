    ...
    public sealed class NcnnConcatLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            for (var i = 0; i < layer.bottomNames.Length; i++)
            {
                ...
                partViews[i] = NcnnGraphSession.TryGetBufferView(layer.bottomNames[i], bufferBlobs, bufferViews);
            ...
            var tensorAxis = NcnnGraphSession.MapNcnnAxisToTensorAxis(firstView.dims, positiveAxis);
            ...
            {
                var v = partViews[i];
                var srcData = NcnnGraphSession.ReadFloatBuffer(partBuffers[i]);
            ...
            System.Collections.Generic.Dictionary<string, NcnnTensorBuffer> bufferViews)
        {
            ...
            var tensorAxis = NcnnGraphSession.MapNcnnAxisToTensorAxis(firstShape.dims, positiveAxis);
            ...
            var useStrictLinearConcat =
                ...
                && NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[0], out var firstTexRef, out _)
            ...
            for (var i = 0; i < layer.bottomNames.Length; i++)
            {
                ...
                if (canUseLowDimLinearConcat)
                {
                    ...
                    {
                        if (!NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[i], out var texRef, out _)
            ...
            out int packs)
        {
            ...
            if (NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, name, out var existingTexture, out shape))
            {
            ...
            if (bufferBlobs != null
                ...
                && view != null)
            {
                shape = new NcnnGraphSession.BufferShape(view.dims, view.w, view.h, view.d, view.c);
                width = view.w;
                height = view.h;
            ...
            System.Collections.Generic.Dictionary<string, NcnnTensorBuffer> bufferViews)
        {
            ...
            var parts = new NcnnGraphSession.TensorRef[layer.bottomNames.Length];
            var shapes = new NcnnGraphSession.BufferShape[layer.bottomNames.Length];
            parts[0] = firstTex;
            shapes[0] = firstShape;
            ...
            for (var i = 0; i < layer.bottomNames.Length; i++)
            {
                ...
                if (!canUseLowDimLinearConcat && !NcnnGraphSession.MatchesPack4TextureStorage(tex, shape))
                {
                    ...
                    if (useStrictLinearConcat)
                    {
                            ...
                            || shape.c != firstShape.c)
                        {
                            return Fail("strict lowdim mismatch part=" + i
                                + " strict=" + NcnnGraphSession.IsStrictLinearMatTexture(tex)
                                + " tex=" + tex.width + "x" + tex.height + "x" + tex.packs
                                + " shape=d" + shape.dims + ":" + shape.w + "x" + shape.h + "x" + shape.d + "x" + shape.c);
                        }
                    }
                    else if (NcnnGraphSession.IsStrictLinearMatTexture(tex)
                        || (tensorAxis != 0 && shape.w != firstShape.w)
                        || (tensorAxis != 1 && shape.h != firstShape.h)
                        || shape.d != firstShape.d
                        || shape.c != firstShape.c
                ...
                {
                    if (!NcnnGraphSession.MatchesPack4TextureStorage(tex, shape)
                        || shape.h != firstShape.h
                        || shape.d != firstShape.d
                        || shape.c != firstShape.c
            ...
            if (parts.Length == 1)
            {
                var storageShape = NcnnGraphSession.GetTextureStorageShape(firstTex, firstShape);
                textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(firstTex, firstShape, storageShape);
            ...
            {
                var outShape = new NcnnGraphSession.BufferShape(3, outW, firstShape.h, 1, firstShape.c);
                var outRt = owner.RentTempArray(outShape.w, outShape.h, firstTex.packs, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
                var dstOffsetX = 0;
            ...
            if (canUseLowDimLinearConcat)
            {
                var outShape = new NcnnGraphSession.BufferShape(firstShape.dims, outW, outH, firstShape.d, firstShape.c);
                if (useStrictLinearConcat)
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outMat, outShape, storageShape);
                    return true;
                }
            ...
            {
                var outShape = new NcnnGraphSession.BufferShape(3, firstShape.w, firstShape.h, 1, outC);
                ...
                for (var i = 1; i < parts.Length; i++)
                {
                    ...
                    var combinedShape = new NcnnGraphSession.BufferShape(3, current3dShape.w, current3dShape.h, 1, current3dShape.c + nextShape.c);
                ...
                NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], current3dTexture, current3dShape);
                return true;
            }
            ...
            {
                var nextShape = shapes[i];
                var combinedShape = new NcnnGraphSession.BufferShape(4, currentShape.w, currentShape.h, currentShape.d, currentShape.c + nextShape.c);
                var outSlices = currentShape.d * Mathf.Max(1, Mathf.CeilToInt(combinedShape.c / 4f));
                var outRt = owner.RentTempArray(combinedShape.w, combinedShape.h, outSlices, NcnnGraphSession.ResolveTensorTextureFormat(4));
                owner.Ops.ConcatPack4Cdhw(
                    currentTexture,
                    parts[i].texture,
                    combinedShape.w,
            ...
            NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], currentTexture, currentShape);
            return true;
        }
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var parts = new NcnnGraphSession.CmdTensorRef[layer.bottomNames.Length];
            var partShapes = new NcnnGraphSession.BufferShape[layer.bottomNames.Length];
            for (var i = 0; i < layer.bottomNames.Length; i++)
            {
                parts[i] = NcnnGraphSession.GetCmdTensor(blobs, layer.bottomNames[i]);
                partShapes[i] = NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[i]);
            }
            ...
            for (var i = 0; i < partShapes.Length; i++)
            {
                ...
                var supportsPack4Storage = canUseLowDimStrictLinearConcat
                    ...
                    : shape.dims <= 2
                        ? !NcnnGraphSession.IsStrictLinearMatTexture(parts[i])
                            && shape.d == 1
                ...
                if (!supportsPack4Storage
                    || (!canUseLowDimLinearConcat && !NcnnGraphSession.MatchesPack4TextureStorage(parts[i], shape))
                    || (canUseLowDimStrictLinearConcat && !NcnnGraphSession.IsStrictLinearMatTexture(parts[i])))
                {
                    canUseExactPack4 = false;
                }
            }

            var outShape = new NcnnGraphSession.BufferShape(firstShape.dims, outW, outH, outD, outC);
            if (layer.bottomNames.Length == 1)
            {
                var storageShape = NcnnGraphSession.GetCmdStorageShape(parts[0], partShapes[0]);
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorAlias(parts[0], outShape, storageShape);
                if (shapes != null)
            ...
            if (canUseExactPack4)
            {
                if (canUseLowDimLinearConcat)
                {
                    ...
                    else
                    {
                        ...
                        blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outArr, outShape, outShape, owned: true);
                    }
                    if (shapes != null)
                        shapes[layer.topNames[0]] = outShape;
                    owner.ConsumeCmd(cmd, blobs, remaining, layer.bottomNames, pinnedNames, shapes);
                ...
                if (outShape.dims == 3)
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 1; i < parts.Length; i++)
                        {
                            ...
                            current = new NcnnGraphSession.CmdTensorRef
                            {
                                texture = outArr,
                                width = outShape.w,
                                height = outShape.h,
                            ...
                            currentShape = new NcnnGraphSession.BufferShape(3, outShape.w, outShape.h, 1, combinedChannels);
                            currentOwned = true;
                        }
                ...
                else
                {
                    ...
                    for (var i = 1; i < parts.Length; i++)
                    {
                        ...
                        currentShape = new NcnnGraphSession.BufferShape(4, outShape.w, outShape.h, outShape.d, combinedChannels);
                        currentOwned = true;
                    }
        ...
        private static int LowDimTextureStorageWidth(NcnnGraphSession.BufferShape logicalShape)
        {
            return Mathf.Max(1, logicalShape.w);
        }

        private static int LowDimTextureStorageHeight(NcnnGraphSession.BufferShape logicalShape)
        {
            return logicalShape.dims >= 2 ? Mathf.Max(1, logicalShape.h) : 1;
        }

        private static bool IsStrictLinearLowDimTexture(NcnnGraphSession.TensorRef tensor, NcnnGraphSession.BufferShape logicalShape)
        {
            if (tensor == null || tensor.texture == null || !NcnnGraphSession.IsStrictLinearMatTexture(tensor))
                return false;
        ...
        private static bool IsStrictLinearLowDimTexture(NcnnGraphSession.CmdTensorRef tensor, NcnnGraphSession.BufferShape logicalShape)
        {
            if (tensor == null || tensor.texture == null || !NcnnGraphSession.IsStrictLinearMatTexture(tensor))
                return false;
