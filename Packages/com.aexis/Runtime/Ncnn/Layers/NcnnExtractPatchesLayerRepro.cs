...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnExtractPatchesLayerRepro : NcnnBaseLayerRepro
    {
        ...
        {
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not NcnnGraphSession.UnfoldPack pack)
            ...
            try
            {
                output = owner.RentTempArray(outStorageShape.w, outStorageShape.h, outDepth, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
                if (foldedD)
                {
                ...
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], output, outShape, outStorageShape);
                output = null;
            }
            finally
            {
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            try
            {
                output = owner.RentTempArray(context.commandBuffer, outStorageShape.w, outStorageShape.h, outDepth, NcnnGraphSession.ResolveTensorTextureFormat(outShape.dims));
                if (foldedD)
                {
                    owner.Ops.ExtractPatchesFoldDPack4(
                ...
                context.blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(output, outShape, outStorageShape, owned: true, blobName: layer.topNames[0]);
        ...
        private static bool CanUsePack4(NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.UnfoldPack pack)
        {
            return src != null
                   && src.texture != null
        ...
        private static bool CanUsePack4(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.UnfoldPack pack)
        {
            return src != null
                   && src.texture != null
                   && (srcShape.dims == 3 || srcShape.dims == 4)
        ...
        private static void ResolveOutputShape(NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.UnfoldPack pack, string layerName, out NcnnGraphSession.BufferShape outShape)
        {
            ...
            outShape = srcShape.dims == 4
                ...
                ? new NcnnGraphSession.BufferShape(4, outW, outH, Mathf.Max(1, srcShape.d) * pack.kernelW * pack.kernelH, Mathf.Max(1, srcShape.c))
                : new NcnnGraphSession.BufferShape(3, outW, outH, 1, outC);
        }

        private static int ResolveSameOrExplicitPadLeft(NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.UnfoldPack pack)
        {
            var kernelExtentW = pack.dilationW * (pack.kernelW - 1) + 1;
        ...
        private static int ResolveSameOrExplicitPadTop(NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.UnfoldPack pack)
        {
            var kernelExtentH = pack.dilationH * (pack.kernelH - 1) + 1;
        ...
        private static string DescribeUnsupported(string prefix, NcnnParamModel.Layer layer, NcnnGraphSession.TensorRef src, NcnnGraphSession.BufferShape shape, NcnnGraphSession.UnfoldPack pack)
        {
            return prefix
        ...
        private static string DescribeUnsupported(string prefix, NcnnParamModel.Layer layer, NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape shape, NcnnGraphSession.UnfoldPack pack)
        {
            return prefix
                   + " requires dims=3 pack4 texture input"
                   + " | layer=" + (layer != null ? layer.name : string.Empty)
        ...
        private static NcnnGraphSession.BufferShape ResolveFoldDStorageShape(NcnnGraphSession.BufferShape logicalShape)
        {
