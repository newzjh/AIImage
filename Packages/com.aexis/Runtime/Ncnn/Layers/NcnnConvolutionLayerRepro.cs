    ...
    public sealed class NcnnConvolutionLayerRepro : NcnnBaseLayerRepro
    {
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        var pack = new NcnnGraphSession.ConvPack();
                                        ...
                                        pack.activationSlope = NcnnGraphSession.ParseLeakySlope(layer);
                                        ...
                                        if (owner.UsesInt4WeightOnlyForLayer(layer))
                                            NcnnGraphSession.UploadInt4WeightOnlyConvWeights(pack, w, b, layer.name);
                                        else if (owner.UsesInt8WeightOnlyForLayer(layer))
                                            NcnnGraphSession.UploadInt8WeightOnlyConvWeights(pack, w, b, layer.name);
                                        else
                                            NcnnGraphSession.UploadRawConvWeights(pack, w, b);
                                        phaseSw.Stop();
                                        ...
                                        {
                                            phaseSw.Restart();
                                            var w4 = NcnnGraphSession.PackWeightsToO4I4K(w, pack.outC, pack.inC, pack.kernelW, pack.outPacks, pack.inPacks);
                                            ...
                                            if (owner.UsesFp16WeightStorage)
                                                pack.packedWeight4Fp16 = NcnnGraphSession.NewFp16Vector4Buffer(w4, "NcnnGraphSession.ConvPackedWeight4Fp16:" + layer.name);

                                            if (NcnnGraphSession.EnableWinograd23
                                                && pack.kernelW == 3
                                                && pack.kernelH == 3
                                                && pack.strideW == 1
                                                && pack.strideH == 1
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var outW = NcnnGraphSession.ComputeConvOut(srcTensor.w, conv.kernelW, conv.dilationW, conv.strideW, conv.padLeft, conv.padRight);
            var outH = NcnnGraphSession.ComputeConvOut(srcTensor.h, conv.kernelH, conv.dilationH, conv.strideH, conv.padTop, conv.padBottom);
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            {
                srcShape = NcnnGraphSession.GetTextureShape(textureShapes, src, layer.bottomNames[0]);
            }
            else
            {
                ...
                var convInputView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[0], bufferBlobs, bufferViews);
                ...
                srcShape = new NcnnGraphSession.BufferShape(3, convInputView.w, convInputView.h, 1, convInputView.c);
                src = NcnnGraphSession.CreateTextureRef(tempInputTex, srcShape, srcShape, owned: false);
            }

            if (srcShape.dims == 2 && conv.inC == 1)
            {
                var promotedShape = new NcnnGraphSession.BufferShape(3, srcShape.w, srcShape.h, 1, 1);
                if (!NcnnGraphSession.IsStrictLinearMatTexture(src))
                    src = NcnnGraphSession.CreateTextureRef(src.texture, promotedShape, promotedShape, owned: false, sharedTextureOwner: src, blobName: layer.bottomNames[0]);
            ...
            else if (!NcnnGraphSession.MatchesPack4TextureStorage(src, srcShape) || src.packs != conv.inPacks)
            {
                throw new InvalidOperationException(
                    ...
                    + " | storage=" + NcnnGraphSession.GetTextureStorageShape(src, srcShape));
            }

            var outWTex = NcnnGraphSession.ComputeConvOut(srcShape.w, conv.kernelW, conv.dilationW, conv.strideW, conv.padLeft, conv.padRight);
            var outHTex = NcnnGraphSession.ComputeConvOut(srcShape.h, conv.kernelH, conv.dilationH, conv.strideH, conv.padTop, conv.padBottom);
            ...
            {
                if (NcnnGraphSession.EnableWinograd23 && conv.useWinograd23)
            ...
            var outShape = new NcnnGraphSession.BufferShape(3, outWTex, outHTex, 1, conv.outC);
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            try
            {
                if (srcShape.dims == 2 && conv.inC == 1)
                {
                    var promotedShape = new NcnnGraphSession.BufferShape(3, srcShape.w, srcShape.h, 1, 1);
                    if (!NcnnGraphSession.IsStrictLinearMatTexture(src))
                        src = NcnnGraphSession.CreateCmdTensorRef(src.texture, promotedShape, promotedShape, owned: false, sharedTextureOwner: src, blobName: layer.bottomNames[0]);
                    srcShape = promotedShape;
                }
                    ...
                    && srcShape.c == conv.inC)
                {
                    ...
                    src = NcnnGraphSession.CreateCmdTensorRef(tempInput, srcShape, srcShape, owned: false);
                }
        ...
        private static bool CanUsePack4CmdPath(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.ConvPack conv)
        {
            return src != null
                && src.texture != null
                && conv != null
                ...
                && srcShape.d == 1
                && srcShape.c == conv.inC
                && NcnnGraphSession.MatchesPack4TextureStorage(src, srcShape)
                && src.packs == conv.inPacks;
        }

        private static bool SupportsCommandBufferPack4(NcnnGraphSession.ConvPack conv, out string reason)
        {
            reason = null;
        ...
        private static string BuildUnsupportedMessage(NcnnParamModel.Layer layer, NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape logicalShape, string reason)
        {
        ...
        private static bool CanExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context, NcnnGraphSession.ConvPack conv)
        {
        ...
        private static NcnnGraphSession.BufferShape ResolveCmdOutputShape(NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.ConvPack conv)
        {
            var outW = Mathf.Max(1, NcnnGraphSession.ComputeConvOut(srcShape.w, conv.kernelW, conv.dilationW, conv.strideW, conv.padLeft, conv.padRight));
            var outH = Mathf.Max(1, NcnnGraphSession.ComputeConvOut(srcShape.h, conv.kernelH, conv.dilationH, conv.strideH, conv.padTop, conv.padBottom));
            return new NcnnGraphSession.BufferShape(3, outW, outH, 1, Mathf.Max(1, conv.outC));
        }
    }
}
