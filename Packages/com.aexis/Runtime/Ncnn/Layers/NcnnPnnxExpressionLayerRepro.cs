...
namespace Aexis.Ncnn
{
    ...
    public sealed class NcnnPnnxExpressionLayerRepro : NcnnBaseLayerRepro
    {
        ...
        private static NcnnGraphSession.BufferShape ResolveOutputShape(float[] values)
        {
            return new NcnnGraphSession.BufferShape(1, Mathf.Max(1, values?.Length ?? 0), 1, 1, 1);
        }

        private static NcnnGraphSession.BufferShape ResolveStorageShape(float[] values)
        {
            return new NcnnGraphSession.BufferShape(3, Mathf.Max(1, values?.Length ?? 0), 1, 1, 1);
        }
