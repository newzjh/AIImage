...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (withScalar != 0)
            {
                ...
                var aView = NcnnGraphSession.TryGetBufferView(layer.bottomNames[0], bufferBlobs, bufferViews);
                if (aBuf == null)
            ...
            var isTargetSftAddLayer = string.IsNullOrEmpty(owner.CodeFormerTargetSftAddLayer)
                ? NcnnGraphSession.CodeFormerSftAddLayers.Contains(layer.name)
            ...
            var isTargetSftMulLayer = string.IsNullOrEmpty(owner.CodeFormerTargetSftMulLayer)
                ? NcnnGraphSession.CodeFormerSftMulLayers.Contains(layer.name)
            ...
            var broadcast = NcnnGraphSession.ResolveBinaryBroadcast(aViewFallback, bViewFallback, aBufFallback.count, bBufFallback.count, layer.name);
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var withScalar = layer.GetInt(1, 0);
            var scalarB = layer.GetFloat(2, 0f);
            var scalarTextureFormat = NcnnGraphSession.ResolveTensorTextureFormat(2);

            if (withScalar != 0)
            {
                if (owner.ShouldForceCurrentLayerBufferPath()
                    || !NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[0], out var aTexScalar, out var aTexShapeScalar)
                ...
                var aTexStorageShapeScalar = NcnnGraphSession.GetTextureStorageShape(aTexScalar, aTexShapeScalar);
                if (NcnnGraphSession.IsPack4LinearMatTexture(aTexScalar, aTexShapeScalar))
                {
                    var storageShape = NcnnGraphSession.GetTextureStorageShape(aTexScalar, aTexShapeScalar);
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outRt, aTexShapeScalar, storageShape);
                    owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
                    return;
                }

                if (NcnnGraphSession.IsStrictLinearMatTexture(aTexScalar))
                {
                    var outLinear = owner.RentTempMat(aTexStorageShapeScalar.w, aTexStorageShapeScalar.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                    owner.Ops.BinaryOpScalarLinearMat(aTexScalar.texture, scalarB, opType, outLinear);
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outLinear, aTexShapeScalar, aTexStorageShapeScalar);
                    owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
                    return;
                }
                ...
                var outRtScalar = owner.RentTempArray(
                    ...
                    scalarDepth,
                    NcnnGraphSession.ResolveTensorTextureFormat(aTexShapeScalar.dims));
                owner.Ops.BinaryOpScalarPack4(aScalarInput, scalarB, scalarDepth, opType, outRtScalar);
                NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outRtScalar, aTexShapeScalar, aTexStorageShapeScalar);
            ...
            NcnnGraphSession.TensorRef aTex = null;
            NcnnGraphSession.TensorRef bTex = null;
            NcnnGraphSession.BufferShape aTexShape = default;
            NcnnGraphSession.BufferShape bTexShape = default;
                    ...
                    out var pack4LinearStorageShape))
            {
                ...
                try
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, pack4LinearAShape, pack4LinearStorageShape);
                    finalTexture = null;
                }
                finally
                {
                    ...
                    out var mixedPack4IsA))
            {
                ...
                try
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, mixedOutShape, mixedStorageShape);
                    finalTexture = null;
                }
                finally
                {
            ...
            if (canUseTextureBinary && NcnnGraphSession.CanUseExactPack4BinaryPath(aTex, aTexShape, bTex, bTexShape))
            {
                ...
                try
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, aTexShape);
                    finalTexture = null;
                }
                finally
                {
                ...
                out var scalar2DBShape))
            {
                ...
                try
                {
                            ...
                            out var scalar2DStorageShape))
                    {
                        ...
                        if (owner.CodeFormerSftAddScale != 1f && opType == 0 && isTargetSftAddLayer)
                        {
                            scaledBTexture = owner.RentTempMat(scalar2DStorageShape.w, scalar2DStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                            owner.Ops.BinaryOpScalarLinearMat(rhsLinear, owner.CodeFormerSftAddScale, 2, scaledBTexture);
                            rhsLinear = scaledBTexture;
                        }

                        finalTexture = owner.RentTempMat(scalar2DStorageShape.w, scalar2DStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                        if (isCodeFormerSftMul && owner.CodeFormerBypassSftMul)
                        {
                        ...
                        else
                        {
                            ...
                            if (isCodeFormerSftMul && owner.CodeFormerSftMulScale != 1f)
                            {
                                var scaledOutTexture = owner.RentTempMat(scalar2DStorageShape.w, scalar2DStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                        ...
                        NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, scalar2DAShape, scalar2DStorageShape);
                        finalTexture = null;
                        owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
                        return;
                    }
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, scalar2DAShape);
                    finalTexture = null;
                }
                finally
                {
                    ...
                    out var scalarBroadcastStorageShape))
            {
                ...
                try
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, scalarBroadcastOutShape, scalarBroadcastStorageShape);
                    finalTexture = null;
                }
                finally
                {
                    ...
                    out var channelVectorTextureIsA))
            {
                ...
                try
                {
                    ...
                    owner.Ops.BinaryOpPack4ChannelVectorTex(
                        ...
                        finalTexture);
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, vectorTextureTexShape);
                    finalTexture = null;
                }
                finally
                {
                    ...
                    out var channelVectorIsA))
            {
                ...
                try
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, vectorTextureShape);
                    finalTexture = null;
                }
                finally
                {
                    ...
                    out var scalarIsA))
            {
                ...
                try
                {
                    var scalarStorageShape = NcnnGraphSession.GetTextureStorageShape(scalarTexture, scalarTextureShape);
                    if (NcnnGraphSession.IsStrictLinearMatTexture(scalarTexture))
                    {
                        finalTexture = owner.RentTempMat(scalarStorageShape.w, scalarStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                        owner.Ops.BinaryOpLinearMatFixedInputScalar(scalarTexture.texture, scalarBuffer, opType, scalarIsA, finalTexture);
                        NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, scalarTextureShape, scalarStorageShape);
                    }
                    else
                    {
                        ...
                        NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, scalarTextureShape);
                    }
                    finalTexture = null;
                }
                finally
                ...
                && TryResolvePack4SpatialBroadcast(pack4ATex, pack4AShape, pack4BTex, pack4BShape, out var broadcastMode, out var outShape, out var outWidth, out var outHeight, out var outPacks))
            {
                ...
                try
                {
                    ...
                    NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], finalTexture, outShape);
                    finalTexture = null;
                }
                finally
                {
            ...
            owner.DebugLog?.Invoke(
                ...
                + " | exact=" + (canUseTextureBinary && NcnnGraphSession.CanUseExactPack4BinaryPath(pack4ATex, pack4AShape, pack4BTex, pack4BShape)).ToString()
        ...
        private static string DescribeTensorRef(NcnnGraphSession.TensorRef tensor, NcnnGraphSession.BufferShape shape)
        {
            ...
            return tensor.width.ToString(CultureInfo.InvariantCulture)
                ...
                + " storage=" + NcnnGraphSession.GetTextureStorageShape(tensor, shape).ToString();
        }

        private static bool TryResolvePack4SpatialBroadcast(
            NcnnGraphSession.TensorRef aTex,
            NcnnGraphSession.BufferShape aShape,
            NcnnGraphSession.TensorRef bTex,
            NcnnGraphSession.BufferShape bShape,
            out int broadcastMode,
            out NcnnGraphSession.BufferShape outShape,
            out int outWidth,
            out int outHeight,
            out int outPacks)
        {
            ...
            if (matchingSpatialExtent
                ...
                && NcnnGraphSession.MatchesPack4TextureStorage(aTex, aShape)
                && NcnnGraphSession.MatchesPack4TextureStorage(bTex, bShape))
            {
                broadcastMode = 3;
                outWidth = bTex.width;
                outHeight = bTex.height;
                outPacks = bTex.packs;
                outShape = new NcnnGraphSession.BufferShape(bShape.dims, bShape.w, bShape.h, bShape.d, bShape.c);
                return true;
            }
                ...
                && NcnnGraphSession.MatchesPack4TextureStorage(bTex, bShape))
            {
                ...
                outPacks = aTex.packs;
                outShape = new NcnnGraphSession.BufferShape(aShape.dims, aShape.w, aShape.h, aShape.d, aShape.c);
                return true;
            }
        ...
        private static bool IsPack4WidthVector(NcnnGraphSession.TensorRef texture, NcnnGraphSession.BufferShape shape)
        {
            return texture != null
                ...
                && texture.height == 1
                && texture.packs == 1
                && NcnnGraphSession.MatchesPack4TextureStorage(texture, shape);
        }

        private static bool IsPack4WidthVectorTarget(NcnnGraphSession.TensorRef texture, NcnnGraphSession.BufferShape shape, int width)
        {
            return texture != null
                && texture.texture != null
                && (shape.dims == 3 || shape.dims == 4)
        ...
        private static bool TryResolvePack4ChannelVectorBroadcast(
            ...
            out NcnnGraphSession.TensorRef texture,
            out NcnnGraphSession.BufferShape textureShape,
        ...
        private static bool TryResolvePack4TextureChannelVectorBroadcast(
            ...
            out NcnnGraphSession.TensorRef texture,
            out NcnnGraphSession.BufferShape textureShape,
            out NcnnGraphSession.TensorRef channelVector,
            out NcnnGraphSession.BufferShape channelVectorShape,
            out bool channelVectorIsA)
        {
            texture = null;
            textureShape = default;
            ...
            if (TryGetExistingChannelVectorTexture(textureBlobs, textureShapes, bName, out var bVector, out var bLogicalShape, out _)
                && NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, aName, out var aTex, out var aShape)
            ...
            if (TryGetExistingChannelVectorTexture(textureBlobs, textureShapes, aName, out var aVector, out var aLogicalShape, out _)
                && NcnnGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, bName, out var bTex, out var bShape)
        ...
        private static bool TryResolveExactScalar2DTextureBinaryPath(
            ...
            string aName,
            string bName,
            out NcnnGraphSession.TensorRef aTex,
            out NcnnGraphSession.BufferShape aShape,
            out NcnnGraphSession.TensorRef bTex,
            out NcnnGraphSession.BufferShape bShape)
        {
            aTex = null;
            aShape = default;
            bTex = null;
        ...
        private static bool TryResolveExactPack4LinearBinaryPath(
            ...
            out NcnnGraphSession.TensorRef bTex,
            out NcnnGraphSession.BufferShape bShape,
            out NcnnGraphSession.BufferShape storageShape)
        {
            aTex = null;
            aShape = default;
            bTex = null;
            ...
            if (!NcnnGraphSession.TryGetExistingTextureContract(textureBlobs, textureShapes, aName, out aTex, out var aContract)
                || !NcnnGraphSession.TryGetExistingTextureContract(textureBlobs, textureShapes, bName, out bTex, out var bContract))
                return false;
            ...
            if (aShape.w != bShape.w
                ...
                || aShape.d != bShape.d
                || aShape.c != bShape.c
                || !NcnnGraphSession.IsPack4LinearMatTexture(aTex, aShape)
                || !NcnnGraphSession.IsPack4LinearMatTexture(bTex, bShape))
                return false;

            var aStorage = aContract.StorageShape;
            var bStorage = bContract.StorageShape;
            if (!NcnnGraphSession.BufferShapeEquals(aStorage, bStorage))
                return false;
        ...
        private static bool TryResolvePack4LinearMixedBinaryPath(
            ...
            string aName,
            string bName,
            out NcnnGraphSession.TensorRef pack4Linear,
            out NcnnGraphSession.TensorRef linear,
            out NcnnGraphSession.BufferShape logicalShape,
            out NcnnGraphSession.BufferShape storageShape,
            out bool pack4IsA)
        {
            pack4Linear = null;
            ...
            if (!NcnnGraphSession.TryGetExistingTextureContract(textureBlobs, textureShapes, aName, out var aTex, out var aContract)
                || !NcnnGraphSession.TryGetExistingTextureContract(textureBlobs, textureShapes, bName, out var bTex, out var bContract))
                return false;
            ...
            var aLinear = NcnnGraphSession.IsStrictLinearMatTexture(aTex);
            var bLinear = NcnnGraphSession.IsStrictLinearMatTexture(bTex);
            if (aPack4 == bPack4)
                return false;
        ...
        private static bool TryResolvePack4LinearMixedBinaryPath(
            ...
            string aName,
            string bName,
            out NcnnGraphSession.CmdTensorRef pack4Linear,
            out NcnnGraphSession.CmdTensorRef linear,
            out NcnnGraphSession.BufferShape logicalShape,
            out NcnnGraphSession.BufferShape storageShape,
            out bool pack4IsA)
        {
            ...
            if (!NcnnGraphSession.TryGetExistingCmdTextureContract(blobs, shapes, aName, out var aTex, out var aContract)
                || !NcnnGraphSession.TryGetExistingCmdTextureContract(blobs, shapes, bName, out var bTex, out var bContract))
                return false;
        ...
        private static bool IsPackedLogical2DTexture(NcnnGraphSession.TensorRef tensor, NcnnGraphSession.BufferShape shape)
        {
            if (NcnnGraphSession.IsPack4LinearMatTexture(tensor, shape))
                return true;
            if (tensor == null
                || tensor.texture == null
                || shape.dims != 2
        ...
        private static bool IsPackedLogical2DTexture(NcnnGraphSession.CmdTensorRef tensor, NcnnGraphSession.BufferShape shape)
        {
            if (NcnnGraphSession.IsPack4LinearMatTexture(tensor, shape))
                return true;
            if (tensor == null
        ...
        {
            if (!NcnnGraphSession.TryGetExistingTextureContract(textureBlobs, textureShapes, name, out texture, out var contract))
            {
                shape = default;
                return false;
            }
        ...
        private static bool CanUseScalar2DTexturePath(NcnnGraphSession.TensorRef texture, NcnnGraphSession.BufferShape shape)
        {
            return texture != null
                && texture.texture != null
                && shape.dims == 2
        ...
        private static bool CanUseExactLinearMatBinaryPath(
            NcnnGraphSession.TensorRef a,
            NcnnGraphSession.BufferShape aShape,
            NcnnGraphSession.TensorRef b,
            NcnnGraphSession.BufferShape bShape,
            out NcnnGraphSession.BufferShape storageShape)
        {
            storageShape = default;
            if (!CanUseScalar2DTexturePath(a, aShape)
                ...
                || !NcnnGraphSession.IsStrictLinearMatTexture(a)
                || !NcnnGraphSession.IsStrictLinearMatTexture(b))
                return false;
            if (aShape.w != bShape.w
                || aShape.h != bShape.h
                || aShape.d != bShape.d
            ...
            var aStorage = NcnnGraphSession.GetTextureStorageShape(a, aShape);
            var bStorage = NcnnGraphSession.GetTextureStorageShape(b, bShape);
        ...
        private static bool CanUseScalarLikeTexturePath(NcnnGraphSession.TensorRef texture, NcnnGraphSession.BufferShape shape)
        {
            if (texture == null || texture.texture == null)
                return false;
            if (NcnnGraphSession.IsPack4LinearMatTexture(texture, shape))
                return true;
            if (texture.packs == 1
            ...
            if (NcnnGraphSession.IsStrictLinearMatTexture(texture))
                return false;
        ...
        private static bool CanUseChannelVectorTexturePath(NcnnGraphSession.TensorRef texture, NcnnGraphSession.BufferShape logicalShape, NcnnGraphSession.BufferShape storageShape)
        {
        ...
        private static bool CanUseScalar2DTexturePath(NcnnGraphSession.CmdTensorRef texture, NcnnGraphSession.BufferShape shape)
        {
            return texture != null
                && texture.texture != null
                && shape.dims == 2
        ...
        private static bool CanUseExactLinearMatBinaryPath(
            NcnnGraphSession.CmdTensorRef a,
            NcnnGraphSession.BufferShape aShape,
            NcnnGraphSession.CmdTensorRef b,
            NcnnGraphSession.BufferShape bShape,
            out NcnnGraphSession.BufferShape storageShape)
        {
            storageShape = default;
            ...
            var aStorage = NcnnGraphSession.GetCmdStorageShape(a, aShape);
            var bStorage = NcnnGraphSession.GetCmdStorageShape(b, bShape);
            if (aStorage.dims != bStorage.dims
                || aStorage.w != bStorage.w
                || aStorage.h != bStorage.h
                || a.width != aStorage.w
        ...
        private static bool CanUseScalarLikeTexturePath(NcnnGraphSession.CmdTensorRef texture, NcnnGraphSession.BufferShape shape)
        {
            if (texture == null || texture.texture == null)
                return false;
            if (NcnnGraphSession.IsPack4LinearMatTexture(texture, shape))
        ...
        private static RenderTexture MaterializeScalarLikeTexture(
            NcnnGraphSession owner,
            NcnnGraphSession.TensorRef source,
            ...
            ref RenderTexture materialized)
        {
            ...
            if (!NcnnGraphSession.IsStrictLinearMatTexture(source))
        ...
        private static ComputeTexture MaterializeScalarLikeTexture(
            NcnnGraphSession owner,
            CommandBuffer cmd,
            NcnnGraphSession.CmdTensorRef source,
            NcnnGraphSession.BufferShape shape,
            RenderTextureFormat format,
        ...
        private static void ReturnTempUnique(NcnnGraphSession owner, ref RenderTexture texture, RenderTexture alias0, RenderTexture alias1)
        {
        ...
        private static void ReturnTempUnique(NcnnGraphSession owner, CommandBuffer cmd, ref ComputeTexture texture, ComputeTexture alias0, ComputeTexture alias1)
        {
            if (texture == null)
                return;
        ...
        private static bool CanUseChannelVectorTexturePath(NcnnGraphSession.CmdTensorRef texture, NcnnGraphSession.BufferShape logicalShape, NcnnGraphSession.BufferShape storageShape)
        {
            if (texture == null || texture.texture == null || texture.packs != 1)
                return false;
            if (logicalShape.dims != 1 && logicalShape.dims != 2)
        ...
        private static bool MatchesChannelVectorWidth(NcnnGraphSession.BufferShape logicalShape, int channels)
        {
        ...
        private static bool TryResolveScalarSingleBroadcastCmdBinaryPath(
            NcnnGraphSession.CmdTensorRef aTex,
            NcnnGraphSession.BufferShape aShape,
            NcnnGraphSession.CmdTensorRef bTex,
            NcnnGraphSession.BufferShape bShape,
            out NcnnGraphSession.CmdTensorRef outATex,
            out NcnnGraphSession.CmdTensorRef outBTex,
            out int broadcastMode,
            out NcnnGraphSession.BufferShape outShape,
            out NcnnGraphSession.BufferShape storageShape)
        {
        ...
        private static bool TryResolveCmdPack4TextureChannelVectorBroadcast(
            ...
            NcnnGraphSession.CmdTensorRef b,
            NcnnGraphSession.BufferShape bShape,
            out NcnnGraphSession.CmdTensorRef texture,
            out NcnnGraphSession.BufferShape textureShape,
            out NcnnGraphSession.CmdTensorRef channelVector,
            out NcnnGraphSession.BufferShape channelVectorShape,
            out bool channelVectorIsA)
        {
            texture = null;
            ...
            if ((aShape.dims == 3 || aShape.dims == 4)
                && NcnnGraphSession.MatchesPack4TextureStorage(a, aShape)
                && CanUseChannelVectorTexturePath(b, bShape, NcnnGraphSession.GetCmdStorageShape(b, bShape))
            ...
            if ((bShape.dims == 3 || bShape.dims == 4)
                && NcnnGraphSession.MatchesPack4TextureStorage(b, bShape)
                && CanUseChannelVectorTexturePath(a, aShape, NcnnGraphSession.GetCmdStorageShape(a, aShape))
            ...
            out NcnnGraphSession.BufferShape storageShape)
        {
            ...
            static int GetRows(NcnnGraphSession.BufferShape shape) => shape.dims == 1 ? 1 : shape.h;
            static int GetCols(NcnnGraphSession.BufferShape shape) => shape.w;
            static int ElementCount(NcnnGraphSession.BufferShape shape) => Mathf.Max(1, shape.w) * Mathf.Max(1, shape.h) * Mathf.Max(1, shape.d) * Mathf.Max(1, shape.c);
            static bool IsScalarSingle(NcnnGraphSession.BufferShape shape) => shape.dims >= 1 && shape.dims <= 2 && ElementCount(shape) == 1;
            static bool CanUseScalarSingleOutput(NcnnGraphSession.BufferShape shape) => shape.dims == 1 || shape.dims == 2;
            static NcnnGraphSession.BufferShape ScalarSingleStorage(NcnnGraphSession.BufferShape shape)
            {
                var width = Mathf.Max(1, shape.w);
                var height = shape.dims == 1 ? 1 : Mathf.Max(1, shape.h);
                return new NcnnGraphSession.BufferShape(3, width, height, 1, 1);
            }
            static bool IsRowVector(NcnnGraphSession.BufferShape shape) => shape.dims == 2 && shape.h == 1 && shape.w > 0;
            static bool IsColumnVector(NcnnGraphSession.BufferShape shape) => shape.dims == 2 && shape.w == 1 && shape.h > 0;
            ...
            if (aShape.dims == 2 && bShape.dims == 1 && aRows == bCols && aCols > 1)
            {
                broadcastMode = 8;
                outShape = aShape;
                storageShape = new NcnnGraphSession.BufferShape(3, aShape.w, aShape.h, 1, 1);
                return true;
            }
            ...
            if (aShape.dims == 1 && bShape.dims == 2 && aCols == bRows && bCols > 1)
            {
                broadcastMode = 7;
                outShape = bShape;
                storageShape = new NcnnGraphSession.BufferShape(3, bShape.w, bShape.h, 1, 1);
                return true;
            }
        ...
        private static bool CanExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            if (owner.ShouldForceCurrentLayerBufferPath())
                return false;
            ...
            if (withScalar != 0)
            {
                return !owner.ShouldForceCurrentLayerBufferPath()
                    && NcnnGraphSession.TryGetExistingTexture(context.textureBlobs, context.textureShapes, layer.bottomNames[0], out var scalarTex, out var scalarTexShape)
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
                        ...
                        do
                        {
                                                ...
                                                var isCodeFormerSftMul = opType == 2 && isTargetSftMulLayer;
                                                var scalarTextureFormat = NcnnGraphSession.ResolveTensorTextureFormat(2);
                                                var a = NcnnGraphSession.GetCmdTensor(blobs, layer.bottomNames[0]);
                                                var aShape = NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[0]);
                                                if (withScalar != 0)
                                                {
                                                    if (!CanUseScalarLikeTexturePath(a, aShape))
                                                        break;
                                                    var aStorageShape = NcnnGraphSession.GetCmdStorageShape(a, aShape);
                                                    if (NcnnGraphSession.IsPack4LinearMatTexture(a, aShape))
                                                    {
                                                        ...
                                                        blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(scalarPack4Out, aShape, aStorageShape, owned: true);
                                                        if (shapes != null)
                                                    ...
                                                    if (NcnnGraphSession.IsStrictLinearMatTexture(a))
                                                    {
                                                        var outLinear = owner.RentTempMat(cmd, aStorageShape.w, aStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                                                        owner.Ops.BinaryOpScalarLinearMat(cmd, a.texture, scalarB, opType, outLinear);
                                                        blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outLinear, aShape, aStorageShape, owned: true);
                                                        if (shapes != null)
                                                            shapes[layer.topNames[0]] = aShape;
                                                        owner.ConsumeCmd(cmd, blobs, remaining, layer.bottomNames, pinnedNames, shapes);
                                                        continue;
                                                    ...
                                                    var outArr = owner.RentTempArray(cmd, aScalarInput.width, aScalarInput.height, scalarDepth, NcnnGraphSession.ResolveTensorTextureFormat(aShape.dims));
                                                    owner.Ops.BinaryOpScalarPack4(cmd, aScalarInput, scalarB, scalarDepth, opType, outArr);
                                                    blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outArr, aShape, aStorageShape, owned: true);
                                                    if (shapes != null)
                                                        shapes[layer.topNames[0]] = aShape;
                                                    if (aScalarMaterialized != null)
                                                ...
                                                {
                                                    var b = NcnnGraphSession.GetCmdTensor(blobs, layer.bottomNames[1]);
                                                    var bShape = NcnnGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[1]);
                                                    if (NcnnGraphSession.IsPack4LinearMatTexture(a, aShape)
                                                        && NcnnGraphSession.IsPack4LinearMatTexture(b, bShape)
                                                        && aShape.w == bShape.w
                                                        && aShape.h == bShape.h
                                                        && aShape.d == bShape.d
                                                        && aShape.c == bShape.c
                                                        && NcnnGraphSession.BufferShapeEquals(NcnnGraphSession.GetCmdStorageShape(a, aShape), NcnnGraphSession.GetCmdStorageShape(b, bShape)))
                                                    {
                                                        var storageShape = NcnnGraphSession.GetCmdStorageShape(a, aShape);
                                                        ...
                                                        blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outArr, aShape, storageShape, owned: true);
                                                        if (shapes != null)
                                                            shapes[layer.topNames[0]] = aShape;
                                                    }
                                                        ...
                                                        out var mixedPack4IsA))
                                                    {
                                                        ...
                                                        blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(outArr, mixedOutShape, mixedStorageShape, owned: true);
                                                        if (shapes != null)
                                                        ...
                                                        && a.packs == b.packs)
                                                    {
                                                        ...
                                                        try
                                                        {
                                                            if (CanUseExactLinearMatBinaryPath(a, aShape, b, bShape, out var scalar2DStorageShape))
                                                            {
                                                                ...
                                                                if (owner.CodeFormerSftAddScale != 1f && opType == 0 && isTargetSftAddLayer)
                                                                {
                                                                    scaledB = owner.RentTempMat(cmd, scalar2DStorageShape.w, scalar2DStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                                                                    owner.Ops.BinaryOpScalarLinearMat(cmd, rhsLinear, owner.CodeFormerSftAddScale, 2, scaledB);
                                                                    rhsLinear = scaledB;
                                                                }

                                                                finalOut = owner.RentTempMat(cmd, scalar2DStorageShape.w, scalar2DStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                                                                if (isCodeFormerSftMul && owner.CodeFormerBypassSftMul)
                                                                {
                                                                ...
                                                                else
                                                                {
                                                                    ...
                                                                    if (isCodeFormerSftMul && owner.CodeFormerSftMulScale != 1f)
                                                                    {
                                                                        var scaledOut = owner.RentTempMat(cmd, scalar2DStorageShape.w, scalar2DStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                                                                ...
                                                                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(finalOut, aShape, scalar2DStorageShape, owned: true);
                                                                if (shapes != null)
                                                                    shapes[layer.topNames[0]] = aShape;
                                                                finalOut = null;
                                                            }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        owner.DebugLog?.Invoke(
                                                            ...
                                                            + " | layer=" + layer.name
                                                            + " | aStorage=" + NcnnGraphSession.GetCmdStorageShape(a, aShape)
                                                            + " | bStorage=" + NcnnGraphSession.GetCmdStorageShape(b, bShape)
                                                            + " | aLogical=" + aShape
                                                            + " | bLogical=" + bShape
                                                        ...
                                                        NcnnGraphSession.ResolveCmdTextureLayout(fallbackShape, out var width, out var height, out var packs);
        ...
        private static bool CanUseExactCmdBinaryPath(NcnnGraphSession.CmdTensorRef a, NcnnGraphSession.BufferShape aShape, NcnnGraphSession.CmdTensorRef b, NcnnGraphSession.BufferShape bShape)
        {
            return a != null
                && b != null
                && a.texture != null
        ...
        private static bool IsCmdPack4WidthVector(NcnnGraphSession.CmdTensorRef texture, NcnnGraphSession.BufferShape shape)
        {
            return texture != null
                && texture.texture != null
                && (shape.dims == 3 || shape.dims == 4)
        ...
        private static bool IsCmdPack4WidthVectorTarget(NcnnGraphSession.CmdTensorRef texture, NcnnGraphSession.BufferShape shape, int width)
        {
            return texture != null
                && texture.texture != null
                && (shape.dims == 3 || shape.dims == 4)
        ...
        private static NcnnGraphSession.BufferShape ResolveCmdOutputShape(NcnnGraphSession.BufferShape aShape, NcnnGraphSession.BufferShape bShape)
        {
