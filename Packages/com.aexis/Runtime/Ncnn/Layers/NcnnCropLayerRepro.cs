...
#pragma warning restore CS0618
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            {
                var storageShape = NcnnGraphSession.GetTextureStorageShape(srcTex, texShape);
                textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(srcTex, texShape, storageShape);
            ...
            else
            {
                ...
                NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outRt, new NcnnGraphSession.BufferShape(3, roi.outw, roi.outh, 1, roi.outc));
            }
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            if (CanUsePack4Crop(src, srcShape, roi))
            {
                ...
                else
                {
                    ...
                    shapes[layer.topNames[0]] = new NcnnGraphSession.BufferShape(3, roi.outw, roi.outh, 1, roi.outc);
                }
            }
            else
            {
                ...
                owner.CopyCmdTensor(
                    ...
                    outPacks,
                    shapes,
                    new NcnnGraphSession.BufferShape(3, roi.outw, roi.outh, 1, roi.outc));
            }

            owner.ConsumeCmd(cmd, blobs, remaining, layer.bottomNames, pinnedNames, shapes);
        }

        private static NcnnGraphSession.CropRoi ResolveCropRoi(
            ...
            List<IDisposable> tempOwned)
        {
            ...
            {
                var bottomShapes = new List<NcnnGraphSession.BufferShape>(layer.bottomNames.Length) { srcShape };
                ...
                return NcnnGraphSession.ResolveCropRoi(srcShape, layer, bottomShapes);
            }
            ...
            if (layer.bottomNames.Length > 1)
            {
                if (layer.GetInt(0, 0) == -233)
                {
                    ...
                    var refData = NcnnGraphSession.ReadFloatBuffer(refBuf);
                    var paramData = new int[refData.Length];
                    for (var i = 0; i < refData.Length; i++)
                        paramData[i] = Mathf.RoundToInt(refData[i]);
                    return NcnnGraphSession.ResolveCropRoi(srcShape, paramData, layer);
                }
                ...
                return NcnnGraphSession.ResolveCropRoi(srcShape, referenceShape, layer);
            }

            var singleBottomShapes = new List<NcnnGraphSession.BufferShape>(1) { srcShape };
            return NcnnGraphSession.ResolveCropRoi(srcShape, layer, singleBottomShapes);
        }
        ...
        private static bool TryResolveCmdCropRoi(
            ...
            NcnnGraphSession.BufferShape srcShape,
            out NcnnGraphSession.CropRoi roi)
        {
            roi = default;
            var hasSliceArrays = (layer.GetInts(-23309, null)?.Length ?? 0) > 0 && (layer.GetInts(-23310, null)?.Length ?? 0) > 0;
            var hasSliceExpr = !string.IsNullOrWhiteSpace(layer.GetString(19, null)) && !string.IsNullOrWhiteSpace(layer.GetString(20, null));
            if (hasSliceArrays || hasSliceExpr)
            {
                ...
                {
                    if (NcnnGraphSession.TryGetCmdShape(shapes, blobs, layer.bottomNames[i], out var secondaryShape))
                        bottomShapes.Add(secondaryShape);
                }
                roi = NcnnGraphSession.ResolveCropRoi(srcShape, layer, bottomShapes);
                return true;
            }
            ...
            if (layer.bottomNames.Length > 1)
            {
                ...
                if (!NcnnGraphSession.TryGetCmdShape(shapes, blobs, layer.bottomNames[1], out var referenceShape))
                    return false;
                roi = NcnnGraphSession.ResolveCropRoi(srcShape, referenceShape, layer);
                return true;
            }

            roi = NcnnGraphSession.ResolveCropRoi(srcShape, layer, new[] { srcShape });
            return true;
        }

        private static bool CanUsePack4Crop(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.CropRoi roi)
        {
            return srcTex != null
                && srcTex.texture != null
                && srcShape.dims == 3
        ...
        private static bool CanUsePack4Crop(NcnnGraphSession.CmdTensorRef src, NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.CropRoi roi)
        {
            return src != null
                && src.texture != null
                && srcShape.dims == 3
        ...
        private static bool IsIdentityCrop(NcnnGraphSession.BufferShape srcShape, NcnnGraphSession.CropRoi roi)
        {
            ...
            out NcnnGraphSession.BufferShape shape)
        {
            ...
            {
                shape = NcnnGraphSession.GetShapeOf(view);
                return true;
            }

            if (textureBlobs.TryGetValue(name, out var tr) && tr != null && tr.texture != null)
            {
                shape = NcnnGraphSession.GetTextureShape(textureShapes, tr, name);
                return true;
            }
        ...
        private static bool TryAliasExistingBuffer(
            ...
            string topName,
            Dictionary<string, ComputeBuffer> bufferBlobs,
            Dictionary<string, NcnnGraphSession.BufferRef> bufferRefs,
            Dictionary<string, NcnnTensorBuffer> bufferViews,
