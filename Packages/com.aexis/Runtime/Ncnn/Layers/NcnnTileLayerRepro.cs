...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var tensorAxis = NcnnGraphSession.MapNcnnAxisToTensorAxis(srcView.dims, axis);
            var outW = srcView.w;
            var outH = srcView.h;
            var outD = srcView.d;
            var outC = srcView.c;
        ...
        public override void ExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (spec.isPassthrough)
            {
                ...
                {
                    var storageShape = NcnnGraphSession.GetTextureStorageShape(passthroughTex, srcShape);
                    textureBlobs[layer.topNames[0]] = NcnnGraphSession.CreateTextureAlias(passthroughTex, spec.outShape, storageShape);
            ...
            var srcStorageShape = NcnnGraphSession.GetTextureStorageShape(src, srcShape);
            if (NcnnGraphSession.IsStrictLinearMatTexture(src))
            {
                var outStorageShape = NcnnGraphSession.ResolveLinearMatStorageShape(spec.outShape);
                var output = owner.RentTempMat(outStorageShape.w, outStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.TileLinearMat(src.texture, srcShape, srcStorageShape, spec.outShape, outStorageShape, spec.repeats, output);
                NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, spec.outShape, outStorageShape);
            }
            else
            {
                ...
                var output = owner.RentTempArray(
                    ...
                    NcnnGraphSession.ResolveTensorTextureFormat(spec.outShape.dims));
                owner.Ops.TilePack4(src.texture, srcShape, spec.outShape, spec.repeats, output);
                NcnnGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, spec.outShape, spec.outShape);
            }
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            if (spec.isPassthrough)
            {
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorAlias(src, spec.outShape, sourceContract.StorageShape);
                if (shapes != null)
            ...
            {
                var outStorageShape = NcnnGraphSession.ResolveLinearMatStorageShape(spec.outShape);
                var output = owner.RentTempMat(cmd, outStorageShape.w, outStorageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.TileLinearMat(cmd, src.texture, srcShape, srcStorageShape, spec.outShape, outStorageShape, spec.repeats, output);
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(output, spec.outShape, outStorageShape, owned: true, blobName: layer.topNames[0]);
            }
            else
            {
                if (src.texture.dimension != TextureDimension.Tex2DArray || !IsDirectPack4StorageView(srcShape, srcStorageShape))
                ...
                var outputStorageShape = spec.outShape.dims <= 2
                    ? new NcnnGraphSession.BufferShape(3, spec.outShape.w, spec.outShape.dims == 2 ? spec.outShape.h : 1, 1, 1)
                    : spec.outShape;
                blobs[layer.topNames[0]] = NcnnGraphSession.CreateCmdTensorRef(output, spec.outShape, outputStorageShape, owned: true, blobName: layer.topNames[0]);
            }
        ...
        {
            public readonly NcnnGraphSession.BufferShape outShape;
            public readonly Vector4Int repeats;
            public readonly bool isPassthrough;

            public TileSpec(NcnnGraphSession.BufferShape outShape, Vector4Int repeats, bool isPassthrough)
            {
            ...
            out NcnnGraphSession.BufferShape shape)
        {
            ...
            if (context.textureBlobs != null
                ...
                && tex.texture != null)
            {
                shape = NcnnGraphSession.GetTextureShape(context.textureShapes, tex, layer.bottomNames[0]);
                return shape.dims >= 1 && shape.dims <= 4;
            }

            var view = NcnnGraphSession.TryGetBufferView(layer.bottomNames[0], context.bufferBlobs, context.bufferViews);
            if (view != null)
            {
        ...
        private static bool TryResolveTileSpec(NcnnParamModel.Layer layer, NcnnGraphSession.BufferShape srcShape, out TileSpec spec)
        {
            spec = default;
            ...
            if (outDims == 1)
                outShape = new NcnnGraphSession.BufferShape(1, outW, 1, 1, 1);
            else if (outDims == 2)
                outShape = new NcnnGraphSession.BufferShape(2, outW, outH, 1, 1);
            else if (outDims == 3)
                outShape = new NcnnGraphSession.BufferShape(3, outW, outH, 1, outC);
            else
                outShape = new NcnnGraphSession.BufferShape(4, outW, outH, outD, outC);

            var sameShape = NcnnGraphSession.BufferShapeEquals(srcShape, outShape);
        ...
        private static bool IsDirectPack4StorageView(NcnnGraphSession.BufferShape logicalShape, NcnnGraphSession.BufferShape storageShape)
        {
            if (NcnnGraphSession.BufferShapeEquals(logicalShape, storageShape))
                return true;
