...
#pragma warning restore CS0618
        ...
        private static bool TryExecuteCommandBufferTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            if (srcTex == null
                ...
                || srcShape.dims != 3
                || srcShape.d != 1
                || !NcnnGraphSession.MatchesPack4TextureStorage(srcTex, srcShape))
            {
                return false;
            }
            ...
            {
                var outShape = new NcnnGraphSession.BufferShape(3, 1, srcShape.h, 1, srcShape.c);
                var outStorage = new NcnnGraphSession.BufferShape(3, 1, srcShape.h, 1, srcShape.c);
                ...
                context.blobs[outTop] = NcnnGraphSession.CreateCmdTensorRef(outRt, outShape, outStorage, owned: true, blobName: outTop);
                ...
                && CanUseScalarTextureReductionOp(op))
            {
                var outShape = keepDims
                    ? new NcnnGraphSession.BufferShape(3, srcShape.w, srcShape.h, 1, 1)
                    : new NcnnGraphSession.BufferShape(2, srcShape.w, srcShape.h, 1, 1);
                var outStorage = new NcnnGraphSession.BufferShape(3, srcShape.w, srcShape.h, 1, 1);
            ...
            {
                context.blobs[outTop] = new NcnnGraphSession.CmdTensorRef
                {
                    texture = pooled,
                    width = 1,
                    height = 1,
                    ...
                    logicalShape = new NcnnGraphSession.BufferShape(3, 1, 1, 1, srcShape.c),
                    hasStorageShape = true,
                    storageShape = new NcnnGraphSession.BufferShape(3, 1, 1, 1, srcShape.c)
                };
                context.shapes[outTop] = new NcnnGraphSession.BufferShape(3, 1, 1, 1, srcShape.c);
                owner.ConsumeCmd(cmd, context.blobs, context.remaining, layer.bottomNames, context.pinnedNames, context.shapes);
                return true;
            }
            ...
            if (reduceSpatialOnly && !keepDims)
            {
                var logicalShape = new NcnnGraphSession.BufferShape(1, srcShape.c, 1, 1, 1);
                var storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(logicalShape);
        ...
        private static bool TryExecuteRenderTexturePath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            if (srcShape.dims != 3 || srcShape.d != 1 || !NcnnGraphSession.MatchesPack4TextureStorage(srcTex, srcShape))
                return false;
            if (TryResolveWidthReductionAxes(srcShape, reduceAll, keepDims, axes, out var reduceWidthOnly)
                && reduceWidthOnly
                && CanUseScalarTextureReductionOp(op))
            {
                ...
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, outTop, outRt, outShape, outStorage);
                owner.Consume(
                    context.textureBlobs,
                    context.bufferBlobs,
                    context.bufferRefs,
            ...
            if (reduceSpatialOnly && keepDims)
            {
                ...
                var outShape = new NcnnGraphSession.BufferShape(3, 1, 1, 1, srcShape.c);
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, outTop, outRt, outShape, outShape);
                owner.Consume(
                    context.textureBlobs,
                    context.bufferBlobs,
                    context.bufferRefs,
            ...
            if (reduceSpatialOnly && !keepDims)
            {
                ...
                owner.ReturnTempArray(pooledRt);
                NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, outTop, outRt, logicalShape, storageShape);
                owner.Consume(
                    context.textureBlobs,
                    context.bufferBlobs,
                    context.bufferRefs,
            ...
            NcnnLayerBufferContext context)
        {
            ...
            {
                storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(outShape);
                outRt = owner.RentTempMat(storageShape.w, storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
            ...
            else
            {
                ...
                storageShape = new NcnnGraphSession.BufferShape(3, outRt.width, outRt.height, 1, 1);
            }
            NcnnGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], outRt, outShape, storageShape);
            owner.Consume(
                context.textureBlobs,
                context.bufferBlobs,
                context.bufferRefs,
        ...
        private static bool TryExecuteCommandBufferScalar2DTexturePath(
            NcnnGraphSession owner,
            NcnnParamModel.Layer layer,
            NcnnGraphSession.CmdTensorRef srcTex,
            ...
            NcnnLayerCommandBufferContext context)
        {
            ...
            {
                storageShape = NcnnGraphSession.ResolveLinearMatStorageShape(outShape);
                outRt = owner.RentTempMat(context.commandBuffer, storageShape.w, storageShape.h, NcnnGraphSession.ResolveLinearMatTextureFormat());
            ...
            owner.DebugLog?.Invoke(
                ...
                + " | layer=" + layer.name
                + " | strictLinear=" + (NcnnGraphSession.IsStrictLinearMatTexture(srcTex) ? "1" : "0")
        ...
        private static bool CanUseScalar2DTexturePath(NcnnGraphSession.TensorRef srcTex, NcnnGraphSession.BufferShape srcShape)
        {
            return srcTex != null
                && srcTex.texture != null
                && srcShape.dims == 2
        ...
        private static bool CanUseScalar2DTexturePath(NcnnGraphSession.CmdTensorRef srcTex, NcnnGraphSession.BufferShape srcShape)
        {
            return srcTex != null
                && srcTex.texture != null
                && srcShape.dims == 2
            ...
            out NcnnGraphSession.BufferShape outShape)
        {
            ...
            {
                outShape = keepDims
                    ? new NcnnGraphSession.BufferShape(2, 1, 1, 1, 1)
                    : new NcnnGraphSession.BufferShape(1, 1, 1, 1, 1);
            ...
            if (axis == 1)
            {
                reduceAlongWidth = true;
                outShape = keepDims
                    ? new NcnnGraphSession.BufferShape(2, 1, srcShape.h, 1, 1)
                    : new NcnnGraphSession.BufferShape(1, srcShape.h, 1, 1, 1);
                return true;
            }
            ...
            if (axis == 0)
            {
                reduceAlongWidth = false;
                outShape = keepDims
                    ? new NcnnGraphSession.BufferShape(2, srcShape.w, 1, 1, 1)
                    : new NcnnGraphSession.BufferShape(1, srcShape.w, 1, 1, 1);
                return true;
            }
