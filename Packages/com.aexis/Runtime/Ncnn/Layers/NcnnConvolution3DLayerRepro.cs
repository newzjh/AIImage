...
{
    public sealed class NcnnConvolution3DLayerRepro : NcnnBaseLayerRepro
    {
        public NcnnConvolution3DLayerRepro()
            : base(NcnnLayerTypes.Convolution3D, supportsBufferPath: true, supportsCommandBufferPath: true)
        {
        }
...
#pragma warning restore CS0618
        ...
        public override NcnnGraphSession.LayerLoadMetrics LoadLayer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnBinReader br)
        {
            ...
            NcnnGpuResourceTracker.RegisterBuffer(pack.packedWeight4, w4.Length, sizeof(float) * 4, "NcnnGraphSession.Conv3DPackedWeight4:" + layer.name);
            pack.packedBias4 = new ComputeBuffer(b4.Length, sizeof(float) * 4, ComputeBufferType.Structured);
            NcnnGpuResourceTracker.RegisterBuffer(pack.packedBias4, b4.Length, sizeof(float) * 4, "NcnnGraphSession.Conv3DPackedBias4:" + layer.name);
        ...
        public override void ExecuteComputeBufferPath(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerBufferContext context)
        {
            ...
            var outW = NcnnGraphSession.ComputeConvOut(srcView.w, conv.kernelW, conv.dilationW, conv.strideW, conv.padLeft, conv.padRight);
            var outH = NcnnGraphSession.ComputeConvOut(srcView.h, conv.kernelH, conv.dilationH, conv.strideH, conv.padTop, conv.padBottom);
            var outD = NcnnGraphSession.ComputeConvOut(srcView.d, conv.kernelD, conv.dilationD, conv.strideD, conv.padFront, conv.padBehind);
...
#pragma warning restore CS0618
            ...
            var outW = NcnnGraphSession.ComputeConvOut(srcShape.w, conv.kernelW, conv.dilationW, conv.strideW, conv.padLeft, conv.padRight);
            var outH = NcnnGraphSession.ComputeConvOut(srcShape.h, conv.kernelH, conv.dilationH, conv.strideH, conv.padTop, conv.padBottom);
            var outD = NcnnGraphSession.ComputeConvOut(srcShape.d, conv.kernelD, conv.dilationD, conv.strideD, conv.padFront, conv.padBehind);
            var outRt = owner.RentTempArray(outW, outH, outD * conv.outPacks, NcnnGraphSession.ResolveTensorTextureFormat(4));
            ...
            NcnnGraphSession.SetTextureBlob(
                ...
                layer.topNames[0],
                outRt,
                new NcnnGraphSession.BufferShape(4, outW, outH, outD, conv.outC));
            owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
        }
        ...
        public override void ExecuteCommandBuffer(NcnnGraphSession owner, NcnnParamModel.Layer layer, NcnnLayerCommandBufferContext context)
        {
            ...
            var outH = NcnnGraphSession.ComputeConvOut(srcShape.h, conv.kernelH, conv.dilationH, conv.strideH, conv.padTop, conv.padBottom);
            var outD = NcnnGraphSession.ComputeConvOut(srcShape.d, conv.kernelD, conv.dilationD, conv.strideD, conv.padFront, conv.padBehind);
            var outShape = new NcnnGraphSession.BufferShape(4, outW, outH, outD, conv.outC);
            var outRt = owner.RentTempArray(cmd, outW, outH, outD * conv.outPacks, NcnnGraphSession.ResolveTensorTextureFormat(4));
            owner.Ops.Conv3dPack4CDHW(
                cmd,
                src.texture,
                srcShape.w,
