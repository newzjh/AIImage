...
float4 AexisDeepFillReadFeature(int x, int y, int pack)
{
...
float AexisDeepFillReadMaskDownsampled(int x, int y)
{
...
float AexisDeepFillPatchValid(int sourceX, int sourceY)
{
    [unroll]
    for (int ky = -1; ky <= 1; ky++)
    {
        ...
        {
            if (AexisDeepFillReadMaskDownsampled(sourceX + kx, sourceY + ky) > 0.5)
                return 0.0;
        }
    }
    return 1.0;
...
void AexisDeepFillV2PatchStats_Impl(uint3 id)
{
    ...
    for (int ky = -1; ky <= 1; ky++)
    {
        ...
        for (int kx = -1; kx <= 1; kx++)
        {
            ...
            {
                float4 value = AexisDeepFillReadFeature(fx, fy, pack);
    ...
    _DeepFillPatchStatsOutArr[int3(sourceX, sourceY, 0)] = float4(norm, AexisDeepFillPatchValid(sourceX, sourceY), 0.0, 0.0);
}

float AexisDeepFillPatchScore(int targetX, int targetY, int sourceIndex)
{
    ...
    for (int ky = -1; ky <= 1; ky++)
    {
        ...
        for (int kx = -1; kx <= 1; kx++)
        {
            ...
            for (int pack = 0; pack < _DeepFillFeaturePacks; pack++)
            {
                float4 targetValue = AexisDeepFillReadFeature(targetFeatureX, targetFeatureY, pack);
                float4 sourceValue = AexisDeepFillReadFeature(sourceFeatureX, sourceFeatureY, pack);
...
void AexisDeepFillV2Scores_Impl(uint3 id)
{
    ...
    float4 score;
    score.x = AexisDeepFillPatchScore(targetX, targetY, sourceBase + 0);
    score.y = AexisDeepFillPatchScore(targetX, targetY, sourceBase + 1);
    score.z = AexisDeepFillPatchScore(targetX, targetY, sourceBase + 2);
    score.w = AexisDeepFillPatchScore(targetX, targetY, sourceBase + 3);
    _DeepFillScoresOutArr[int3(targetX, targetY, sourcePack)] = score;
}

float4 AexisDeepFillSourceValid4(int sourcePack)
{
    int sourceBase = sourcePack * 4;
    float4 valid = 0.0;
    [unroll]
    for (int lane = 0; lane < 4; lane++)
    {
        ...
        int sourceX = sourceIndex % _DeepFillMatchW;
        int sourceY = sourceIndex / _DeepFillMatchW;
        AexisWriteLane(valid, lane, _DeepFillPatchStatsArr[int3(sourceX, sourceY, 0)].y);
    }
    return valid;
}

void AexisDeepFillV2Softmax_Impl(uint3 groupId, uint3 groupThreadId)
{
    ...
    for (int sourcePack = lane; sourcePack < _DeepFillSourcePacks; sourcePack += 128)
    {
        ...
        weight *= AexisDeepFillSourceValid4(sourcePack);
        _DeepFillWeightsOutArr[int3(targetX, targetY, sourcePack)] = weight;
    }
}

void AexisDeepFillV2Reconstruct_Impl(uint3 id)
{
