...
{
    public sealed class AexisTensorBuffer : IDisposable
    {
        public int dims { get; }
        public int w { get; }
        public int h { get; }
        ...
        public AexisTensorBuffer(int w, int h, int c)
        {
            ...
            AexisGpuResourceTracker.RegisterBuffer(buffer, count, sizeof(float), "AexisTensorBuffer(3d)");
        }

        public AexisTensorBuffer(int w)
        {
            if (w <= 0) throw new ArgumentOutOfRangeException(nameof(w));
            dims = 1;
            this.w = w;
            ...
            AexisGpuResourceTracker.RegisterBuffer(buffer, w, sizeof(float), "AexisTensorBuffer(1d)");
        }

        public AexisTensorBuffer(int w, int h)
        {
            if (w <= 0) throw new ArgumentOutOfRangeException(nameof(w));
            if (h <= 0) throw new ArgumentOutOfRangeException(nameof(h));
            dims = 2;
            ...
            AexisGpuResourceTracker.RegisterBuffer(buffer, checked(w * h), sizeof(float), "AexisTensorBuffer(2d)");
        }

        public AexisTensorBuffer(int w, int h, int d, int c)
        {
            if (w <= 0) throw new ArgumentOutOfRangeException(nameof(w));
            if (h <= 0) throw new ArgumentOutOfRangeException(nameof(h));
            ...
            AexisGpuResourceTracker.RegisterBuffer(buffer, checked(w * h * d * c), sizeof(float), "AexisTensorBuffer(4d)");
        }

        internal AexisTensorBuffer(ComputeBuffer existing, int dims, int w, int h, int d, int c, bool ownsBuffer, Action<ComputeBuffer> releaseBuffer = null)
        {
        ...
        public AexisTensorBuffer View(int dims, int w, int h, int d, int c)
        {
            ...
            return new AexisTensorBuffer(buffer, dims, w, h, d, c, false);
        }

        public AexisTensorBuffer Reshape(int newDims, int newW, int newH = 1, int newD = 1, int newC = 1)
        {
            return View(newDims, newW, newH, newD, newC);
        }

        public AexisTensorBuffer ExpandDims(int axis)
        {
            var inDims = dims;
        ...
        public void Dispose()
        {
            ...
            AexisGpuResourceTracker.ReleaseBuffer(buffer, "AexisTensorBuffer.Dispose");
