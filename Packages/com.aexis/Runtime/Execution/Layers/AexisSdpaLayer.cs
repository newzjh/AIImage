...
namespace Aexis.Execution
{
    ...
    public sealed class AexisSdpaLayer : AexisBaseLayer
    {
        public AexisSdpaLayer()
            : base(AexisLayerTypes.SDPA, supportsBufferPath: true, supportsCommandBufferPath: true)
        {
        }
        ...
        {
            owner._extraPacks[layer.name] = new AexisGraphSession.SdpaPack
            {
        ...
        public override void ExecuteComputeBufferPath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not AexisGraphSession.SdpaPack sp)
            ...
            var queryView = AexisGraphSession.TryGetBufferView(layer.bottomNames[0], bufferBlobs, bufferViews);
            var keyView = AexisGraphSession.TryGetBufferView(layer.bottomNames[1], bufferBlobs, bufferViews);
            var valueView = AexisGraphSession.TryGetBufferView(layer.bottomNames[2], bufferBlobs, bufferViews);
            ...
            if (sp.attnMask)
            {
                ...
                attnMaskView = AexisGraphSession.TryGetBufferView(layer.bottomNames[3], bufferBlobs, bufferViews);
            ...
            if (sp.kvCache)
            {
                ...
                var pastKeyView = AexisGraphSession.TryGetBufferView(layer.bottomNames[pastKeyBottom], bufferBlobs, bufferViews);
                var pastValueView = AexisGraphSession.TryGetBufferView(layer.bottomNames[pastValueBottom], bufferBlobs, bufferViews);
...
#pragma warning restore CS0618
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            var queryShape = AexisGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[0]);
            var keyShape = AexisGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[1]);
            var valueShape = AexisGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[2]);
            ...
            {
                var pastKeyBottom = sp.attnMask ? 4 : 3;
                var pastKeyShape = AexisGraphSession.GetCmdShape(shapes, blobs, layer.bottomNames[pastKeyBottom]);
                dstSeqLen += pastKeyShape.h;
            }

            var outShape = new AexisGraphSession.BufferShape(3, valueShape.w, queryShape.h, 1, queryShape.c);
            ...
            ref RenderTexture valueCache)
        {
            ...
            if (inPlaceKvCache)
            {
                var inPlaceKeyStorageShape = AexisGraphSession.GetTextureStorageShape(plan.pastKey, plan.keyShape);
                var inPlaceValueStorageShape = AexisGraphSession.GetTextureStorageShape(plan.pastValue, plan.valueShape);
                context.textureBlobs[layer.topNames[1]] = AexisGraphSession.CreateTextureAlias(plan.pastKey, plan.keyShape, inPlaceKeyStorageShape);
                context.textureBlobs[layer.topNames[2]] = AexisGraphSession.CreateTextureAlias(plan.pastValue, plan.valueShape, inPlaceValueStorageShape);
            ...
            {
                var keyCapacityStorageShape = new AexisGraphSession.BufferShape(3, plan.keyShape.w, keyCache.height, plan.keyShape.d, plan.keyShape.c);
                var valueCapacityStorageShape = new AexisGraphSession.BufferShape(3, plan.valueShape.w, valueCache.height, plan.valueShape.d, plan.valueShape.c);
                AexisGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[1], keyCache, plan.keyShape, keyCapacityStorageShape);
                AexisGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[2], valueCache, plan.valueShape, valueCapacityStorageShape);
                keyCache = null;
                valueCache = null;
                return;
            }

            var keyStorageShape = AexisGraphSession.GetTextureStorageShape(plan.key, plan.keyCurrentShape);
            var valueStorageShape = AexisGraphSession.GetTextureStorageShape(plan.value, plan.valueCurrentShape);
            context.textureBlobs[layer.topNames[1]] = AexisGraphSession.CreateTextureAlias(plan.key, plan.keyShape, keyStorageShape);
            context.textureBlobs[layer.topNames[2]] = AexisGraphSession.CreateTextureAlias(plan.value, plan.valueShape, valueStorageShape);
            context.textureShapes[layer.topNames[1]] = plan.keyShape;
            context.textureShapes[layer.topNames[2]] = plan.valueShape;
        }

        private static bool CanAppendInPlace(AexisGraphSession owner, in SdpaRtPlan plan)
        {
            ...
            ref ComputeTexture valueCache)
        {
            ...
            if (plan.hasPastCache)
            {
                var keyCapacityStorageShape = new AexisGraphSession.BufferShape(3, plan.keyShape.w, keyCache.height, plan.keyShape.d, plan.keyShape.c);
                var valueCapacityStorageShape = new AexisGraphSession.BufferShape(3, plan.valueShape.w, valueCache.height, plan.valueShape.d, plan.valueShape.c);
                context.blobs[layer.topNames[1]] = AexisGraphSession.CreateCmdTensorRef(keyCache, plan.keyShape, keyCapacityStorageShape, owned: true);
                context.blobs[layer.topNames[2]] = AexisGraphSession.CreateCmdTensorRef(valueCache, plan.valueShape, valueCapacityStorageShape, owned: true);
                keyCache = null;
                valueCache = null;
            }
            else
            {
                var keyStorageShape = AexisGraphSession.GetCmdStorageShape(plan.key, plan.keyCurrentShape);
                var valueStorageShape = AexisGraphSession.GetCmdStorageShape(plan.value, plan.valueCurrentShape);
                context.blobs[layer.topNames[1]] = AexisGraphSession.CreateCmdTensorAlias(plan.key, plan.keyShape, keyStorageShape);
                context.blobs[layer.topNames[2]] = AexisGraphSession.CreateCmdTensorAlias(plan.value, plan.valueShape, valueStorageShape);
            }
            ...
            out SdpaRtPlan plan)
        {
            ...
            AexisGraphSession.TensorRef pastKey = null;
            AexisGraphSession.TensorRef pastValue = null;
            AexisGraphSession.BufferShape pastKeyShape = default;
            AexisGraphSession.BufferShape pastValueShape = default;
            ...
            if (common.sp.kvCache)
            {
                ...
                var hasPastKey = layer.bottomNames.Length > pastKeyBottom
                    && AexisGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[pastKeyBottom], out pastKey, out pastKeyShape);
                var hasPastValue = layer.bottomNames.Length > pastValueBottom
                    && AexisGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, layer.bottomNames[pastValueBottom], out pastValue, out pastValueShape);
                ...
                if (hasPastKey)
                {
                    if (!TryValidatePastCache(keyShape, valueShape, pastKey, pastKeyShape, pastValue, pastValueShape))
                        return false;
                    attentionKeyShape = new AexisGraphSession.BufferShape(3, keyShape.w, pastKeyShape.h + keyShape.h, 1, keyShape.c);
                    attentionValueShape = new AexisGraphSession.BufferShape(3, valueShape.w, pastValueShape.h + valueShape.h, 1, valueShape.c);
                }
            }
            ...
            AexisGraphSession.TensorRef attnMask = null;
            ...
            if (common.sp.attnMask)
            {
                if (layer.bottomNames.Length <= 3
                    ...
                    || attnMask.packs != 1
                    || AexisGraphSession.IsStrictLinearMatTexture(attnMask))
                    return false;
            }
            ...
            out SdpaCmdRtPlan plan)
        {
            ...
            AexisGraphSession.CmdTensorRef pastKey = null;
            AexisGraphSession.CmdTensorRef pastValue = null;
            AexisGraphSession.BufferShape pastKeyShape = default;
            AexisGraphSession.BufferShape pastValueShape = default;
            var attentionKeyShape = keyShape;
            var attentionValueShape = valueShape;
            ...
            AexisGraphSession.CmdTensorRef attnMask = null;
            AexisGraphSession.BufferShape attnMaskShape = default;
            if (common.sp.attnMask)
            {
                if (layer.bottomNames.Length <= 3
        ...
        private static bool TryResolveCommonPlan(AexisGraphSession owner, AexisGraphModel.Layer layer, out SdpaCommonPlan plan)
        {
            plan = default;
        ...
        private static bool TryValidateTextureInputs(
            object queryTex,
            AexisGraphSession.BufferShape queryShape,
            object keyTex,
            AexisGraphSession.BufferShape keyShape,
            object valueTex,
            AexisGraphSession.BufferShape valueShape,
            in SdpaCommonPlan common,
            out AexisGraphSession.BufferShape outputShape,
            out AexisGraphSession.BufferShape outputStorageShape)
        {
            outputShape = default;
            ...
            outputShape = new AexisGraphSession.BufferShape(3, valueShape.w, queryShape.h, 1, queryShape.c);
            outputStorageShape = outputShape;
            return true;
        }

        private static bool TryValidatePastCache(
            AexisGraphSession.BufferShape currentKeyShape,
            AexisGraphSession.BufferShape currentValueShape,
            object pastKey,
            AexisGraphSession.BufferShape pastKeyShape,
            object pastValue,
            AexisGraphSession.BufferShape pastValueShape)
        {
        ...
        {
            public readonly AexisGraphSession.SdpaPack sp;
            public readonly float scale;

            public SdpaCommonPlan(AexisGraphSession.SdpaPack sp)
            {
                this.sp = sp;
                scale = sp.scale;
            }
        ...
        private readonly struct SdpaRtPlan
        {
            ...
            public readonly AexisGraphSession.TensorRef query;
            public readonly AexisGraphSession.BufferShape queryShape;
            public readonly AexisGraphSession.BufferShape queryStorageShape;
            public readonly int querySlices;
            public readonly AexisGraphSession.TensorRef key;
            public readonly AexisGraphSession.BufferShape keyCurrentShape;
            public readonly AexisGraphSession.BufferShape keyShape;
            public readonly AexisGraphSession.TensorRef value;
            public readonly AexisGraphSession.BufferShape valueCurrentShape;
            public readonly AexisGraphSession.BufferShape valueShape;
            public readonly AexisGraphSession.TensorRef pastKey;
            public readonly AexisGraphSession.BufferShape pastKeyShape;
            public readonly AexisGraphSession.TensorRef pastValue;
            public readonly AexisGraphSession.BufferShape pastValueShape;
            public readonly bool hasPastCache;
            public readonly AexisGraphSession.TensorRef attnMask;
            ...
            public readonly AexisGraphSession.BufferShape outputShape;
            ...
            public SdpaRtPlan(
                float scale,
                bool causal,
                AexisGraphSession.TensorRef query,
                AexisGraphSession.BufferShape queryShape,
                AexisGraphSession.TensorRef key,
                AexisGraphSession.BufferShape keyShape,
                AexisGraphSession.TensorRef value,
                AexisGraphSession.BufferShape valueShape,
                AexisGraphSession.TensorRef pastKey,
                AexisGraphSession.BufferShape pastKeyShape,
                AexisGraphSession.TensorRef pastValue,
                AexisGraphSession.BufferShape pastValueShape,
                AexisGraphSession.BufferShape attentionKeyShape,
                AexisGraphSession.BufferShape attentionValueShape,
                AexisGraphSession.TensorRef attnMask,
                AexisGraphSession.BufferShape outputShape,
                AexisGraphSession.BufferShape outputStorageShape)
            {
                ...
                keyTransposedShape = new AexisGraphSession.BufferShape(4, keyShape.h, keyShape.w, Mathf.Max(1, keyShape.d), keyShape.c);
                ...
                scoresShape = new AexisGraphSession.BufferShape(4, keyShape.h, queryShape.h, Mathf.Max(1, queryShape.d), queryShape.c);
        ...
        private readonly struct SdpaCmdRtPlan
        {
            public readonly float scale;
            public readonly bool causal;
            public readonly AexisGraphSession.CmdTensorRef query;
            public readonly AexisGraphSession.BufferShape queryShape;
            public readonly AexisGraphSession.BufferShape queryStorageShape;
            public readonly int querySlices;
            public readonly AexisGraphSession.CmdTensorRef key;
            public readonly AexisGraphSession.BufferShape keyCurrentShape;
            public readonly AexisGraphSession.BufferShape keyShape;
            public readonly AexisGraphSession.CmdTensorRef value;
            public readonly AexisGraphSession.BufferShape valueCurrentShape;
            public readonly AexisGraphSession.BufferShape valueShape;
            public readonly AexisGraphSession.CmdTensorRef pastKey;
            public readonly AexisGraphSession.BufferShape pastKeyShape;
            public readonly AexisGraphSession.CmdTensorRef pastValue;
            public readonly AexisGraphSession.BufferShape pastValueShape;
            public readonly bool hasPastCache;
            public readonly AexisGraphSession.CmdTensorRef attnMask;
            public readonly bool hasAttnMask;
            public readonly AexisGraphSession.BufferShape keyTransposedShape;
            public readonly AexisGraphSession.BufferShape keyTransposedStorageShape;
            public readonly int keyTransposedSlices;
            ...
            public SdpaCmdRtPlan(
                float scale,
                bool causal,
                AexisGraphSession.CmdTensorRef query,
                AexisGraphSession.BufferShape queryShape,
                AexisGraphSession.CmdTensorRef key,
                AexisGraphSession.BufferShape keyShape,
                AexisGraphSession.CmdTensorRef value,
                AexisGraphSession.BufferShape valueShape,
                AexisGraphSession.CmdTensorRef pastKey,
                AexisGraphSession.BufferShape pastKeyShape,
                AexisGraphSession.CmdTensorRef pastValue,
                AexisGraphSession.BufferShape pastValueShape,
                AexisGraphSession.BufferShape attentionKeyShape,
                AexisGraphSession.BufferShape attentionValueShape,
                AexisGraphSession.CmdTensorRef attnMask,
                AexisGraphSession.BufferShape outputShape,
                AexisGraphSession.BufferShape outputStorageShape)
            {
                this.scale = Mathf.Approximately(scale, 0f) ? 1f / Mathf.Sqrt(Mathf.Max(1, queryShape.w)) : scale;
