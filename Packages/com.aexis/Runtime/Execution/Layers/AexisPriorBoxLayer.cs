...
namespace Aexis.Execution
{
    ...
    public sealed class AexisPriorBoxLayer : AexisBaseLayer
    {
        public AexisPriorBoxLayer()
            : base(AexisLayerTypes.PriorBox, supportsBufferPath: true, supportsCommandBufferPath: true)
        {
        }
        ...
        {
            var pack = new AexisGraphSession.PriorBoxPack
            {
        ...
        public override void ExecuteComputeBufferPath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not AexisGraphSession.PriorBoxPack pp)
        ...
        public override void ExecuteRenderTexturePath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            var spec = ResolvePriorBoxSpec(
                layer,
                pp,
                (string name, out AexisGraphSession.BufferShape shape) =>
                {
            ...
            try
            {
                output = owner.RentTempMat(spec.storageShape.w, spec.storageShape.h, AexisGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.PriorBox(
                    output,
                    spec.mode,
                    spec.featW,
                    ...
                    spec.stepH,
                    pp.offset);
                AexisGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, spec.logicalShape, spec.storageShape);
                output = null;
            }
            finally
            {
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            var spec = ResolvePriorBoxSpec(
                ...
                (string name, out AexisGraphSession.BufferShape shape) =>
                {
                    shape = AexisGraphSession.GetCmdShape(shapes, blobs, name);
                    return true;
                });
            ...
            try
            {
                output = owner.RentTempMat(cmd, spec.storageShape.w, spec.storageShape.h, AexisGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.PriorBox(
                    cmd,
                    output,
                    spec.mode,
                    ...
                    spec.stepH,
                    pp.offset);
                blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(output, spec.logicalShape, spec.storageShape, owned: true);
                output = null;
                if (shapes != null)
        ...
        private readonly struct PriorBoxSpec
        {
            ...
            public PriorBoxSpec(int mode, int featW, int featH, int imageW, int imageH, int numPrior, float stepW, float stepH, AexisGraphSession.BufferShape logicalShape)
            {
                this.mode = mode;
                this.featW = featW;
                this.featH = featH;
                ...
                storageShape = AexisGraphSession.ResolveLinearMatStorageShape(logicalShape);
            }
        }

        private delegate bool TryGetShapeDelegate(string name, out AexisGraphSession.BufferShape shape);

        private static PriorBoxSpec ResolvePriorBoxSpec(AexisGraphModel.Layer layer, AexisGraphSession.PriorBoxPack pp, TryGetShapeDelegate tryGetShape)
        {
            ...
            if (useMxnet)
            {
                ...
                return new PriorBoxSpec(0, featW, featH, 1, 1, numPrior, stepW, stepH, new AexisGraphSession.BufferShape(1, total, 1, 1, 1));
            }
            ...
            return new PriorBoxSpec(1, featW, featH, imageW, imageH, caffeNumPrior, stepWidth, stepHeight, new AexisGraphSession.BufferShape(2, caffeTotal, 2, 1, 1));
        }

        private static ComputeBuffer NewParamBuffer(float[] values)
        {
            return AexisGraphSession.NewBuffer(values != null && values.Length > 0 ? values : new[] { 0f });
        }
        ...
        private static float[] BuildMxnetPrior(int w, int h, AexisGraphSession.PriorBoxPack pp)
        {
        ...
        private static float[] BuildCaffePrior(int w, int h, int imageW, int imageH, AexisGraphSession.PriorBoxPack pp)
        {
