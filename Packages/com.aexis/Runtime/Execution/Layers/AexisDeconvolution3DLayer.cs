...
{
    public sealed class AexisDeconvolution3DLayer : AexisBaseLayer
    {
        public AexisDeconvolution3DLayer()
            : base(AexisLayerTypes.Deconvolution3D, supportsBufferPath: true, supportsCommandBufferPath: true)
        {
        }
...
#pragma warning restore CS0618
        ...
        public override AexisGraphSession.LayerLoadMetrics LoadLayer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisWeightReader br)
        {
            ...
            if (owner.KeepRawConvWeightsForTexturePath || owner.ForceBufferConvolution || owner.ForceBufferConvolutionAll)
            {
                ...
                AexisGpuResourceTracker.RegisterBuffer(pack.rawWeight, w.Length, sizeof(float), "AexisGraphSession.Deconv3DRawWeight:" + layer.name);
                pack.rawBias = new ComputeBuffer(b.Length, sizeof(float), ComputeBufferType.Structured);
                AexisGpuResourceTracker.RegisterBuffer(pack.rawBias, b.Length, sizeof(float), "AexisGraphSession.Deconv3DRawBias:" + layer.name);
            ...
            AexisGpuResourceTracker.RegisterBuffer(pack.packedWeight4, w4.Length, sizeof(float) * 4, "AexisGraphSession.Deconv3DPackedWeight4:" + layer.name);
            pack.packedBias4 = new ComputeBuffer(b4.Length, sizeof(float) * 4, ComputeBufferType.Structured);
            AexisGpuResourceTracker.RegisterBuffer(pack.packedBias4, b4.Length, sizeof(float) * 4, "AexisGraphSession.Deconv3DPackedBias4:" + layer.name);
        ...
        public override void ExecuteComputeBufferPath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            var outW = AexisGraphSession.ComputeDeconvOut(srcView.w, deconv.kernelW, deconv.dilationW, deconv.strideW, deconv.padLeft, deconv.padRight, deconv.outputPadRight);
            var outH = AexisGraphSession.ComputeDeconvOut(srcView.h, deconv.kernelH, deconv.dilationH, deconv.strideH, deconv.padTop, deconv.padBottom, deconv.outputPadBottom);
            var outD = AexisGraphSession.ComputeDeconvOut(srcView.d, deconv.kernelD, deconv.dilationD, deconv.strideD, deconv.padFront, deconv.padBehind, deconv.outputPadBehind);
...
#pragma warning restore CS0618
            ...
            var outD = AexisGraphSession.ComputeDeconvOut(srcShape.d, deconv.kernelD, deconv.dilationD, deconv.strideD, deconv.padFront, deconv.padBehind, deconv.outputPadBehind);
            var outRt = owner.RentTempArray(outW, outH, outD * deconv.outPacks, AexisGraphSession.ResolveTensorTextureFormat(4));
            ...
            AexisGraphSession.SetTextureBlob(
                ...
                layer.topNames[0],
                outRt,
                new AexisGraphSession.BufferShape(4, outW, outH, outD, deconv.outC));
            owner.Consume(textureBlobs, bufferBlobs, bufferRefs, bufferViews, remaining, layer.bottomNames, pinnedNames);
        }
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            var outH = AexisGraphSession.ComputeDeconvOut(srcShape.h, deconv.kernelH, deconv.dilationH, deconv.strideH, deconv.padTop, deconv.padBottom, deconv.outputPadBottom);
            var outD = AexisGraphSession.ComputeDeconvOut(srcShape.d, deconv.kernelD, deconv.dilationD, deconv.strideD, deconv.padFront, deconv.padBehind, deconv.outputPadBehind);
            var outShape = new AexisGraphSession.BufferShape(4, outW, outH, outD, deconv.outC);
            var outRt = owner.RentTempArray(cmd, outW, outH, outD * deconv.outPacks, AexisGraphSession.ResolveTensorTextureFormat(4));
            owner.Ops.Deconvolution3dPack4CDHW(
                cmd,
                src.texture,
                srcShape.w,
