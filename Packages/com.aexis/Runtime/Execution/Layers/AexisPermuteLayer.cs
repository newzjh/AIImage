...
namespace Aexis.Execution
{
    ...
    public sealed class AexisPermuteLayer : AexisBaseLayer
    {
        public AexisPermuteLayer() : base(AexisLayerTypes.Permute, supportsBufferPath: true, supportsCommandBufferPath: true) { }
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            var axes = AexisGraphSession.ResolvePermuteAxes(dims, orderType, layer.name);
            var outShape = AexisGraphSession.ResolvePermuteShape(srcTensor, dims, axes);
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            if (canUseLinearMat || canUsePack4)
            {
                ...
                if (orderType == 0)
                {
                    blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorAlias(src, srcShape, sourceContract.StorageShape);
                    if (shapes != null)
                ...
                else
                {
                    ...
                    else
                    {
                        if (srcShape.dims == 4)
                        {
                            ...
                            var outArr4D = owner.RentTempArray(cmd, outShape.w, outShape.h, outDepth4D, AexisGraphSession.ResolveTensorTextureFormat(outShape.dims));
                            owner.Ops.PermutePack4Cdhw(
                                cmd,
                                src.texture,
                                srcShape.w,
                                ...
                                outShape.c,
                                outArr4D);
                            blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(outArr4D, outShape, outShape, owned: true, blobName: layer.topNames[0]);
                        }
                        else
                        {
                            var outChannels = srcShape.dims == 2 ? 1 : outShape.c;
                            ...
                            var storageShape = srcShape.dims == 2
                                ? new AexisGraphSession.BufferShape(3, outShape.w, outShape.h, 1, 1)
                                : outShape;
                            blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(outArr, outShape, storageShape, owned: true, blobName: layer.topNames[0]);
                        }
                    }
                    if (shapes != null)
            ...
            out AexisGraphSession.BufferShape srcShape)
        {
            ...
            if (!AexisGraphSession.TryGetExistingTexture(textureBlobs, textureShapes, inputName, out srcTex, out srcShape))
                return false;
            ...
            System.Collections.Generic.Dictionary<string, AexisGraphSession.BufferShape> textureShapes)
        {
            ...
            var isQkvPrep = srcShape.w == storageShape.w
                ...
                && HasSingleConsumerOfType(owner.Model, layer.topNames, AexisLayerTypes.SDPA);
        ...
        private static bool HasSingleConsumerOfType(AexisGraphModel model, string[] topNames, AexisLayerTypeKey type)
        {
        ...
        private static bool HasSingleContextFlattenConsumer(AexisGraphModel model, string[] topNames)
        {
            if (model == null || topNames == null || topNames.Length == 0)
                return false;
            ...
            System.Collections.Generic.Dictionary<string, AexisGraphSession.BufferShape> textureShapes)
        {
            ...
            var outPacks = Mathf.Max(1, Mathf.CeilToInt(outShape.c / 4f));
            var outRt = owner.RentTempArray(foldStorageShape.w, foldStorageShape.h, outPacks, AexisGraphSession.ResolveTensorTextureFormat(outShape.dims));
            owner.Ops.PermutePack4CdhwFoldD(
                ...
                outShape.c,
                outRt);
            AexisGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outRt, outShape, foldStorageShape);
            owner.DebugLog?.Invoke(
            ...
            CommandBuffer cmd)
        {
            ...
            var outPacks = Mathf.Max(1, Mathf.CeilToInt(outShape.c / 4f));
            var outRt = owner.RentTempArray(cmd, foldStorageShape.w, foldStorageShape.h, outPacks, AexisGraphSession.ResolveTensorTextureFormat(outShape.dims));
            owner.Ops.PermutePack4CdhwFoldD(
                cmd,
                src.texture,
                srcShape.w,
                ...
                outShape.c,
                outRt);
            blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(outRt, outShape, foldStorageShape, owned: true, blobName: layer.topNames[0]);
            if (shapes != null)
                shapes[layer.topNames[0]] = outShape;
            owner.DebugLog?.Invoke(
            ...
            CommandBuffer cmd)
        {
            ...
            owner.Ops.PermutePack4FoldDToCdhw(
                ...
                outShape.c,
                outRt);
            blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(outRt, outShape, outShape, owned: true, blobName: layer.topNames[0]);
            if (shapes != null)
                shapes[layer.topNames[0]] = outShape;
            owner.DebugLog?.Invoke(
        ...
        private static bool IsExactCdhwStorage(AexisGraphSession.TensorRef tensor, AexisGraphSession.BufferShape logicalShape)
        {
        ...
        private static bool IsExactCdhwStorage(AexisGraphSession.CmdTensorRef tensor, AexisGraphSession.BufferShape logicalShape)
        {
            if (tensor == null || tensor.texture == null || logicalShape.dims != 4)
                return false;
        ...
        private static bool IsFoldDStorage(AexisGraphSession.TensorRef tensor, AexisGraphSession.BufferShape logicalShape)
        {
        ...
        private static bool IsFoldDStorage(AexisGraphSession.CmdTensorRef tensor, AexisGraphSession.BufferShape logicalShape)
        {
            if (tensor == null || tensor.texture == null || !TryResolveFoldDStorageShape(logicalShape, out var foldStorageShape))
                return false;
            var storageShape = AexisGraphSession.GetCmdStorageShape(tensor, logicalShape);
        ...
        private static bool TryResolveFoldDStorageShape(AexisGraphSession.BufferShape logicalShape, out AexisGraphSession.BufferShape storageShape)
        {
            ...
            storageShape = new AexisGraphSession.BufferShape(4, logicalShape.w, foldedHeight, 1, logicalShape.c);
            return true;
        }

        private static bool CanUseExactCdhwStorageShape(AexisGraphSession.BufferShape logicalShape)
        {
            if (logicalShape.dims != 4
                || logicalShape.w <= 0
                || logicalShape.h <= 0
        ...
        private static bool CanUsePack4Permute(AexisGraphSession.TensorRef srcTex, AexisGraphSession.BufferShape srcShape, int orderType, out Vector4Int axes, out AexisGraphSession.BufferShape outShape)
        {
            axes = default;
            outShape = default;
        ...
        private static bool CanUsePack4Permute(AexisGraphSession.CmdTensorRef src, AexisGraphSession.BufferShape srcShape, int orderType, out Vector4Int axes, out AexisGraphSession.BufferShape outShape)
        {
            axes = default;
            outShape = default;
        ...
        private static bool CanUseLinearMatPermute(AexisGraphSession.TensorRef srcTex, AexisGraphSession.BufferShape srcShape, int orderType, out Vector4Int axes, out AexisGraphSession.BufferShape outShape)
        {
            axes = default;
            outShape = default;
        ...
        private static bool CanUseLinearMatPermute(AexisGraphSession.CmdTensorRef src, AexisGraphSession.BufferShape srcShape, int orderType, out Vector4Int axes, out AexisGraphSession.BufferShape outShape)
        {
            axes = default;
            outShape = default;
        ...
        private static bool CanUsePack4PermuteCore(AexisGraphSession.BufferShape srcShape, int orderType, out Vector4Int axes, out AexisGraphSession.BufferShape outShape)
        {
            axes = default;
            outShape = default;
            if (srcShape.dims == 2)
            {
                ...
                if (orderType == 1)
                {
                    ...
                    outShape = new AexisGraphSession.BufferShape(2, srcShape.h, srcShape.w, 1, 1);
                    return true;
                }
            ...
            {
                axes = AexisGraphSession.ResolvePermuteAxes(3, orderType, "PermutePack4");
                outShape = AexisGraphSession.ResolvePermuteShape(srcShape, 3, axes);
                return outShape.dims == 3 && outShape.d == 1;
            }

            if (srcShape.dims == 4)
            {
                axes = AexisGraphSession.ResolvePermuteAxes(4, orderType, "PermutePack4CDHW");
                outShape = AexisGraphSession.ResolvePermuteShape(srcShape, 4, axes);
