...
namespace Aexis.Execution
{
    ...
    public sealed class AexisQuantizeLayer : AexisBaseLayer
    {
        public AexisQuantizeLayer()
            : base(AexisLayerTypes.Quantize, supportsBufferPath: true, supportsCommandBufferPath: true)
        {
        }
        ...
        public override AexisGraphSession.LayerLoadMetrics LoadLayer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisWeightReader br)
        {
            ...
            var qp = new AexisGraphSession.QuantizePack
            {
                scaleDataSize = layer.GetInt(0, 1)
            };

            phaseSw.Restart();
            qp.scaleCpu = br.ReadTensorAsFloat32(qp.scaleDataSize, 0, 0, 0, 1);
            phaseSw.Stop();
            readMs += phaseSw.ElapsedMilliseconds;

            phaseSw.Restart();
            qp.scale = AexisGraphSession.NewBuffer(qp.scaleCpu);
            phaseSw.Stop();
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not AexisGraphSession.QuantizePack qp)
