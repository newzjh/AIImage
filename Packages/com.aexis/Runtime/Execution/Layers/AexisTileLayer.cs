...
namespace Aexis.Execution
{
    ...
    public sealed class AexisTileLayer : AexisBaseLayer
    {
        public AexisTileLayer() : base(AexisLayerTypes.Tile, supportsBufferPath: true, supportsCommandBufferPath: true) { }
...
#pragma warning restore CS0618
        ...
        public override void ExecuteComputeBufferPath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            var tensorAxis = AexisGraphSession.MapNcnnAxisToTensorAxis(srcView.dims, axis);
            var outW = srcView.w;
            var outH = srcView.h;
            var outD = srcView.d;
            var outC = srcView.c;
        ...
        public override void ExecuteRenderTexturePath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            if (spec.isPassthrough)
            {
                ...
                {
                    var storageShape = AexisGraphSession.GetTextureStorageShape(passthroughTex, srcShape);
                    textureBlobs[layer.topNames[0]] = AexisGraphSession.CreateTextureAlias(passthroughTex, spec.outShape, storageShape);
            ...
            var srcStorageShape = AexisGraphSession.GetTextureStorageShape(src, srcShape);
            if (AexisGraphSession.IsStrictLinearMatTexture(src))
            {
                var outStorageShape = AexisGraphSession.ResolveLinearMatStorageShape(spec.outShape);
                var output = owner.RentTempMat(outStorageShape.w, outStorageShape.h, AexisGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.TileLinearMat(src.texture, srcShape, srcStorageShape, spec.outShape, outStorageShape, spec.repeats, output);
                AexisGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, spec.outShape, outStorageShape);
            }
            else
            {
                ...
                var output = owner.RentTempArray(
                    ...
                    AexisGraphSession.ResolveTensorTextureFormat(spec.outShape.dims));
                owner.Ops.TilePack4(src.texture, srcShape, spec.outShape, spec.repeats, output);
                AexisGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], output, spec.outShape, spec.outShape);
            }
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            if (spec.isPassthrough)
            {
                blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorAlias(src, spec.outShape, sourceContract.StorageShape);
                if (shapes != null)
            ...
            {
                var outStorageShape = AexisGraphSession.ResolveLinearMatStorageShape(spec.outShape);
                var output = owner.RentTempMat(cmd, outStorageShape.w, outStorageShape.h, AexisGraphSession.ResolveLinearMatTextureFormat());
                owner.Ops.TileLinearMat(cmd, src.texture, srcShape, srcStorageShape, spec.outShape, outStorageShape, spec.repeats, output);
                blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(output, spec.outShape, outStorageShape, owned: true, blobName: layer.topNames[0]);
            }
            else
            {
                if (src.texture.dimension != TextureDimension.Tex2DArray || !IsDirectPack4StorageView(srcShape, srcStorageShape))
                ...
                var outputStorageShape = spec.outShape.dims <= 2
                    ? new AexisGraphSession.BufferShape(3, spec.outShape.w, spec.outShape.dims == 2 ? spec.outShape.h : 1, 1, 1)
                    : spec.outShape;
                blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(output, spec.outShape, outputStorageShape, owned: true, blobName: layer.topNames[0]);
            }
        ...
        {
            public readonly AexisGraphSession.BufferShape outShape;
            public readonly Vector4Int repeats;
            public readonly bool isPassthrough;

            public TileSpec(AexisGraphSession.BufferShape outShape, Vector4Int repeats, bool isPassthrough)
            {
            ...
            out AexisGraphSession.BufferShape shape)
        {
            ...
            if (context.textureBlobs != null
                ...
                && tex.texture != null)
            {
                shape = AexisGraphSession.GetTextureShape(context.textureShapes, tex, layer.bottomNames[0]);
                return shape.dims >= 1 && shape.dims <= 4;
            }

            var view = AexisGraphSession.TryGetBufferView(layer.bottomNames[0], context.bufferBlobs, context.bufferViews);
            if (view != null)
            {
        ...
        private static bool TryResolveTileSpec(AexisGraphModel.Layer layer, AexisGraphSession.BufferShape srcShape, out TileSpec spec)
        {
            spec = default;
            ...
            if (outDims == 1)
                outShape = new AexisGraphSession.BufferShape(1, outW, 1, 1, 1);
            else if (outDims == 2)
                outShape = new AexisGraphSession.BufferShape(2, outW, outH, 1, 1);
            else if (outDims == 3)
                outShape = new AexisGraphSession.BufferShape(3, outW, outH, 1, outC);
            else
                outShape = new AexisGraphSession.BufferShape(4, outW, outH, outD, outC);

            var sameShape = AexisGraphSession.BufferShapeEquals(srcShape, outShape);
        ...
        private static int[] ResolveRepeats(AexisGraphModel.Layer layer)
        {
        ...
        private static bool IsDirectPack4StorageView(AexisGraphSession.BufferShape logicalShape, AexisGraphSession.BufferShape storageShape)
        {
