...
namespace Aexis.Execution
{
    ...
    public sealed class AexisShortConvLayer : AexisBaseLayer
    {
        public AexisShortConvLayer()
            : base(AexisLayerTypes.ShortConv, supportsBufferPath: false, supportsCommandBufferPath: true)
        {
        }
        ...
        {
            owner._extraPacks[layer.name] = new AexisGraphSession.ShortConvPack();
            return default;
        }
        ...
        public override void ExecuteRenderTexturePath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            var mixedShape = AexisGraphSession.GetTextureShape(context.textureShapes, mixedRef, layer.bottomNames[1]);
            var cacheShape = AexisGraphSession.GetTextureShape(context.textureShapes, cacheRef, layer.bottomNames[2]);
            ...
            try
            {
                ...
                AexisGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], output, mixedShape, StorageShape(mixedShape, output));
                AexisGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[1], cacheOut, cacheShape, StorageShape(cacheShape, cacheOut));
                output = null;
                cacheOut = null;
            }
            finally
        ...
        private static AexisGraphSession.TensorRef RequireTexture(AexisLayerBufferContext context, string name, string layer)
        {
        ...
        private static TextureView ToArray(AexisGraphSession owner, AexisGraphSession.TensorRef source, Dictionary<string, AexisGraphSession.BufferShape> shapes, string name)
        {
            if (source.texture.dimension == TextureDimension.Tex2DArray)
                return new TextureView { texture = source.texture, temporary = false };
            var logical = AexisGraphSession.GetTextureShape(shapes, source, name);
            var storage = AexisGraphSession.GetTextureStorageShape(source, logical);
        ...
        private static AexisGraphSession.BufferShape StorageShape(AexisGraphSession.BufferShape logical, RenderTexture texture)
        {
            return logical.dims <= 2
                ? new AexisGraphSession.BufferShape(3, texture.width, texture.height, 1, Mathf.Max(1, texture.volumeDepth) * 4)
                : new AexisGraphSession.BufferShape(logical.dims, texture.width, texture.height, logical.d, Mathf.Max(1, texture.volumeDepth) * 4);
        }
    }
}
