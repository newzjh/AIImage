...
namespace Aexis.Execution
{
    ...
    public sealed class AexisGatedDeltaRuleLayer : AexisBaseLayer
    {
        public AexisGatedDeltaRuleLayer()
            : base(AexisLayerTypes.GatedDeltaRule, supportsBufferPath: false, supportsCommandBufferPath: true)
        {
        }
        ...
        {
            owner._extraPacks[layer.name] = new AexisGraphSession.GatedDeltaRulePack
            {
                epsilon = 1e-6f
            };
            return default;
        ...
        public override void ExecuteRenderTexturePath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            var qShape = AexisGraphSession.GetTextureShape(context.textureShapes, q, layer.bottomNames[4]);
            var vShape = AexisGraphSession.GetTextureShape(context.textureShapes, v, layer.bottomNames[6]);
            var stateShape = AexisGraphSession.GetTextureShape(context.textureShapes, state, layer.bottomNames[7]);
            ...
            try
            {
                ...
                AexisGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], output, vShape, AexisGraphSession.GetTextureStorageShape(v, vShape));
                AexisGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[1], finalState, stateShape, AexisGraphSession.GetTextureStorageShape(state, stateShape));
                output = null;
        ...
        private static ScalarView RequireScalar(AexisGraphSession owner, AexisLayerBufferContext context, string name, string layer)
        {
            ...
            var shape = AexisGraphSession.GetTextureShape(context.textureShapes, tensor, name);
            var elements = Mathf.Max(1, shape.w) * Mathf.Max(1, shape.h) * Mathf.Max(1, shape.d) * Mathf.Max(1, shape.c);
            var linear = owner.RentTempMat(elements, 1, AexisGraphSession.ResolveLinearMatTextureFormat());
        ...
        private static AexisGraphSession.TensorRef RequireArray(AexisLayerBufferContext context, string name, string layer)
        {
