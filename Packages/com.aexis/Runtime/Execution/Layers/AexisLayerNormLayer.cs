...
namespace Aexis.Execution
{
    ...
    public sealed class AexisLayerNormLayer : AexisBaseLayer
    {
        public AexisLayerNormLayer() : base(AexisLayerTypes.LayerNorm, supportsBufferPath: true, supportsCommandBufferPath: true) { }
        ...
        public override AexisGraphSession.LayerLoadMetrics LoadLayer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisWeightReader br)
        {
                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        var lp = new AexisGraphSession.LayerNormPack();
        ...
        public override void ExecuteBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            if (!owner.ShouldForceCurrentLayerBufferPath()
                ...
                && AexisGraphSession.TryGetExistingTexture(context.textureBlobs, context.textureShapes, layer.bottomNames[0], out var src, out var srcShape)
...
#pragma warning restore CS0618
        ...
        public override void ExecuteRenderTexturePath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
            ...
            var isStrictLinear = AexisGraphSession.IsStrictLinearMatTexture(srcTex);
            ...
            try
            {
                ...
                {
                    AexisGraphSession.SetTextureBlob(textureBlobs, textureShapes, layer.topNames[0], outRt, srcShape, srcShape);
                    outRt = null;
                }
            }
            finally
        ...
        private static void ResolveLayerNormRowsCols(AexisTensorBuffer srcView, int affineSize, string layerName, out int rows, out int cols)
        {
            if (srcView == null)
        ...
        private static bool ShouldPromoteAttentionPrepTexture(AexisGraphSession owner, AexisGraphModel.Layer layer)
        {
            ...
            return gemm != null
                && gemm.type == AexisLayerTypes.Gemm
                && gemm.GetInt(5, 0) != 0;
        }
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            if (CanUsePack4WidthCmdPath(src, srcShape, lp))
            {
                ...
                var isStrictLinear = AexisGraphSession.IsStrictLinearMatTexture(src);
                var packCount = Mathf.Max(1, Mathf.CeilToInt(Mathf.Max(1, srcShape.c) / 4f));
                var logicalDepth = srcShape.dims == 4 ? Mathf.Max(1, srcShape.d) : 1;
                var sliceCount = logicalDepth * packCount;
                ...
                try
                {
                    ...
                    {
                        blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(outArr, srcShape, srcShape, owned: true);
                        outArr = null;
                    }
                }
                finally
        ...
        private static RenderTextureFormat ResolveLayerNormOutputFormat(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisGraphSession.BufferShape srcShape)
        {
            if (ShouldPromoteAttentionPrepTexture(owner, layer))
                return RenderTextureFormat.ARGBFloat;
            return AexisGraphSession.ResolveTensorTextureFormat(srcShape.dims);
        }

        private static bool TryGetPack4WidthTexture(
            AexisGraphSession owner,
            AexisGraphModel.Layer layer,
            AexisGraphSession.LayerNormPack lp,
        ...
        private static bool CanUsePack4WidthPath(AexisGraphSession.TensorRef srcTex, AexisGraphSession.BufferShape srcShape, AexisGraphSession.LayerNormPack lp)
        {
            if (CanUsePack4Linear2DPath(srcTex, srcShape, lp))
                return true;

            if (srcTex != null && AexisGraphSession.IsStrictLinearMatTexture(srcTex))
            {
                var storageShape = AexisGraphSession.GetTextureStorageShape(srcTex, srcShape);
                return lp != null
                    && lp.affine
        ...
        private static bool CanUsePack4WidthCmdPath(AexisGraphSession.CmdTensorRef src, AexisGraphSession.BufferShape srcShape, AexisGraphSession.LayerNormPack lp)
        {
            if (CanUsePack4Linear2DPath(src, srcShape, lp))
                return true;

            if (src != null && AexisGraphSession.IsStrictLinearMatTexture(src))
            {
                var storageShape = AexisGraphSession.GetCmdStorageShape(src, srcShape);
        ...
        private static bool CanUsePack4Linear2DPath(AexisGraphSession.TensorRef srcTex, AexisGraphSession.BufferShape srcShape, AexisGraphSession.LayerNormPack lp)
        {
            return srcTex != null
                && srcTex.texture != null
                && lp != null
                ...
                && srcShape.h > 0
                && srcShape.w == lp.affineSize
                && AexisGraphSession.IsPack4LinearMatTexture(srcTex, srcShape);
        }

        private static bool CanUsePack4Linear2DPath(AexisGraphSession.CmdTensorRef src, AexisGraphSession.BufferShape srcShape, AexisGraphSession.LayerNormPack lp)
        {
            return src != null
                && src.texture != null
                && lp != null
