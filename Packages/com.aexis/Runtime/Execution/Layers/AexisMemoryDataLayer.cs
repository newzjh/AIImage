...
namespace Aexis.Execution
{
    ...
    public sealed class AexisMemoryDataLayer : AexisBaseLayer
    {
        public AexisMemoryDataLayer() : base(AexisLayerTypes.MemoryData, supportsBufferPath: true, supportsCommandBufferPath: true) { }
        ...
        public override AexisGraphSession.LayerLoadMetrics LoadLayer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisWeightReader br)
        {
                        ...
                        var phaseSw = new Stopwatch();
                                        ...
                                        phaseSw.Restart();
                                        var a = AexisGraphSession.ReadClipMatAsFloat32(br, w, h, d, c, loadType);
                                        phaseSw.Stop();
                                        ...
                                        if (h > 0) dims = 2;
                                        if (c > 0) dims = d > 0 ? 4 : 3;
                                        var memoryPack = new AexisGraphSession.MemoryDataPack
                                        {
                                            dims = dims,
                                            w = Mathf.Max(1, w),
                                            h = Mathf.Max(1, h),
        ...
        public override void ExecuteComputeBufferPath(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerBufferContext context)
        {
                        ...
                        do
                        {
                                                ...
                                                {
                                                    var logicalShape = new AexisGraphSession.BufferShape(mp.dims, mp.w, mp.h, mp.d, mp.c);
                                                ...
                                                var tensor = new AexisTensorBuffer(mp.data, mp.dims, mp.w, mp.h, mp.d, mp.c, false);
...
#pragma warning restore CS0618
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            owner.PublishCmdTensorBufferOutput(
                cmd,
                layer.topNames[0],
                new AexisTensorBuffer(mp.data, mp.dims, mp.w, mp.h, mp.d, mp.c, false),
                preferTexture: true,
                blobs,
                shapes);
        }

        private static bool TryCreateLinearMatTexture(AexisGraphSession.MemoryDataPack memoryPack)
        {
            if (memoryPack == null
            ...
            try
            {
                ...
                texture.Create();
                AexisGpuResourceTracker.RegisterTexture(texture, "NcnnMemoryDataLinearMat");
            ...
            finally
            {
                ...
                if (texture != null)
                {
                    AexisGpuResourceTracker.ReleaseTexture(texture, "NcnnMemoryDataLinearMat.create-failed");
                    texture.Release();
        ...
        private static bool TryCreatePack4Texture(AexisGraphSession.MemoryDataPack memoryPack)
        {
            ...
            finally
            {
                ...
                if (texture != null)
                {
                    AexisGpuResourceTracker.ReleaseTexture(texture, "NcnnMemoryDataPack4.create-failed");
                    texture.Release();
                    UnityEngine.Object.DestroyImmediate(texture);
                }
            }
            ...
            Dictionary<string, AexisGraphSession.BufferShape> textureShapes)
        {
            ...
            textureBlobs[layer.topNames[0]] = AexisGraphSession.CreateTextureRef(memoryPack.pack4Rt, shape, shape, owned: false, blobName: layer.topNames[0]);
            ...
            Dictionary<string, AexisGraphSession.BufferShape> shapes)
        {
            ...
            var shape = new AexisGraphSession.BufferShape(memoryPack.dims, memoryPack.w, memoryPack.h, memoryPack.d, memoryPack.c);
            blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(texture, shape, shape, owned: true, blobName: layer.topNames[0]);
            ...
            Dictionary<string, AexisGraphSession.BufferShape> shapes)
        {
            ...
            blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(vector, logicalShape, storageShape, owned: true, blobName: layer.topNames[0]);
            if (shapes != null)
                shapes[layer.topNames[0]] = logicalShape;
            owner.DebugLog?.Invoke(
        ...
        private static bool TryGetOrCreateChannelVectorPack4Rt(AexisGraphSession.MemoryDataPack memoryPack, out RenderTexture packedVector)
        {
            ...
            try
            {
                if (memoryPack.pack4Rt != null)
                {
                    AexisGpuResourceTracker.ReleaseTexture(memoryPack.pack4Rt, "NcnnMemoryDataChannelVector.recreate");
        ...
        private static bool ShouldUseVistaPromptPack4RtOnly(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisGraphSession.MemoryDataPack memoryPack)
        {
            return owner != null
            ...
            Dictionary<string, AexisGraphSession.BufferShape> textureShapes)
        {
            ...
            var logicalShape = new AexisGraphSession.BufferShape(memoryPack.dims, memoryPack.w, memoryPack.h, memoryPack.d, memoryPack.c);
            ...
            Dictionary<string, AexisGraphSession.BufferShape> shapes)
        {
            ...
            var logicalShape = new AexisGraphSession.BufferShape(memoryPack.dims, memoryPack.w, memoryPack.h, memoryPack.d, memoryPack.c);
            blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(outArr, logicalShape, logicalShape, owned: true);
            if (shapes != null)
                shapes[layer.topNames[0]] = logicalShape;
            owner.DebugLog?.Invoke(
        ...
        private static bool TryCreateMemoryDataBuffer(AexisGraphSession.MemoryDataPack memoryPack)
        {
        ...
        internal static bool TryGetOrCreateVistaPromptPack4Rt(AexisGraphSession.MemoryDataPack promptMemoryPack, int featureChannels, out RenderTexture promptRt)
        {
            ...
            try
            {
                if (promptMemoryPack.pack4Rt != null)
                {
                    AexisGpuResourceTracker.ReleaseTexture(promptMemoryPack.pack4Rt, "VistaPromptPack4Rt.recreate");
            ...
            rt.Create();
            AexisGpuResourceTracker.RegisterTexture(rt, "VistaPromptPack4Rt");
