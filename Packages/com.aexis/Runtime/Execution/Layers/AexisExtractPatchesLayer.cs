...
namespace Aexis.Execution
{
    ...
    public sealed class AexisExtractPatchesLayer : AexisBaseLayer
    {
        public AexisExtractPatchesLayer()
            : base(AexisLayerTypes.ExtractPatches, supportsBufferPath: false, supportsCommandBufferPath: true)
        {
        }
        ...
        {
            if (!owner._extraPacks.TryGetValue(layer.name, out var packObj) || packObj is not AexisGraphSession.UnfoldPack pack)
            ...
            try
            {
                output = owner.RentTempArray(outStorageShape.w, outStorageShape.h, outDepth, AexisGraphSession.ResolveTensorTextureFormat(outShape.dims));
                if (foldedD)
                {
                ...
                AexisGraphSession.SetTextureBlob(context.textureBlobs, context.textureShapes, layer.topNames[0], output, outShape, outStorageShape);
                output = null;
            }
            finally
            {
        ...
        public override void ExecuteCommandBuffer(AexisGraphSession owner, AexisGraphModel.Layer layer, AexisLayerCommandBufferContext context)
        {
            ...
            try
            {
                output = owner.RentTempArray(context.commandBuffer, outStorageShape.w, outStorageShape.h, outDepth, AexisGraphSession.ResolveTensorTextureFormat(outShape.dims));
                if (foldedD)
                {
                    owner.Ops.ExtractPatchesFoldDPack4(
                ...
                context.blobs[layer.topNames[0]] = AexisGraphSession.CreateCmdTensorRef(output, outShape, outStorageShape, owned: true, blobName: layer.topNames[0]);
        ...
        private static bool CanUsePack4(AexisGraphSession.TensorRef src, AexisGraphSession.BufferShape srcShape, AexisGraphSession.UnfoldPack pack)
        {
            return src != null
                   && src.texture != null
        ...
        private static bool CanUsePack4(AexisGraphSession.CmdTensorRef src, AexisGraphSession.BufferShape srcShape, AexisGraphSession.UnfoldPack pack)
        {
            return src != null
                   && src.texture != null
                   && (srcShape.dims == 3 || srcShape.dims == 4)
        ...
        private static void ResolveOutputShape(AexisGraphSession.BufferShape srcShape, AexisGraphSession.UnfoldPack pack, string layerName, out AexisGraphSession.BufferShape outShape)
        {
            ...
            outShape = srcShape.dims == 4
                ...
                ? new AexisGraphSession.BufferShape(4, outW, outH, Mathf.Max(1, srcShape.d) * pack.kernelW * pack.kernelH, Mathf.Max(1, srcShape.c))
                : new AexisGraphSession.BufferShape(3, outW, outH, 1, outC);
        }

        private static int ResolveSameOrExplicitPadLeft(AexisGraphSession.BufferShape srcShape, AexisGraphSession.UnfoldPack pack)
        {
            var kernelExtentW = pack.dilationW * (pack.kernelW - 1) + 1;
        ...
        private static int ResolveSameOrExplicitPadTop(AexisGraphSession.BufferShape srcShape, AexisGraphSession.UnfoldPack pack)
        {
            var kernelExtentH = pack.dilationH * (pack.kernelH - 1) + 1;
        ...
        private static string DescribeUnsupported(string prefix, AexisGraphModel.Layer layer, AexisGraphSession.TensorRef src, AexisGraphSession.BufferShape shape, AexisGraphSession.UnfoldPack pack)
        {
            return prefix
        ...
        private static string DescribeUnsupported(string prefix, AexisGraphModel.Layer layer, AexisGraphSession.CmdTensorRef src, AexisGraphSession.BufferShape shape, AexisGraphSession.UnfoldPack pack)
        {
            return prefix
                   + " requires dims=3 pack4 texture input"
                   + " | layer=" + (layer != null ? layer.name : string.Empty)
        ...
        private static AexisGraphSession.BufferShape ResolveFoldDStorageShape(AexisGraphSession.BufferShape logicalShape)
        {
