...
namespace AIImage.Inference.Core
{
    ...
    public sealed class OnnxD3ImportResult
    {
        ...
        public OnnxSentisPlanDiagnostic[] diagnostics = Array.Empty<OnnxSentisPlanDiagnostic>();
    ...
    public static class OnnxD3Importer
    {
        ...
        public static OnnxD3ImportResult Import(byte[] modelBytes, OnnxD3ImportOptions options = null)
        {
            ...
            var diagnostics = new List<OnnxSentisPlanDiagnostic>();
            ...
            foreach (var source in model.nodes)
            {
                ...
                if (!OnnxSentisAdapter.TryAdapt(imported, out var canonical, out var diagnostic))
                {
        ...
        private static OnnxSentisImportedNode BuildImportedNode(NodeInfo node, Dictionary<string, TensorInfo> tensors, OnnxD3ImportOptions options)
        {
            ...
            return new OnnxSentisImportedNode
            {
        ...
        private static int OutputRank(NodeInfo node, OnnxSentisImportedNode imported) => node.opType == "NonZero" ? 2 : (node.opType == "OneHot" ? imported.inputRank + 1 : imported.inputRank);
        ...
        private static OnnxSentisPlanDiagnostic WithNode(OnnxSentisPlanDiagnostic diagnostic, string name, string op) => diagnostic ?? new OnnxSentisPlanDiagnostic { code = "import-failed", message = op + " node " + name + " could not be imported.", recommendedAction = "Check the D3 importer contract." };
