...
namespace AIImage.Inference.Core
{
    ...
    public sealed class OnnxSentisNodeContract
    {
    ...
    public sealed class OnnxSentisPlanDiagnostic
    {
    ...
    public static class OnnxSentisShapePlanner
    {
        public static OnnxSentisPlanDiagnostic Validate(OnnxSentisNodeContract node)
        {
            if (node == null)
        ...
        private static OnnxSentisPlanDiagnostic ValidateDynamicCapacity(OnnxSentisNodeContract node)
        {
        ...
        private static string NodeName(OnnxSentisNodeContract node) => string.IsNullOrWhiteSpace(node.name) ? "<unnamed>" : node.name;

        private static OnnxSentisPlanDiagnostic Reject(string code, string message, string action)
        {
            return new OnnxSentisPlanDiagnostic { code = code, message = message, recommendedAction = action };
        }
    }
}
